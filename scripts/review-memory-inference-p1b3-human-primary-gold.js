...
function validateCandidateFixture(fixture) {
  if (!hasExactKeys(fixture, ['name', 'cases'])) {
    throw new TypeError('P1-B3 candidate fixture에는 name과 cases만 있어야 합니다.');
  }
  if (fixture.name !== CANDIDATE_FIXTURE_NAME || !Array.isArray(fixture.cases)) {
    throw new TypeError('P1-B3 candidate fixture identity가 올바르지 않습니다.');
  }
  if (JSON.stringify(fixture.cases.map(item => item.caseId)) !== JSON.stringify(FIXED_CASE_IDS)) {
    throw new TypeError('P1-B3 candidate fixture는 고정된 60개 opaque ID를 순서대로 포함해야 합니다.');
  }
  for (const candidate of fixture.cases) {
    if (
      !hasExactKeys(candidate, ['caseId', 'inputPayload'])
      || !hasExactKeys(candidate.inputPayload, ['evidence', 'expectedSchema'])
      || typeof candidate.inputPayload.evidence !== 'string'
      || candidate.inputPayload.evidence.trim() === ''
      || !EXTRACTION_SCHEMA_IDS.includes(candidate.inputPayload.expectedSchema)
    ) {
      throw new TypeError(`P1-B3 evidence-only candidate가 올바르지 않습니다: ${candidate.caseId}`);
    }
  }
  return fixture;
}
...
function buildCompletedMapping(fixture, labelsByCaseId, completedAt = new Date().toISOString()) {
  ...
  if (!(labelsByCaseId instanceof Map) || labelsByCaseId.size !== fixture.cases.length) {
    throw new TypeError('60개 blind HUMAN label이 모두 있어야 completed mapping을 만들 수 있습니다.');
  }
...
function helpText() {
  return [
    'Usage: npm run review:memory-inference-p1b3-human-primary-gold',
    '',
    `Protocol: ${REVIEW_PROTOCOL_VERSION}`,
    `Completed output: ${DEFAULT_OUTPUT}`,
    '60개 응답이 모두 끝난 뒤에만 output을 새로 만듭니다.',
  ].join('\n');
}
...
async function main(argv = process.argv.slice(2)) {
  if (argv.length > 0) {
    ...
    throw new Error('P1-B3 blind review command는 조정 인자를 허용하지 않습니다.');
  }
  ...
  process.stdout.write('P1-B3 blind HUMAN primary review: 모든 60개 응답 완료 후에만 결과를 저장합니다.\n\n');
  try {
...
if (require.main === module) {
  main().then(
    ...
    error => {
      console.error(`P1-B3 blind HUMAN review failed: ${error.message}`);
      process.exitCode = 1;
    },
  );
}
