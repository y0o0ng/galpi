...
const DEFAULT_OUTPUT = '/tmp/xion-p1b2d-human-primary-labels.json';
const CANDIDATE_FIXTURE_NAME = 'xion-local-memory-inference-p1b2d-triage-boundary-candidates-v1';
const REVIEW_PROTOCOL_VERSION = 'xion-p1b2d-human-primary-v1';
const LABELS = Object.freeze(['NO_WRITE', 'WRITE_CANDIDATE', 'ESCALATE']);
const FIXED_REVIEW_ORDER = Object.freeze([
  8, 2, 13, 0, 11, 5, 14, 7, 3, 10, 1, 12, 6, 4, 9,
]);
const FIXED_CASE_IDS = Object.freeze(Array.from({ length: 15 }, (_, index) => (
  `p1b2d-triage-boundary-${String(index + 1).padStart(3, '0')}`
)));
...
function validateCandidateFixture(fixture) {
  if (!hasExactKeys(fixture, ['name', 'cases'])) {
    throw new TypeError('P1-B2d candidate fixture에는 name과 cases만 있어야 합니다.');
  }
  if (fixture.name !== CANDIDATE_FIXTURE_NAME || !Array.isArray(fixture.cases)) {
    throw new TypeError('P1-B2d candidate fixture identity가 올바르지 않습니다.');
  }
  if (JSON.stringify(fixture.cases.map(item => item.caseId)) !== JSON.stringify(FIXED_CASE_IDS)) {
    throw new TypeError('P1-B2d candidate fixture는 고정된 15개 opaque ID를 순서대로 포함해야 합니다.');
  }
  for (const candidate of fixture.cases) {
    if (
      ...
      throw new TypeError(`P1-B2d evidence-only candidate가 올바르지 않습니다: ${candidate.caseId}`);
    }
  }
  return fixture;
}
...
function buildCompletedMapping(fixture, labelsByCaseId, completedAt = new Date().toISOString()) {
  ...
  if (!(labelsByCaseId instanceof Map) || labelsByCaseId.size !== fixture.cases.length) {
    throw new TypeError('15개 blind HUMAN label이 모두 있어야 completed mapping을 만들 수 있습니다.');
  }
...
function helpText() {
  return [
    'Usage: npm run review:memory-inference-p1b2d-triage-gold',
    '',
    `Protocol: ${REVIEW_PROTOCOL_VERSION}`,
    `Completed output: ${DEFAULT_OUTPUT}`,
    '15개 응답이 모두 끝난 뒤에만 output을 새로 만듭니다.',
  ].join('\n');
}
...
async function main(argv = process.argv.slice(2)) {
  if (argv.length > 0) {
    ...
    throw new Error('P1-B2d blind review command는 조정 인자를 허용하지 않습니다.');
  }
  ...
  process.stdout.write('P1-B2d blind HUMAN primary review: 모든 15개 응답 완료 후에만 결과를 저장합니다.\n\n');
  try {
...
if (require.main === module) {
  main().then(
    ...
    error => {
      console.error(`P1-B2d blind HUMAN review failed: ${error.message}`);
      process.exitCode = 1;
    },
  );
}
