...
const baseUrl = (process.env.AI_COUNCIL_URL || `http://${host}:${port}`).replace(/\/$/, '');
...
server.registerTool(
  'get_config',
  {
    description: 'Read ai-council runtime config and model availability.',
    inputSchema: {},
  },
...
server.registerTool(
  'list_notes',
  {
    description: 'List tracked Obsidian notes from ai-council. Defaults to active notes only.',
    inputSchema: {
...
server.registerTool(
  'search_notes',
  {
    description: 'Search active Obsidian notes through ai-council hybrid note search.',
    inputSchema: {
...
server.registerTool(
  'validate_vault',
  {
    description: 'Run Codex vault validation through ai-council.',
    inputSchema: {},
  },
...
server.registerTool(
  ...
  {
    description: 'Run one pending Codex organize job through ai-council. This may edit vault CODEX sections via the server validation pipeline.',
    inputSchema: {},
  },
...
server.registerTool(
  'archive_note',
  {
    description: 'Archive one active note by filename through ai-council. This moves the note to _archive and updates DB/frontmatter.',
    inputSchema: {
      filename: z.string().regex(/^[^/\\]+\.md$/),
    },
  },
...
server.registerTool(
  'restore_note',
  {
    description: 'Restore one archived note by filename through ai-council. This moves the note back to the active vault and marks it pending.',
    inputSchema: {
      filename: z.string().regex(/^[^/\\]+\.md$/),
    },
  },
...
server.registerTool(
  ...
  {
    description: 'List note merge candidates from ai-council. Read-only: merge execution must be approved through Clawd before any merge API is called.',
    inputSchema: {},
  },
