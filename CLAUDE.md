...
- 다음 후보: **슬롯 구조**(동시보유 수·종목당 비중·진입 빈도)가 다음 축이다. K는 답이 났다 · `jt-k42`에 비용 스트레스와 인접값을 돌려 blocker 둘을 없앤다 · `RX` 하나를 손으로 확정하면 민감도 아티팩트가 사라진다 · BULL-only 게이트는 여전히 별도 PR이고 K 실험이 그 가설을 승격시키지는 않았다(K84는 BULL에서 벤치마크에 가장 크게 졌다).
