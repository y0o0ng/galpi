...
- 홀드아웃(2025-08-07~2026-08-07)은 2026-08-10에 **한 번 소모됐다.** 연구 런은 구간에서 아예 제외해 쓰지 않는다. `record_holdout_run`의 키가 정책 서명이라 **엔진 변경은 세지 못한다**는 것을 알고 쓴다.
- **RS63,5는 실제로 예측한다 — 단 극단 상위 몇 종목에서만이다**(2026-08-14 신호 연구, `runs/signal-rs63/README.md`). NDX100 D1이 +84세션에서 유니버스 평균 대비 +2.15%이고 무작위 20표본 분포 밖이다. 그런데 합집합에서는 D1이 52종목이라 희석돼 사라지고(+0.25%) TOP5만 +2.71%로 남는다. **십분위는 유니버스 크기에 종속되므로 "상위 10%"라는 말로 두 유니버스를 비교하지 않는다.** 효과는 +21부터 붙어 84세션까지 감쇠가 없다.
- **중첩 표본을 유의성으로 읽지 않는다.** 매일 뽑으면 +84 수익률이 83/84 겹쳐 관측이 4,674개처럼 보여도 독립인 것은 56개다. K개 위상을 각각 평균 내 흩어짐을 본다 — 위상 하나만 봤다면 +84에서 +0.26%를 보고 "효과 없음"으로 읽었을 것이고 실제 중앙값은 +2.25%다.
- **PR #9(`trading/rs63-signal-study`)가 열려 있다.** 남은 일은 하나 — 무작위 표본을 `top_n` 크기로도 뽑고 위상 표에 TOP5를 더해 **TOP5의 유의성을 측정으로** 말하는 것이다(현재는 추정만 있다). 25분이고 그 브랜치에 커밋을 얹는다.
- 그다음 후보: K를 63·84로 넓힌다(신호 연구가 근거를 줬다. 몇 개까지 볼지 미리 정한다) · `jt-k42`에 비용 스트레스와 인접값을 돌려 blocker 둘을 없앤다 · `RX` 하나를 손으로 확정하면 민감도 아티팩트가 사라진다. **신호는 종목 선택 층에 있는데 전략 수익으로 이어지지 않으므로, 어느 층에서 새는지가 큰 질문이다.**
