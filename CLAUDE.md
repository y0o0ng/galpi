...
- 현재 개발 상태: V4 논문 검색 2차 Mac 구현 완료. `lib/paper-notes.js`, `note_type: paper`, 활성 노트 기준 `paper_id` 중복 차단, 저장 버튼·새로고침 상태 복원, 기존 임베딩·저장 이벤트 기반 Codex 큐 연결을 구현했고 `node:test` 17개와 임시 서버 mock 통합 검증을 통과했다. Pi 배포·실데이터 인수 전이다.
...
- 코드 구조 방향: `server.js`는 현재 5,573줄. 기존 코드를 한꺼번에 분해하지 말고, 논문 검색·음성 입력 같은 큰 신규 기능은 별도 모듈로 작성하며 서버에는 설정과 얇은 라우트만 둘 것
