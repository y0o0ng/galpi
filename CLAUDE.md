...
- 다음은 **지표 파라미터를 `PolicyVersion`으로 이관**하는 것이다. 1.4가 "지표 파라미터"를 사용자 승인 항목으로 두는데 지금은 모듈 상수라 14.4·21.2의 인접값 안정성 검증(18/20/22일)을 돌릴 수 없다. 그다음이 워크포워드 분할(14.4)과 실데이터다. 이 셋이 있어야 14.7 판정이 의미를 가진다.
