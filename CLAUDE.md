...
- **아직 인수하지 않은 것 둘**: 홈 `알아둘 것`(플래그가 꺼져 있어 화면을 본 적이 없다)과 재확인 질문(`expressed` 관심이 `review_after`에 닿은 적이 없다). 코드와 오프라인 테스트는 있지만 **실물로 본 적은 없다.**
...
- **아직 정하지 못한 것 셋.** `SURFACE_THRESHOLD`(잠정값) · 원문 fetch(문턱과 함께 연다) · 11.2절 최근 언급 매칭이 너무 엄격한 문제. 셋 다 실기사 30~50건 표본이 필요하고 **첫 5건에서는 문턱이 그럴듯해 보였다**(5건 중 1건 통과, 눈으로 봐도 그게 제일 볼 만했다).
