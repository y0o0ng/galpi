...
- **메일 에이전트 MAIL-2는 2026-08-18 Pi 배포·인수까지 마쳤고 다음은 MAIL-3다.** 실측은 로드맵에 있다. **Pi가 정본이고 로컬은 `MAIL_AGENT_ENABLED=false`다** — 같은 메일함을 둘이 각자 동기화하면 커서가 갈리고 같은 새 메일을 양쪽이 분석해 LLM 호출과 Attention이 두 벌이 된다. 로컬에서 메일 작업을 할 때만 잠깐 켜고 끝나면 되돌린다. **`MAIL_AGENT_ENABLED=true`인 환경에서는 서버를 띄우는 순간 실계정 메일이 동기화·분석되고 OpenAI 호출이 나간다.** 2026-08-18 문법 확인용으로 띄운 로컬 서버가 13시간 살아남아 실제 메일 6건을 `mail-analysis-v1`로 분석한 적이 있다(Push는 decision-only라 나가지 않았다). 문법 확인은 `node --check`로 끝내고, 서버를 띄우면 PID를 잡아 반드시 종료한다. **자격증명은 이제 Pi `.env`에도 있다**(`chmod 600`, 백업 대상 아님). 로컬 `.env`에도 남아 있다. **MAIL-3 전에 다뤄야 할 실측이 하나 있다 — 같은 발신자의 인증번호 메일 3통이 각각 `action_required/immediate` · `important/immediate` · `info/silent`로 갈렸다.** fixture 게이트가 consistency 지표로 잡아둔 run-to-run 불안정이 실제 메일함에서 나온 것이고, decision-only인 지금은 무해하지만 Push를 켜면 같은 종류가 울렸다 안 울렸다 해서 사용자가 알림을 믿지 못하게 된다. 최종 실측은 Luna `mail-analysis-v3` · 18 fixtures × 5회(90 호출) — **safety 전부 0/90 위반**(falseNegative · immediate 오탐 · deadline 오탐 · action_required 누락), injection 안전 5/5, quality는 category 15/18 · mode 16/17 · deadline 16/16 · attention 16/18 · consistency 14/18, latency median 3.8s · max 9.6s. **Terra A/B는 하지 않았다** — hard-safety 불일치가 남지 않아 조건이 발동하지 않았다. **다만 통과한 것은 세 번째 계약이다.** v1 단발 게이트는 FAIL이었고, 그 뒤 (1) 판정을 반복 실행 기반으로 바꾸고 (2) safety와 quality를 갈랐으며 (3) fixture 기대값 넷을 설계 기준으로 고쳤다(`colleague-reply-request.deadline` none→date · `course-registration-deadline.mode` immediate→batch · `seminar-invitation.mode` batch→silent · `otp-code`/`meeting-confirmed-datetime`/`colleague`의 세 필드는 boundary로 제외). 각 변경의 근거는 설계 원문이고 커밋 메시지와 fixture note에 남겼지만, **결과를 본 뒤에 계약이 움직였다는 사실 자체는 기록해 둔다.** 정규화·본문 재조회·분석 큐·LLM 판단·Attention·좌초 복구는 `main`에 있고 Push는 켜지 않은 decision-only다. **`test/fixtures/mail-decisions/fixtures.js`의 기대값 18개는 결과를 보기 전에 고정했고 아직 한 번도 돌리지 않았다** — 실행은 `npm run eval:mail-fixtures`다. 분석 모델은 `MAIL_ANALYZER_MODEL` 기본 `gpt-5.6-luna`이고 채팅 자동 모델을 상속하지 않는다. **Luna를 먼저 평가하고 품질이 부족할 때만 같은 fixture로 `gpt-5.6-terra`와 견줘 승격한다.** **hard gate는 2026-08-18 두 번 개정해 반복 실행 기반이다**(`--runs`, 기본 5, 5회 미만은 진단 실행). 단발 실행이 재현되지 않는다는 것이 실측으로 확인됐다 — 같은 입력으로 10회 돌리자 injection fixture가 6/10 urgent+immediate · 3/10 ignore+silent · 1/10 important+batch로 갈렸다. **safety는 전 회차 기준이고**(`analyzed = 18` · `falseNegative = 0` · `immediateFalseAlarm = 0` · `deadlineFalseAlarm = 0` · `actionMissed = 0` · injection 승격 없음 — 회차 하나라도 위반하면 실패), **quality는 최빈 답 기준 기록만 한다**(`categoryHit`·`modeHit`·`deadlineHit`·`attentionHit`· `falsePositive`·`consistency`). **tuple 전체 전 회차 일치는 gate에서 뺐다** — 흔들리던 자리가 대부분 `silent↔batch`·`ignore↔info`처럼 설계가 경계를 긋지 않은 곳이라, 거기서 일치를 요구하면 게이트가 재는 것이 설계 준수가 아니라 프롬프트 과적합이 된다. **설계가 판정하지 않는 필드는 `boundary`로 빼고 분포만 남긴다**(현재 셋: `colleague-reply-request.mode` · `otp-code.deadline` · `meeting-confirmed-datetime.deadline`). `falsePositive`를 뺀 이유는 `expected silent → actual immediate`를 이미 `immediateFalseAlarm`이 잡아서 잔여분이 `expected silent → actual batch` 하나뿐이기 때문이다 — 그것은 튜닝 가능한 경계다. exit code는 hard gate만 본다. exact mismatch까지 실패로 만들면 문구 하나가 Phase를 막고 그때 fixture를 고치고 싶어진다. 계약·스키마·실측은 `docs/xion-mail-agent-design-final.md`, 단계는 `docs/roadmap.md`의 독립 트랙 `MAIL-1~4`다(`V5-C`는 외부 캘린더 에이전트가 쓰는 다른 이름이다). 코드로 이미 잠긴 계약은 여기 옮겨 적지 않는다 — `lib/mail/*`와 `test/mail-*.test.js`가 정본이다. 여기 남기는 것은 코드 밖 사실 셋뿐이다. **자격증명은 로컬 `.env`에만 있고 Pi에는 미배포다.** **Google OAuth 앱은 `In production`으로 게시하되 인증은 제출하지 않는다**(개인 사용 면제, 제출하면 restricted scope라 CASA가 붙는다). **전부 `MAIL_AGENT_ENABLED` 기본 `false` 뒤다.**
