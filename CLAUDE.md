...
- 이제 남은 blocker는 `SURVIVORSHIP_BIASED`와 `EARNINGS_GATE_DISABLED`뿐이고 **둘 다 코드로 없앨 수 없다.** 14.7 판정에 필요한 나머지는 다 갖췄고 남은 것은 데이터 구매 결정이다. point-in-time 구성원·상장폐지 가격·실적 일정이 필요하다.
