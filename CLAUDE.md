...
- 다음 개발 시작점: `docs/voice-halfduplex-design.md`의 H3 검증·되묻기다. 날짜·시각·금액 같은 핵심 슬롯이 불확실할 때 답변 생성 전에 그 슬롯만 되묻고, 정정 왕복을 `pending_turn` 하나로 유지해 메시지 1쌍으로만 저장하며, 되묻기는 누적 2회 상한이다. 병합은 LLM에 맡기지 않고 슬롯 값만 치환한다. 그 뒤 H5 Realtime 정리다. 지연 개선은 스트리밍 TTS(약 1,000ms) → 침묵 1200→1000ms → earcon 순서이며 `/api/chat` 모델 호출부를 스트리밍으로 바꿔야 해서 작지 않다. 음성 턴이 텍스트 답변과 겹칠 때의 `isLoading` 거절 경로는 일부러 만들기 애매해 계속 미검증으로 남을 수 있다. 그 뒤 `docs/voice-halfduplex-design.md`의 H3 검증·되묻기 → H5 Realtime 정리다. 음성 턴이 텍스트 답변과 겹칠 때의 `isLoading` 거절 경로는 실기기 검증 전이다. 지연 개선은 스트리밍 TTS → 침묵 1200→1000ms → earcon 순서로 아직 미구현이다. 지연 개선은 스트리밍 TTS(약 1,000ms) → 침묵 1200→1000ms(200ms) → earcon 순서이며 아직 미구현이다. 폰↔Pi 왕복 3회를 1회로 줄이는 통합은 설계만 남겼고 스트리밍 TTS 구현 시 함께 검토한다. 더 빠른 텍스트 모델로 내리는 선택은 하지 않는다.
