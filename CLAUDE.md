...
- 현재 개발 상태: 일반 노트를 읽기 전용으로 여는 `public/note-panel.js`, 공개 PDF 외부 링크, Clawd의 전체 앱·패널 위 이동을 Mac에서 구현·테스트했으며 Pi 배포 전이다. 단순 열람은 activeNotes를 바꾸지 않고 PDF 다운로드·파싱·Pi 프록시는 하지 않는다.
- 다음 개발 시작점: 최신 변경을 Pi에 배포해 모바일 지식 패널과 PDF 링크를 인수한 뒤, 실제 Claude 질문에서 저장 논문이 자동 컨텍스트로 잡히는지와 Codex가 paper 노트의 태그·링크를 처리하는지 확인한다. 통과하면 V4 음성 입력으로 진행한다. Pi `.env`는 `PAPER_SEARCH_MOCK=false`를 유지한다.
