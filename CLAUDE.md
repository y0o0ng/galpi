...
- **메일 에이전트 MAIL-2는 2026-08-18 코드가 들어갔고 남은 것은 fixture 게이트 실행과 인수다.** 정규화·본문 재조회·분석 큐·LLM 판단·Attention·좌초 복구는 `main`에 있고 Push는 켜지 않은 decision-only다. **`test/fixtures/mail-decisions/fixtures.js`의 기대값 18개는 결과를 보기 전에 고정했고 아직 한 번도 돌리지 않았다** — 실행은 `npm run eval:mail-fixtures`이고 **합격 문턱은 아직 사전등록하지 않았다.** 문턱을 정하기 전에 돌리면 숫자를 보고 기준을 맞추게 되므로 그 순서를 지킨다. 분석 모델은 `MAIL_ANALYZER_MODEL` 기본 `gpt-5.6-luna`이고 채팅 자동 모델을 상속하지 않는다. **Luna를 먼저 평가하고 품질이 부족할 때만 같은 fixture로 `gpt-5.6-terra`와 견줘 승격한다.** 계약·스키마·실측은 `docs/xion-mail-agent-design-final.md`, 단계는 `docs/roadmap.md`의 독립 트랙 `MAIL-1~4`다(`V5-C`는 외부 캘린더 에이전트가 쓰는 다른 이름이다). 코드로 이미 잠긴 계약은 여기 옮겨 적지 않는다 — `lib/mail/*`와 `test/mail-*.test.js`가 정본이다. 여기 남기는 것은 코드 밖 사실 셋뿐이다. **자격증명은 로컬 `.env`에만 있고 Pi에는 미배포다.** **Google OAuth 앱은 `In production`으로 게시하되 인증은 제출하지 않는다**(개인 사용 면제, 제출하면 restricted scope라 CASA가 붙는다). **전부 `MAIL_AGENT_ENABLED` 기본 `false` 뒤다.**
