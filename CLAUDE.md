...
- V4.5-C 약속 루프 구현 상태: `docs/task-reminder-design.md`가 단일 상세 기준이다. schema v5는 `notes.ai_readable`을 Markdown frontmatter와 동기화하고 명시적 `false` 노트를 답변·검색·A1b·논문 전문·MCP AI 읽기·임베딩·Codex 대상/참고 목록·파생 그래프에서 제외하되 사람의 직접 열람은 유지한다. 기존/필드 없는 노트는 `true`, 잘못된 값은 fail-close `false`이며 vault는 평면 구조를 유지한다. schema v6 task·event·reminder 정본과 API에 이어 C1b 결정론적 scheduler, stable fired 알림 합성, `/task`·`/today`, 알림센터 단일 task renderer, 읽기 전용 일정 에이전트 7일 요약을 `assistant-scheduler`·`task-panel`·`agent-panel` 등 독립 모듈로 로컬 구현했다. 앱 시작·focus·visible·foreground 60초 갱신은 기존 채팅 7초 polling과 분리했고 `ASSISTANT_TASKS_ENABLED` 기본값은 `false`다. 전체 회귀 148/148을 통과했다. schema v5/v6과 C1b는 아직 Pi에 적용하지 않았고 1440×900·390×844 실브라우저 확인과 schema v7 private HTTPS·PWA·Web Push가 남아 있다. `owner_agent`와 역할별 쓰기 규칙은 첫 실제 에이전트 노트 writer 때만 추가하며 폴더·범용 ACL·`relative_path`는 미룬다.
...
- 다음 개발 시작점: 2026-07-18 Pi A1b 실사용 trace는 12회 중 hash가 있는 고유 질문 11개이고 1회는 hash 없는 과거 실행이다. 30개 중간 점검까지 고유 질문 19개가 더 필요하며, 그동안 오류·손상 JSON·8,000자/6청크 상한 도달은 0, 추가 지연은 평균 33.3ms·p95 44ms·최대 44ms였다. 30개에서 원문 opt-in review로 과회수·최신성·supersession·abstention을 점검하고 50개에서 A2 전환 여부를 판단한다. 검토 전에는 A2 실제 답변 회수로 전환하지 않는다. 그동안 A1b 경계를 바꾸지 않는 V4.5-C C1을 병행하며, 다음 순서는 C1c Tailscale Serve private HTTPS·PWA·Web Push schema v7 → C1b 실브라우저 viewport 확인 → C1d Pi 일괄 인수다. schema v5/v6을 중간 배포하지 않는다. 백그라운드 탭 지속 refresh는 계약하지 않고 Pi scheduler와 이벤트성 push를 사용한다. 제품 승격 순서는 V4.5→V4-B 음성→V5-A 딜 스카우트→V5-B 주식 분석이다. 단, 딜 스카우트 Phase -1과 게시·LLM·갈피 DB/vault 접근이 없는 Phase 0은 별도 컨펌과 공식 API 키 또는 승인 전 예외 검토 뒤 A1b 관찰과 격리해 병행할 수 있다. 논문 10+10 품질 평가는 별도 컨펌 전이며 Pi `.env`는 `PAPER_SEARCH_MOCK=false`, `npm audit` 기존 경고는 자동 수정하지 않는다.
- 완료된 V3.5: 모든 모델 경로 KST 현재 시각 주입, `[N일 후]` 경과 마커, 짧은 사실 확인 자동 저장 차단, 한 글자 기능어 검색 노이즈 제거
- 코드 구조 방향: `server.js`는 현재 6,914줄. 기존 코드를 한꺼번에 분해하지 말고, V4.5는 topic-store·topic-mutation·migration·retrieval·memory·trace·task·task-route·scheduler를 별도 모듈로 작성하며 서버에는 설정과 얇은 연결만 둘 것
