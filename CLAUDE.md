...
- **현재 의미 권위는 semantic contract v2다 — `fixtures/local-memory-inference-p1b6-skeleton-effective-current-v2.json`, 45 CLEAR / 11 ESCALATE.** v1(34/22)은 덮어쓰지 않고 이전 권위의 역사 증거로 그대로 남아 있고, Exact56(32/24)은 불변 계보다. v2는 **v1의 정확한 원본 바이트에서** 파생되고 11행만 수정·45행은 그대로다. 계약 핵심: **모르는 것은 모호한 것이 아니다**(질문·미결·대략·조건·귀속·원인 미상은 그 자체로 ESCALATE가 아니다), 그리고 **전제를 보태야 성립하는 경쟁 읽기는 인정하지 않는다**(경과 규정·옛 순서·숨은 선호 등). `a19bb9e9`는 **현재 규칙 기본값**이다. `5fc872af`·`be0efa30`·`f58debd8`·`28736b74` 네 개는 skeleton은 ESCALATE지만 **개별 표면은 CLEAR로 붕괴할 수 있어 표면 QA에서 판정한다.**
...
- **301행 strong-model 의미 검토는 외부에서 실행됐고 원본 결과 바이트로 v2에 대해 reconcile했다 — 267 일치 / 34 HUMAN 라우팅.** 영수증은 `…-strong-model-semantic-review-batch-003-attempt-001.json`이고 원본 아티팩트 SHA로 묶여 있다(아티팩트 자체는 커밋 안 함, 리뷰어 모델 정보는 없어서 안 지어냄). **이미 발행된 301행 blind packet 바이트는 그대로다** — packet 출처와 현재 reconcile 권위는 별개다. 34건 중 20건이 위 네 mixed skeleton에 몰렸다.
- **34건 HUMAN 판정 + 32행 보정을 섞은 66행 blind 검토가 끝났다 — `COMPLETE_NEEDS_RESOLUTION`.** 영수증은 `fixtures/local-memory-inference-p1b6-batch-003-human-adjudication-calibration-attempt-001.json`이고 원본 결과(`2515be0e…`)는 커밋하지 않았다. 라우팅 34건은 13 일치(`HUMAN_ADJUDICATED`) / 21 반대(ineligible), 보정 32건은 27 일치 / 5 불일치(ineligible)라 **26건이 명시적 해결 대기**이고 그중 18건이 네 mixed skeleton이다. **다음은 수용이 아니라 이 26건의 해결 결정이다.** 역할은 결과 검증 뒤 원본 strong-model 바이트로 인증된 population에서만 다시 도출한다. **보정 일치는 `HUMAN_ADJUDICATED`로 승격하지 않고(`CATALOG_STRONG_MODEL_CONFIRMED` 유지) 표본 밖 235건으로 외삽하지도 않는다.** 보정 불일치·FIX·REJECT는 그 표면만 fail-close한다. 정본은 P1-B6 설계의 `Combined blind HUMAN adjudication and calibration packet` 절이다. FINAL_HELD_OUT은 반복 HUMAN 검토 대신 선정 후 독립 2차 strong-model 검토를 받는다.
