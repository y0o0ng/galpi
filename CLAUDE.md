...
- **관심 문구가 결과를 지배한다.** `buildQuery`가 topic을 **그대로 검색어로** 쓰고 `reason`은 검색에 안 쓰이며 판단 프롬프트에만 들어간다. 같은 주제라도 문구에 따라 오는 기사가 통째로 달라졌다(`피지컬 AI 관련 정보` → 국내 일반지·보도자료·스팸). **홍보성 보도자료가 세 축을 다 넘는 것은 문턱이 아니라 관심 정의로 고친다.**
