...
- Docker 1단계 개발 상태: `GALPI_DATA_DIR` 공통 경로와 DB/WAL/SHM·Vault·backup 분리, Node.js 24.16.0 multi-stage image, read-only app Compose, native/container test와 amd64/arm64 build CI를 구현했다. 로컬 native 전체 회귀는 211/211이다. 현재 Mac에는 Docker CLI가 없어 실제 container build는 첫 GitHub Actions에서 확인하며, Pi에는 Docker를 배포하지 않고 native `galpi.service`와 host Codex 경계를 유지한다.
