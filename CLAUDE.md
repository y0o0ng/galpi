...
- **하나로 좁혀지지 않으면 고르지 않는다.** 후보 검증은 넷을 다 요구한다 — 제출 이력이 멤버십 구간과 겹칠 것, 8-K를 낼 것, SIC가 있을 것, **구간 안에 Item 2.02 실적 제출이 있을 것**. 마지막 것이 지주회사·중간 법인·신탁을 걸러낸다. 조용히 첫 번째를 고르면 `LEHMAN BROTHERS HOLDINGS`의 첫 매치가 신탁(`0001382976`)이라 자신 있게 틀린다. 최종 실측: 미해결 223 → **37개**, 2층 49개·3층 127개·선행 병합 33개 해결, 실적 126,876행·섹터 869/907.
