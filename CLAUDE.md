...
- **`R/2ATR`가 실제로 수량을 정한다.** 구속 제약이 `RISK` 96%(J63)·97%(J126)이고 `corr(정규화 ATR, 진입 notional 비중)` Spearman이 -0.999·-0.998이라 현재 sizing은 실측상 거의 순수한 inverse-normalized-vol 가중이다. `R ≈ net_notional_return / stop_fraction`도 실측으로 성립한다(J63 0.231 vs 실제 0.229). **다만 이 구조가 변환 손실을 만드는지는 재지 않았다** — 그러려면 수량 규칙을 실제로 바꿔봐야 하고 그것이 다음 PR의 단일 ablation이다.
...
- 다음 후보: **수량 규칙 단일 ablation**이 다음 축이지만 **근거는 아직 부족하다** — 퍼널이 부호는 모든 층에서 유지되는데 R·달러에서 크기가 무너지는 것을 보였고 구속 제약이 `RISK`임을 확인했지만, 정렬 진단이 X3이라 배분 희석 가설이 지지되지 않았다. 다음 PR에서 `R/2ATR` 대안 팔 **하나만** 사전등록해 열되 그 전에 X3의 U자 구조를 어떻게 잴지 정한다 · `jt-k42`에 비용 스트레스와 인접값을 돌려 blocker 둘을 없앤다 · `RX` 하나를 손으로 확정하면 민감도 아티팩트가 사라진다 · BULL-only 게이트와 레짐 조건부 J는 여전히 별도 PR이다.
