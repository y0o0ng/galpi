...
- 다음 개발 시작점: H4 2차(청유형 어휘, 등록·취소가 아닌 말이 오면 카드를 그 턴에 취소)와 TTS 1.3을 Pi에 배포하고 실기기로 인수한다. H4 통과 기준은 말로 등록·취소가 각각 동작하고, 다른 얘기를 하면 카드가 취소되면서 대화가 이어지며, 취소된 카드가 뒤늦은 맞장구로 되살아나지 않는 것이다. 그 뒤 `docs/voice-halfduplex-design.md`의 H3 검증·되묻기 → H5 Realtime 정리다. 음성 턴이 텍스트 답변과 겹칠 때의 `isLoading` 거절 경로는 실기기 검증 전이다. 지연 개선은 스트리밍 TTS → 침묵 1200→1000ms → earcon 순서로 아직 미구현이다. 지연 개선은 스트리밍 TTS(약 1,000ms) → 침묵 1200→1000ms(200ms) → earcon 순서이며 아직 미구현이다. 폰↔Pi 왕복 3회를 1회로 줄이는 통합은 설계만 남겼고 스트리밍 TTS 구현 시 함께 검토한다. 더 빠른 텍스트 모델로 내리는 선택은 하지 않는다.
