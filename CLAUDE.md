...
- 다음 개발 시작점: H2를 Pi에 배포하고 실기기 5턴으로 인수한다(통과 기준: 5턴이 `shared-main`에 순서대로 저장, 헛기침 턴 0건 저장, topic 자동 저장 0건). 그 뒤 `docs/voice-halfduplex-design.md`의 H3 검증·되묻기 → H4 일정·메모 확인 카드 → H5 Realtime 정리다. 지연 개선은 스트리밍 TTS(약 1,000ms) → 침묵 1200→1000ms(200ms) → earcon 순서이며 아직 미구현이다. 폰↔Pi 왕복 3회를 1회로 줄이는 통합은 설계만 남겼고 스트리밍 TTS 구현 시 함께 검토한다. 더 빠른 텍스트 모델로 내리는 선택은 하지 않는다.
