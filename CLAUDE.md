...
- **EDGAR가 유니버스 규모에서 무너진다.** 907종목 중 **223종목이 CIK를 못 찾고** 207종목이 구간 미달이다(실적 103,750행, 섹터 684종목). 원인은 SEC `company_tickers.json`이 10,398개뿐이고 폐지 종목은 물론 `AEP`·`CMA`·`K`·`HES` 같은 현재 대형주도 없다는 것이다. `company_tickers_exchange.json`도 같은 10,398개라 소스를 바꿔서 풀리지 않는다. **이게 위험한 이유는 실적·섹터 게이트가 fail-close라서다** — 못 찾은 223종목은 영영 진입 대상이 안 되고, 그 목록이 폐지 종목 쪽으로 크게 기울어 있어 **생존편향이 실적 게이트로 되돌아온다.** `CIK_OVERRIDES`를 손으로 223개 채우는 것은 답이 아니고 회사 검색 경로가 따로 필요하다.
