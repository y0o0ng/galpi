...
- **ZERO에서 J126이 무너진다.** 총수익 +19.5%(J63) vs +4.7%(J126), 기대값 +0.040 vs **-0.841**로 순위가 둘 다 뒤집힌다. **폐지 건수 때문이 아니다** — J126은 폐지가 6건(J63 9건)인데 ZERO 손실이 -$21,643(J63 -$11,109)으로 건당 3배다. **여기서 열어뒀던 "J126이 저변동 종목을 골라 더 큰 포지션이 된다"는 2026-08-16 퍼널 진단이 재서 기각했다**(아래). 남은 사실은 data-exit 거래가 두 J 모두 큰 notional 쪽에 있다는 것이지 J126 특유의 선택 성향이 아니다.
...
- **"J126이 저변동 종목을 고른다"는 기각됐다.** 체결 진입의 정규화 ATR 중앙 차이가 -3.0%, 진입 notional 비중이 +4.1%로 둘 다 사전등록 문턱 5% 미만이다. 대신 raw 수익률 상위 10거래의 `stop_fraction` 중앙이 J126 12.40% vs 전체 6.90%로 **고변동 꼬리**에 있는데, J당 10건이라 그 자체로는 근거가 약하고 다음 PR에서 따로 사전등록할 가설이다.
- **ZERO 취약성은 data-exit 거래의 위치 문제이고 J126 특유가 아니다.** 폐지·정체불명 거래는 두 J 모두 진입 notional 비중 97~98백분위 · 정규화 ATR 1~3백분위이고 구속 제약도 `CAPITAL`이 절반이다. 표본은 J63 9건 · J126 6건이라 분포 결론으로 확장하지 않는다.
