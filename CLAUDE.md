...
- **P1-B6 batch-002는 HUMAN 결정(64 KEEP / 55 CLEAR / 9 ESCALATE)까지 끝났지만 frozen Exact56 reconciliation이 40 match / 24 mismatch로 `RECONCILIATION_NEEDS_FIX`다.** `humanReviewCompleted`는 false다. **24건은 전부 한 방향이다 — 현재 HUMAN `CLEAR` vs frozen skeleton `ESCALATE`.** 양쪽 라벨은 그대로 두었고 mismatch 목록은 blind 패킷에 절대 들어가면 안 되는 별도 진단 산출물에만 있다. **한 방향으로 쏠린 불일치는 판단 오류 24건이 아니라 경계 정의 문제라서, 사용자가 `CONSERVATIVE_PRAGMATIC_INTERPRETATION`을 승인했고 24건짜리 표적 adjudication 패킷을 만들어 뒀다(`fixtures/local-memory-inference-p1b6-pragmatic-adjudication-batch-002.json`, 10개 skeleton 그룹).** 그 패킷은 blind 패킷이 아니고 현재 HUMAN 결정·frozen humanLabel·의도 라벨·모델 판정을 일부러 빼서 불일치 방향을 숨긴다. **규칙은 prospective이고 기존 라벨은 조용히 바뀌지 않는다 — reconciliation은 여전히 미해결이고 다음은 채택이 아니라 semantic adjudication이다.** accepted pool은 30 그대로이고 final gold·training·raw episode·private replay는 `UNOPENED`, production은 그대로다. 정본은 `docs/Memory research/local-memory-inference/local-memory-inference-p1b6-design.md`와 run receipt다.
