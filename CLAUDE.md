...
- **MAIL-3 전에 정해야 할 것이 하나 있다 — 같은 발신자의 인증번호 메일 3통이 `action_required/immediate` · `important/immediate` · `info/silent`로 갈렸다.** decision-only인 지금은 무해하지만 Push를 켜면 같은 종류가 울렸다 안 울렸다 해서 사용자가 알림을 믿지 못하게 된다. **`reasoning.effort`로는 못 고친다**(2026-08-18 18×10 대조 실험). 이 모델은 `temperature`·`seed`를 거부하고, `none`은 consistency를 12/18 → 15/18로 올렸지만 `deadline 오탐`을 0 → 4/180으로 만들어 safety를 깼다 — 명시된 날짜는 10/10 정확한데 `"목요일 회의 전까지"` 같은 상대 표현을 4/10에서 놓친다. `low`·`medium`은 기본값보다 더 흔들렸다. 남은 선택지는 프롬프트로 OTP류 명시(과적합 위험) · MAIL-3 batch 묶기로 라우팅 층에서 흡수 · Terra A/B · 수용이고 아직 고르지 않았다. 비교는 `npm run eval:mail-fixtures -- --runs N --reasoning X`로 다시 한다.
