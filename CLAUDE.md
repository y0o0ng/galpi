...
- **비용 절감분이 세후 개선분보다 크다.** 비용 $11,965 → $10,941(-1.02%p)인데 세후 총수익 개선은 +0.24%p다. 이것을 "J126의 신호 알파가 더 높다"로 읽지 않는다. 정식 zero-cost counterfactual은 돌리지 않았다.
...
- **SMA200 신규진입 게이트가 economics를 개선했다 — 로드맵 시작 후 처음으로 노출 일치 격차가 양수다**(2026-08-16 PR #16, `runs/market-gate-portfolio/`). control 재현은 정확했고(512거래·+35.63%·0.233R) 처치는 `regime_mode` 하나다. `S` +35.63% → **+47.84%**(`ΔS` +12.20%p), `G` **−10.82% → +4.99%**(`ΔG` +15.81%p). label은 **`ECONOMICS_AND_RELATIVE_IMPROVED`**이고 다음 단계는 사전등록한 **경우 2** — SMA200을 유지한 채 다음 alpha construction으로 간다.
- **그 label을 "타이밍이 좋아졌다"로 읽지 않는다.** 승격 조건은 `ΔS > 0` AND `ΔG > 0`뿐이고 **`ΔB`의 부호를 요구하지 않는다.** 실제로 `ΔB = −3.61%`로 **노출·타이밍 기여는 음수**다. 개선은 전부 matched-SPY 상대 경로에서 왔다. 그래서 label 이름을 `ALPHA_AND_TIMING_IMPROVED`에서 바꿨고, `ΔB < 0`이면서 승격 label이 나오는 조합을 테스트가 값으로 잠근다.
