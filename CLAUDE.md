...
- **재시도 가능한 실패로 `queued`에 갇힌 노트는 저절로 풀리지 않았다.** 그 실패는 노트를 `queued`로 되돌리고 job은 `failed`로 끝내는데, 새 job은 `pending`만 골라서 그 노트가 어느 job에도 다시 들어가지 못했다. 재시작도 `pending`·`running` job만 손댄다. 2026-08-08에 job 선택을 "살아 있는 job이 들고 있지 않은 `pending`+`queued`"로 고치고, 에이전트 탭 사서 Codex 블록에 `대기열 정리` 버튼을 붙였다. 버튼은 기존 `POST /api/organize/queue`를 부르고 자동 큐 문턱을 기다리지 않는다. `/api/organize/status`가 `queueable`·`stranded`를 함께 준다.
...
- 다음은 **EOD $19.99 결제**다. 파이프라인이 실데이터에서 끝까지 도는 것을 봤고 구성원 이력도 갖췄으므로 결제 조건은 충족됐다. 결제하면 15년 가격이 들어와 `NO_WALK_FORWARD`가 풀리고, 구성원 이력과 합쳐 `SURVIVORSHIP_BIASED`도 풀린다. 다만 **편향 없음은 증명해야 하는 주장**이라 `missing_universe_symbols`가 비어야 `delisted_coverage_verified=True`를 넘길 수 있다. 삭제 의무 때문에 구독 한 달 시계가 결제 순간부터 돈다.
