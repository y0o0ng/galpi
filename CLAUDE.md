...
- **알파 개입 네 번 중 셋을 썼다.** 시장 상태(PR #15·#16, **유지**) · 종목 absolute momentum(PR #17, **종료**) · signal-aligned exit(PR #18, **종료**). 남은 것은 FIP 하나다.
...
- **연구 코어 문서에서 0.25%를 "거래당 위험"이라고 부르지 않는다.** 정본 설명은 `core/jt.py` 한 곳에 있다. **이름은 바꾸지 않았다** — `planned_risk`·`open_risk`·`risk_dollars`·`return_r`은 `paper-core-v1`·스키마·배포된 산출물과 공유하는 field라 rename은 별도 PR의 compatibility plan이다. **`paper-core-v1`은 `CORE` 청산이라 2ATR를 실제로 집행하므로 설계 9.1의 "거래당 계획 위험 0.25%"는 그 정책에서 맞는 표현이다.**
