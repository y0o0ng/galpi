...
- 트레이딩 전략 구축 계약: 전략 family마다 로드맵이 하나다. **연구 예산·종료 조건·Phase별 사전등록이 거기 있다.** 실험 산출물 색인은 `trading/runs/README.md`다.
  ...
  - `quality-value` (Phase 0 진행 중 — Step 4 PIT shares/ME까지 구현했고 다음은 Step 5다): `docs/trading/strategies/quality-value-roadmap.md`. **Step 4 구현 정본은 `docs/trading/strategies/qv-step4-shares-me-design.md`이고 P2 same-regime selector·C3 continuity·identity manifest가 거기서 얼어붙었다.**
    - **XBRL QName은 economic identity가 아니다**(사용자 결정으로 Step 4 계약 하나를 다시 열어 닫았다). economic identity bundle은 `issuers`·`share_classes`·`prose_aliases` **세 파일**이고 `xbrl_aliases.jsonl`은 삭제됐다. QName -> class는 그 관계를 등록인이 명시로 세운 **SEC accession 안에서만** 참인 파생 관측이다(`qv_xbrl_class_bindings` · `backtest/qv_xbrl_binding.py`). 옛 시간 구간 alias(`qv_share_class_xbrl_aliases` · `resolve_member`)는 은퇴했고 표만 물리적으로 남는다. 계약은 `docs/trading/strategies/qv-step4-shares-me-design.md` §1.4다.
    ...
    - **표지 제목은 filing 관측이지 자동으로 production prose alias가 아니고(B1), 탄생 증거만으로는 `effective_to = null`이 되지 않는다(B2).** 둘 다 사용자 결정으로 CLOSED다. 5A-2 법적 증거 공급기(`backtest/qv_identity_legal_evidence.py`)가 SEC governing instrument에서 **명시 사실만** 읽고, 생성기와 승격기가 `class_evidence_from_legal_proof` **같은 함수**를 쓴다. 계약은 `docs/trading/strategies/qv-step5-phase0-materialization-design.md`의 5A-2 절이다.
    - **filename 없는 embedded SEC 문서를 `<SEQUENCE>`로 가리킨다**(사용자 결정으로 5A-2 transport 계약 하나를 다시 열어 닫았다). 증거 문서 주소는 typed locator이고 `document_name` **XOR** `document_sequence`다 — 2001년 이전 flat-layout accession의 자연키는 `(CIK, accession, source-backed <SEQUENCE>)`이고 ordinal·합성 파일명·연도 cutoff는 주소가 아니다. 구조가 결정론적이지 않으면 그 accession은 `legacy_layout` fail-close다. 공유 parser는 `backtest/qv_sec_embedded.py` 하나이고 manifest bundle은 `qv-identity-bundle-v3`이라 **그 앞의 5A-1 inventory·5A-2 checkpoint는 stale이다.** legal semantics는 하나도 바뀌지 않았다 — 계약은 `docs/trading/strategies/qv-step5-phase0-materialization-design.md`의 5A-2 절과 `qv-step4-shares-me-design.md` §1.2다.
    - **법적 시점(O2)과 연결 정규화(P2)를 사용자 결정으로 좁게 열어 닫았다.** 발효일은 명시 발효일 · **같은 Exhibit 3 안의 주 증명이 명시한 발효일** · **제출 발효 조항 + 같은 문서의 주 FILED 스탬프** 셋에서만 나온다 — SEC 수리 시각도 서명일 단독도 여전히 법적 시점이 아니다. **주 자료 안의 날짜만** `MM/DD/YYYY`와 대문자 월 이름을 읽고(`FILED 09:00 AM 01/03/2022`), 그 게이트 밖과 공유 `qv_events._iso_date`는 그대로다. 자동 class 연결의 anchor는 exact N1이 먼저이고, 실패했을 때만 **끝에 붙은 숫자 액면가 수식**을 무시한 core designation으로 잇는다. 그 자리에서 **닫는 인용부호 하나**를 지나가는 것은 안전 장치다 — `designated “Common Stock,” par value $0.00001`의 액면가를 못 읽으면 진짜 충돌이 한쪽만 있는 정상 연결로 보인다. **그 정규화는 연결 경계에만 있다** — `qv_manifest.prose_key`·`qv-class-id-v1`·production JSONL은 그대로이고, P2로 연결된 class는 governing 이름의 `GOVERNING_INSTRUMENT` bridge만 얻고 표지 제목은 alias 수명을 받지 못한다.
...
- **P1-B6 exact-56은 CLOSED/FROZEN이고 anchor 표현은 `CLOSED / SINGLE_REPRESENTATIVE`다.** surface batch-001은 source-audit attempt 002에서 32 PASS 뒤 primary blind HUMAN review attempt 001이 31 KEEP / 1 FIX / 0 REJECT로 끝났고, 단일 dialogue-role FIX를 반영해 현재 batch는 fresh source audit가 필요하다. HUMAN review는 닫히지 않았고 학습 실행·raw episode·private replay는 `UNOPENED`, production은 그대로다. 정본은 `docs/Memory research/local-memory-inference/local-memory-inference-p1b6-design.md`와 run receipt다.
