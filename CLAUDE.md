...
- **원인은 비용이 아니라 슬롯 점유다.** K를 두 배로 하면 비용이 절반이 되는데($11,965→$5,958, 회전 1.11→0.59배/년) 그래도 총수익이 낮다. 거래가 514→262로 반토막 나고 `MAX_POSITIONS_REACHED`가 12,243→16,030으로 는다. 노출은 ~18%로 같으므로 **같은 자본이 더 적은 수의 더 긴 베팅에 묶인 것**이고, 손실은 지배적 레짐(`BULL/LOW_VOL`, 2,780/4,676세션)에서 노출 일치 대비 K84 -33.0%p로 나타난다. **다음 축은 K가 아니라 슬롯 수·종목당 비중·진입 빈도다.**
...
- **RS의 진짜 우위는 노출에 있다.** 무작위는 노출 26%로 도는데 RS는 18%다 — 모멘텀 상위 종목은 ATR가 커서 `R/2ATR` 수량이 작아지고 같은 위험에 자본을 덜 쓴다. 그래서 노출을 통제하면 RS가 K42에서 10/10, K84에서 9/10으로 무작위를 이긴다. **다만 그 격차가 K와 함께 커지지는 않는다.**
