...
- V4.5-C 약속 루프 설계·선행 구현 상태: `docs/task-reminder-design.md`를 단일 상세 기준으로 2026-07-19 C0 설계를 개정했다. schema v5는 `notes.ai_readable`을 Markdown frontmatter와 동기화하고 명시적 `false` 노트를 답변·검색·A1b·논문 전문·MCP AI 읽기·임베딩·Codex 대상/참고 목록·파생 그래프에서 제외하되 사람의 직접 열람은 유지하도록 로컬 구현했으며 전체 테스트 132/132를 통과했다. 기존/필드 없는 노트는 `true`, 잘못된 값은 fail-close `false`다. vault는 평면 구조를 유지하고 `owner_agent`와 역할별 쓰기 규칙은 첫 실제 에이전트 노트 writer 때만 추가하며 폴더·범용 ACL·`relative_path`는 미룬다. C1은 명시적 `/task`, 날짜 전용·KST 시각 기한, 단발성 reminder, Today·예정·Inbox, 완료·취소·다시 열기·삭제·복원·확인·1시간 미루기다. 완료·취소는 계속 참조하는 `closed`, 잘못 만든 항목은 일반 검색·AI에서 제외하는 `deleted`이며 물리 purge는 없다. task·event·reminder v6, private HTTPS·PWA·Web Push subscription/delivery v7을 순차 적용하고 알림센터·foreground refresh를 fallback으로 유지한다. 에이전트 탭 최상단에는 7일·지연/오늘/예정/Inbox·오늘/지연 최대 3개·다음 알림·push 상태·`일정 추가 | 전체 일정`을 보여주는 읽기 전용 일정 블록을 둔다. schema v5는 아직 Pi에 적용하지 않았고 task/reminder/push 코드는 아직 구현하지 않았다.
