...
- **다음은 NDX100 과거 판 스냅샷 복구다.** 남은 4구간 중 `DISCK`·`LMCK`는 NDX 변경 이력에 편출만 있고 **편입이 없어** 구간이 floor부터 시작하는데, 두 시리즈는 각각 2008-09·2014-07에 생겼다. 더 중요한 것은 **NDX100이 "Selected changes" 불완전성을 아직 한 번도 점검받지 않았다는 것**이다(SP500에서 그것이 베어스턴스·워싱턴뮤추얼·메릴린치·GM을 통째로 빠뜨렸다). `revision_at("NDX100", ...)`이 2015년을 물어도 2005년 판을 주는데 문서 제목이 리다이렉트라 판 이력이 다른 문서에 있어서다. 제목을 바로잡고 `constituent_symbols`의 400~600개 창을 지수별로 두면 같은 방식이 돈다. 나머지 둘은 `COV`(공고가 닫은 벤더 구멍)·`HET`(구간에 바 0행)이다.
