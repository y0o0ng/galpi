...
- H6a 잠금화면 단축어 서버는 Pi 기술 인수를 마쳤다. schema v11의 활성 Push 구독 연계 hash-only credential·durable receipt와 exact route가 공용 턴 코어·`shared-main`을 쓰며, topic 자동 저장과 일정·노트·Codex 쓰기를 막는다. 코드 flag 기본값은 `false`, 운영 Pi는 `true`이며 활성 iOS 구독 credential 1개를 발급했다. 실제 단일 턴·재시도와 잠금화면 10회는 H6b 인수 전이다.
...
- 다음 음성 작업은 H6b 잠금화면 실제 단일 턴·동일 request ID 재시도와 10회 중 9회 성공 인수다. 이를 통과한 뒤에만 bounded 3턴을 연다. 침묵 1200→1000ms, earcon, H3 되묻기는 각각 별도 변경이며 더 빠른 텍스트 모델로 내리지 않는다.
