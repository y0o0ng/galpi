...
- composer는 위 입력·아래 도구의 두 줄 구조다. 보조 기능과 첨부는 `+` 메뉴에 두고, 오른쪽 주 액션은 빈 입력에서 반이중 음성·입력값이 생기면 전송으로 전환한다. 활성 음성 루프의 종료 버튼은 입력값보다 우선한다. 데스크톱은 전체 폭 하단 바 없이 600px composer만 띄우고 40px 제어를 쓰며, 모바일은 safe-area 하단 면과 44px 타깃을 유지한다.
