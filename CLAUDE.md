...
- **열린 질문(사용자 승인 필요): "계획된 총 손절 위험"을 현재가−손절가로 재고 있어 동시 보유가 5가 아니라 4에서 멈춘다.** 5종목 × 0.25% = 1.25%로 두 한도가 정확히 맞물리는 것은 설계가 진입 시점 계획 위험의 합을 의도했다는 근거다. 대안은 `수량 × max(0, min(현재가, 진입가) − 손절가)`다. 승인 전에는 바꾸지 않는다.
- 다음은 성과 지표(14.7)다.
