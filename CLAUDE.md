...
- H4 일정 카드 음성 확인 구현 상태: 2026-08-01 실기기 피드백으로 H3보다 먼저 구현했다. 카드를 누르려면 폰을 잡아야 해서 핸즈프리가 깨지기 때문이다. 판정은 모델이 아니라 좁은 어휘로 한다. 확인 카드의 존재 이유가 "모델이 틀려도 사람이 막는다"라서 그 확인마저 모델에 맡기면 안전장치가 사라진다. 명령 자체는 정규화 뒤 8자 이하만 후보이고, 문장부호로 조각을 나눠 전부 같은 뜻일 때만 받으며, 앞에 붙는 맞장구와 목적어를 최대 두 번까지 떼고 다시 본다. 조각 하나의 바깥 상한은 14자다. 음성은 자기 요청을 만들지 않고 `TaskPanel.getPendingScheduleConfirmation()`이 노출한 카드 버튼과 똑같은 핸들러를 부르므로 같은 `clientRequestId`로 기존 `POST /api/tasks`를 한 번만 호출한다. 확인 명령 턴은 모델도 저장도 거치지 않고 결과는 카드 상태 문구로 남는다. 등록 실패는 카드를 해제하지 않는다.
- H4 2차 정리: 1차 실기기에서 말로 등록·취소는 모두 동작했으나 `등록해줄래?` 청유형이 목록에 없어 빗나갔고, 어휘가 빗나가면 발화가 LLM으로 가서 같은 일정 카드가 다섯 번 쌓였다. 전자는 `등록`·`저장` 어간 + 허용 꼬리 목록 규칙으로 바꿔 청유형을 받되 `등록할까?`·`등록됐어?` 같은 물음과 미루기는 계속 LLM으로 보낸다. 후자는 처음에 서버에서 `hasPendingScheduleCandidate`로 `schedule_prepare`를 막는 방식을 배포했다가 사용자 판단으로 되돌렸다. 중복은 막았지만 카드가 떠 있는 동안 다른 새 일정도 못 만드는 부작용이 있었고 "나중에 등록" 시나리오가 실제로 쓰이지 않기 때문이다. 대신 등록·취소가 아닌 말이 오면 카드를 그 턴에 조용히 취소하고 평소대로 대화로 넘어간다. 이 규칙 하나가 확인 창(N턴) 개념과 서버 게이트를 함께 없앴고 카드가 LLM 호출 전에 취소되므로 서버는 대기 카드를 알 필요가 없다. `오전 9시로 해줘`가 등록을 부르지 않는 것은 의도한 동작이다. 부분 문자열로 찾았다면 시간 정정 중에 일정이 등록됐을 것이다.
...
- H4 실기기 2차 결과: TTS 1.3은 사용자가 좋다고 인수했다. 말로 등록은 3번 시도해 1번 성공했고 실패한 둘은 모두 정규화 길이가 정확히 9자로 8자 상한에 걸렸다. `등록해 줘. 등록해 줄래?`는 안 먹힌 것 같아 반복해서 말한 경우이고 `일정 카드 등록해줄래?`는 목적어를 붙인 경우다. 8자는 실제 말버릇에 비해 빡빡했으므로 조각 분할(문장부호로 나눠 전부 같은 뜻일 때만 수락)과 접두어 다중 제거(최대 2회, 뗀 나머지가 8자 이하일 때만)를 넣었다. 조각 바깥 상한 14자는 문장을 계속 배제한다. `카드 만들어줘`는 사용자 판단으로 건드리지 않았다. 저장이 없는 것이 성공의 증거라는 점을 기억할 것. 확인 명령 턴은 모델도 저장도 거치지 않으므로 말로 등록이 성공하면 `messages`에는 아무것도 남지 않고 `assistant_tasks`만 는다. 이 비대칭을 모르면 성공을 수동 클릭으로 잘못 읽는다.
- H4 인수 완료: 2026-08-02 3차(조각 분할, 접두어 다중 제거)를 Pi에 배포하고 실기기로 인수했다. `messages` +2(요청·카드 준비)와 그 16초 뒤 `assistant_tasks` +1이 저장 없이 찍혀 말로 등록이 동작함을 확인했고 `note_chunks` 110·`auto_save_decisions` 195는 불변이었다. 어떤 표현을 썼는지는 확인 턴을 저장하지 않으므로 데이터로 확인할 수 없다. H4는 닫는다.
- H3 관측(B) 상태: 2026-08-02 반이중 23턴을 검토한 결과 핵심 슬롯 오전사가 0건이었다. 날짜·시각 7개가 전부 정확했고 오전사 3건은 모두 `시온` 호칭(`지온아`, `시원아`, `시온TV`)이라 뜻을 바꾸지 않는다. H3이 겨냥한 실패는 Realtime을 접으면서 사라진 것으로 보이므로 되묻기 문턱을 추측으로 정하지 않는다. `gpt-transcribe`가 토큰별 logprob을 준다는 것을 실제 호출로 확인했고(`include[]=logprobs`), 동작을 바꾸지 않는 관측만 먼저 배포했다. 로그에는 토큰 수·최저·평균 logprob·-0.5 미만 토큰 수·길이만 남기고 토큰 문자열은 담지 않으며 시각으로 `shared-main`과 대사한다. logprobs가 없어도 전사는 성공하는 fail-open이다. DB·Vault 백업 `20260803-0023`, 코드 복구본 `code-h3b-confidence-pre-20260802-2100.tar.gz` 뒤 배포하고 PID `164771`로 재시작했다. 실제 오전사 표본이 쌓이면 그때 문턱을 정해 되묻기를 붙인다.
- C1 조각 재생 구현 상태: 답변을 문장 조각으로 나눠 TTS를 여러 번 부르고 지금 조각을 재생하는 동안 다음 조각을 미리 받는다. LLM 스트리밍이 없으므로 지연은 아직 줄지 않으며, 목적은 재생 큐와 iOS 동작을 되돌리기 쉬운 단계에서 먼저 검증하는 것이다. 나누기는 새 `POST /api/voice/speak/segments`가 하고 오디오 라우트는 그대로 조각마다 호출한다. 600자 상한과 닫는 말은 `selectSpokenText` 한 곳에서만 적용한다. 소수점·줄임표에서 끊지 않고, 12자 미만 조각은 앞뒤로 붙이며, 조각 수 상한은 8이다. 재생은 제스처로 잠금을 푼 `Audio` 요소 하나만 재사용한다. 조각마다 새 요소를 만들면 iOS에서 두 번째부터 막힌다. 다음 조각 받기는 재생 시작 전에 걸어야 겹친다. 로컬 회귀 308/308이고 브라우저에서 조각 3개·요소 1개·재생 3회를 실측했다. 다만 headless는 마이크를 못 열어 클라이언트 `speak()` 자체가 아니라 같은 순서를 손으로 돌려 확인했다.
- C1 Pi 배포·인수 완료: 2026-08-03 DB·Vault 백업 `20260803-0052`, 코드 복구본 `code-c1-segments-pre-20260803-0130.tar.gz` 뒤 배포하고 PID `166355`로 재시작했다. 파일 5개 hash 일치, Pi 회귀 304/304, 서빙 hash 일치, warning 0건이다. 실기기 4턴에서 863자·388자·99자 답변이 각각 3·4·2조각으로 나뉘어 끊김 없이 이어 재생됐고 사용자가 잘 된다고 인수했다. `note_chunks` 110·`assistant_tasks` 12 불변이다.
- B 첫 신뢰도 실측: 4턴 모두 정확히 전사됐고 `low=0`이었다. `min`은 -0.065, -0.024, -0.323, -0.425로 가장 낮은 값이 표식 -0.5 바로 위다. 이전에 세 번 틀렸던 `시온아`도 이번엔 맞았다(min -0.065). 맞은 턴이 -0.5를 넘지 않는다는 것은 알았지만 **틀린 턴이 넘는지는 여전히 모른다.** 오류 표본이 0건이라 문턱을 정할 수 없다. 덤으로 발화 길이가 최대 18초로 길다는 것을 확인했다. 나중에 침묵 임계값을 만질 때 쓸 값이다.
- C2 스트리밍 구현 상태: 문장이 완성되는 즉시 조각을 progress `speech` 이벤트로 흘려보내 음성이 먼저 읽기 시작한다. **음성 턴에서만 스트리밍하고** 텍스트 채팅은 기존 비스트리밍 호출과 `result` 한 번을 그대로 써서 회귀 위험을 만들지 않았다. 화면 타이핑은 열지 않았다. `runOpenAIResponsesToolLoop`에 `createResponseStream`·`onDelta`·`onDiscardedText`를 더했고, `lib/voice-tts.js`에 점진 분할기 `createSpokenSegmenter`를 만들어 상한 600자와 닫는 말을 스트리밍 중에도 지킨다. 조각이 하나도 안 오면 기존 C1 경로로 자동 폴백한다. 첫 조각이 도착하는 순간 마이크를 닫아 에코를 막는다. 로컬 회귀 319/319다.
- C2 지연 실측 정정: 설계가 근거로 삼았던 `첫 바이트 372ms`는 HTTP 첫 바이트였고 첫 토큰이 아니었다. `gpt-5.6-terra`는 추론 모델이라 첫 텍스트까지 1.2~1.4초가 reasoning에 들어가며 이 시간은 스트리밍으로 줄지 않는다. 실제 절약은 답변 길이에 좌우된다. 12자 4ms, 252자 960ms, 2747자 15.5초였고 우리 코드 경로 끝단 확인에서 295자 답변이 1154ms 앞당겨졌다. 짧은 턴은 체감 차이가 없고 길수록 커진다. 도구 호출 앞에는 텍스트 delta가 0개여서 흘려보낸 문장이 최종 답변이 아니게 되는 경합은 이 모델에서 일어나지 않지만, 생기면 조각을 버리고 로그로 센다.
- C2 실기기 1차 결과: 체감이 확실히 빨라졌다고 인수했고(말하고 1.5~2초 뒤 채팅이 올라옴) 웹 검색 도구를 부르는 92자 발화에 4,089자 답변이라는 최악 조건에서 `⚠️ 음성 조각 폐기` 0건, warning 0건, `note_chunks` 110·`assistant_tasks` 12 불변이었다. 도구 호출 앞에 텍스트를 흘리지 않는다는 것을 운영에서 확인했다. 다만 결함 넷이 나왔다. 조각마다 TTS를 따로 불러 한 문장만 음량이 작았고, 전체 음량이 부족했고, 문서형 답변의 앞 600자를 읽어 개요를 읽다 끊겼고, `시온아`가 또 틀렸다(`시훈아`).
- C2 1차 보정 상태: 넷을 각각 고쳤다. 음량은 서버에서 조각마다 RMS 0.14를 목표로 맞추고 피크 0.97·이득 상한 6배로 클리핑과 잡음 증폭을 막는다. 재려면 조각 전체가 필요해 스트리밍 대신 버퍼링하며 조각이 짧아 비용이 작다. 끊김의 진짜 원인은 C2가 만든 회귀였다. 점진 분할기에 문장 수 제한을 두지 않아 같은 4,089자 답변에서 C1이 105자를 읽던 것을 C2는 598자를 읽고 뚝 끊겼다. 스트리밍도 3조각에서 멈추도록 맞춰 250자·4조각이 됐고, 닫는 말은 `end()`로 미뤄 답변이 마침 상한에서 끝났으면 붙이지 않는다. 여기에 음성 턴은 결론을 먼저 말하게 하는 지시문을 더했다. 마크다운 기호는 벗기지 않는다. 읽힐 것으로 의심했으나 되전사로 확인하니 `## 결론`은 `결론`, `---`는 무음이었고 길이도 7.70초로 같았으며 기호를 뺀 쪽이 오히려 `1.`을 `1점`으로 읽었다. 전사는 `gpt-transcribe`의 `prompt`에 `시온, 시온아, 갈피`만 좁게 넣었고 같은 오디오에서 `시오나` → `시온아`로 바뀌는 것을 확인했다. 문장을 넣으면 전사가 그쪽으로 끌려가므로 이름만 둔다. 로컬 회귀 323/323이다.
- B 두 번째 신뢰도 표본: `시온아`를 `시훈아`로 틀린 턴에서 처음으로 `low=1`, `min=-0.652`가 나왔다. 정확했던 앞선 5턴은 `low=0`, `min`이 -0.024~-0.425였다. 다만 -0.652가 그 이름 토큰이라고 단정할 수는 없다. 토큰 문자열을 남기지 않기 때문이다. 정황은 맞지만 증명은 아니다.
- C2 2차 보정 상태: 실기기에서 마지막 턴 뒤 듣기로 돌아가지 못하는 결함이 나왔다. 원인은 provider가 스트리밍으로 만들며 WAV `data` 크기에 `0xFFFFFFFF`를 적어 보내는 것이었다. C1까지는 청크 전송이라 연결 종료가 끝을 알렸지만 C2에서 버퍼링하며 `Content-Length`가 붙자 재생기가 데이터가 더 온다고 믿고 `ended`를 쏘지 않았다. 정규화하면서 헤더 길이를 진짜 값으로 고쳐 쓴다. 함께 `playAudio`에 감시 타이머를 넣었다. 재생이 4초 동안 진행되지 않으면 넘어가고 `timeupdate`가 오는 동안에는 계속 미룬다. 설계에 "마이크가 꺼진 채 멈추는 상태를 만들지 않는다"고 적어두고도 재생에는 타임아웃이 없었다. 음량 목표는 사용자 요청으로 0.14에서 0.18로 올렸다. 로컬 회귀 326/326이다.
- 다음 개발 시작점: C2 1·2차 보정(음량 0.18, 결론 먼저, 전사 힌트, WAV 헤더, 재생 감시)을 Pi에 배포하고 실기기로 인수한다. 통과 기준은 문장 간 음량이 고르고 전체가 충분히 크며, 긴 답변에서도 앞 두세 문장만으로 답이 되고 중간에 끊긴 느낌이 없으며, `시온`이 제대로 전사되는 것이다. 통과 기준은 긴 답변에서 첫 문장이 눈에 띄게 빨리 들리고, 조각이 순서대로 끊김 없이 이어지며, 짧은 답변과 일정 카드 흐름이 그대로 동작하고, 텍스트 채팅이 변화 없이 동작하는 것이다. 그 뒤 침묵 1200→1000ms와 earcon이 남았고 H3 되묻기는 실제 오전사 표본이 쌓인 뒤로 미룬다. 음성 턴이 텍스트 답변과 겹칠 때의 `isLoading` 거절 경로는 일부러 만들기 애매해 계속 미검증으로 남을 수 있다. 그 뒤 `docs/voice-halfduplex-design.md`의 H3 검증·되묻기 → H5 Realtime 정리다. 음성 턴이 텍스트 답변과 겹칠 때의 `isLoading` 거절 경로는 실기기 검증 전이다. 지연 개선은 스트리밍 TTS → 침묵 1200→1000ms → earcon 순서로 아직 미구현이다. 지연 개선은 스트리밍 TTS(약 1,000ms) → 침묵 1200→1000ms(200ms) → earcon 순서이며 아직 미구현이다. 폰↔Pi 왕복 3회를 1회로 줄이는 통합은 설계만 남겼고 스트리밍 TTS 구현 시 함께 검토한다. 더 빠른 텍스트 모델로 내리는 선택은 하지 않는다.
