...
const FOLLOWUP_CANDIDATE_FIXTURE_NAME = 'xion-local-memory-inference-p1b6-skeleton-pass1-followup-candidates-v1';
const FOLLOWUP_REVIEW_PROTOCOL_VERSION = 'xion-p1b6-skeleton-human-pass1-followup-v1';
const FOLLOWUP_REVIEW_RECEIPT_NAME = 'xion-local-memory-inference-p1b6-skeleton-pass1-followup-receipt-v1';
...
const FIXED_AMBIGUITY_RUBRIC = [
  'Fixed ambiguity rubric:',
  'Judge whether the candidate focus and semantic relations establish one sufficiently resolved decision-relevant interpretation for downstream durability classification.',
  'CLEAR: Explicit user uncertainty, tentativeness, provisionality, temporariness, approximation, or negative status may be CLEAR when that semantic status itself is unambiguous.',
  'ESCALATE: Choose ESCALATE only when materially different decision-relevant interpretations remain unresolved.',
  'CLEAR does not mean durable or memory-worthy. Do not decide durability.',
].join('\n');
...
function validateSkeletonCandidateFixture(fixture, options = {}) {
  const fixtureName = options.fixtureName || CANDIDATE_FIXTURE_NAME;
  ...
  if (fixture.name !== fixtureName || !Array.isArray(fixture.candidates)) {
    throw new TypeError('P1-B6 skeleton candidate fixture identity가 올바르지 않습니다.');
  }
  if (fixture.candidates.length < minCandidates) {
    throw new TypeError(`P1-B6 blind review fixture에는 최소 ${minCandidates}개 candidate가 있어야 합니다.`);
  }
...
function parseSkeletonCandidateFixture(rawBytes, options = {}) {
  return validateSkeletonCandidateFixture(JSON.parse(Buffer.from(rawBytes).toString('utf8')), options);
}
...
function deterministicReviewOrder(fixture, options = {}) {
  const protocolVersion = options.protocolVersion || REVIEW_PROTOCOL_VERSION;
  validateSkeletonCandidateFixture(fixture, options);
  const order = fixture.candidates
    .map((candidate, canonicalIndex) => ({
      canonicalIndex,
      id: candidate.semanticSkeletonId,
      key: crypto.createHash('sha256')
        .update(`${protocolVersion}\0${candidate.semanticSkeletonId}`)
        .digest('hex'),
    }))
...
function buildCompletedReviewReceipt({
  fixture,
  fixtureSha256,
  reviewsById,
  completedAt,
  fixtureName = CANDIDATE_FIXTURE_NAME,
  protocolVersion = REVIEW_PROTOCOL_VERSION,
  receiptName = REVIEW_RECEIPT_NAME,
}) {
  validateSkeletonCandidateFixture(fixture, { fixtureName });
...
function validateCompletedReviewReceipt(receipt, rawFixtureBytes, options = {}) {
  const fixtureName = options.fixtureName || CANDIDATE_FIXTURE_NAME;
  const protocolVersion = options.protocolVersion || REVIEW_PROTOCOL_VERSION;
  const receiptName = options.receiptName || REVIEW_RECEIPT_NAME;
  const fixture = parseSkeletonCandidateFixture(rawFixtureBytes, { fixtureName });
