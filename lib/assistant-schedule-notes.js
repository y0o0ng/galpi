...
function buildActiveScheduleContext(snapshot, options = {}) {
  ...
  const lines = [
    ...
    '반복 일정은 가장 이른 회차 하나만 실린다. 나머지 회차는 접혀 있고 [반복 #id]에 남은 개수를 적는다.',
  ];
  ...
  activeTasks.forEach(task => {
    ...
    const marks = [`[#${task.id}]`];
    if (task.series) {
      // 반복은 가장 이른 회차 하나만 들어 있다. 남은 회차 수를 함께 적지 않으면
      // 이 반복이 한 번만 남은 것처럼 읽힌다.
      const remaining = Number.isFinite(task.series.remaining) && task.series.remaining > 0
        ? `, 다음 회차 ${task.series.remaining}개`
        : '';
      marks.push(`[반복 #${task.series.id} ${escapeContext(seriesRuleLabel(task.series))}${remaining}]`);
    }
...
function historyDay(task) {
  if (task.dueKind === 'date') return Number(task.dueDate.slice(8, 10));
  if (task.dueKind === 'datetime') return kstParts(task.dueAt).day;
  return null;
}

// 매일 반복이면 한 달에 30줄이 쌓인다. 같은 시리즈의 회차는 한 항목으로 묶고
// 날짜만 나열한다. 제목이 다르면(사용자가 그 회차만 고친 경우) 따로 묶어서
// 그 결정을 지우지 않는다.
function seriesGroupEntry(tasks) {
  const first = tasks[0];
  const days = tasks.map(historyDay).filter(Number.isFinite);
  const times = new Set(
    tasks.filter(task => task.dueKind === 'datetime')
      .map(task => formatKstDateTime(task.dueAt).slice(11))
  );
  const time = times.size === 1 ? ` · ${[...times][0]}` : '';
  const lines = [`- **반복 ${tasks.length}회${time}** · ${escapeContext(first.title)}`];
  if (oneLine(first.detail)) lines.push(`  - 설명: ${escapeContext(first.detail)}`);
  if (days.length > 0) lines.push(`  - 날짜: ${days.join('·')}일`);
  return lines.join('\n');
}

// 정렬된 목록에서 시리즈 회차만 묶는다. 묶인 항목은 그 시리즈의 가장 이른 회차
// 자리에 남으므로 재생성해도 순서가 같다.
function historySection(tasks) {
  ...
  for (const task of tasks) {
    if (task.seriesId === null || task.seriesId === undefined) {
      order.push({ single: task });
      continue;
    }
    const key = `${task.seriesId}|${task.title}`;
    if (!groups.has(key)) {
      groups.set(key, []);
      order.push({ key });
    }
  ...
  return order
    .map(item => (item.single ? historyEntry(item.single) : seriesGroupEntry(groups.get(item.key))))
    .join('\n');
}
...
function buildScheduleHistoryNote({ monthKey, tasks, previousRaw = '', updatedAt }) {
  ...
  const completedText = historySection(completed);
  const cancelledText = historySection(cancelled);
