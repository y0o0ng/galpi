...
function serializeActiveNoteIdentifiers(activeNotes) {
  if (!Array.isArray(activeNotes)) return null;
  const filenames = [];
  const seen = new Set();
  ...
  function record({
    ...
    try {
      insertRun({
        ...
        // 이 hash는 예나 지금이나 사용자 발화 원문이다. 해석된 질의는 별도 칸이다.
        querySha256: query ? sha256(query) : null,
        resolutionOutcome: resolution?.outcome || null,
        retrievalQuerySha256: resolution?.retrievalQuery
          ? sha256(resolution.retrievalQuery)
          : null,
