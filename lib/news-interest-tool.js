...
function summarize(interest) {
  return { topic: interest.topic, state: interest.state };
}
...
function createNewsInterestSession({ interests = [], apply, maxCalls = 2 } = {}) {
  ...
  function buildActions(input) {
    ...
    if (!existing) {
      return { actions: [{ op: 'add', topic, state, reason }], added: true };
    }
  ...
  return {
    ...
    async execute(name, input = {}) {
      ...
      try {
        ...
        if (!plan.removed && after) saved.push(summarize(after));
        return {
