...
// `last_seen`의 뜻: **사용자가 마지막으로 그 관심을 실제로 표현하거나 언급한 시점.**
// 시스템이 재확인 예정일을 미뤘다는 이유로는 바뀌지 않는다. 이 값이 곧 "언제부터
// 조용했나"의 기준이라, 시스템이 건드리는 순간 재확인 판정이 자기 꼬리를 문다.
// `reschedule`이 `update`와 따로 있는 이유는 위의 `last_seen` 정의 하나 때문이다.
// 재확인 예정일을 미루는 것은 시스템의 일정 관리이지 사용자가 그 주제를 말했다는
// 뜻이 아니다. 두 뜻을 한 op에 담으면 미루기가 언급 기록을 위조한다.
const OPS = ['add', 'update', 'remove', 'reschedule', 'noop'];
...
// 수집이 실제로 던지는 검색어. topic보다 길게 허용한다 — topic은 사람이 읽는
// 이름이라 짧아야 하지만 검색어는 주제를 설명해야 한다. normalize.js가 180자로
// 자르므로 그보다 길게 두지 않는다.
...
const FIELD_PATTERN = /^(state|last_seen|review_after|reason|aliases|query):\s*(.*)$/;
...
function interestSection(interest) {
  ...
  // 생성된 검색어는 사람이 볼 수 있는 자리에 둔다. 보이지 않으면 "왜 이런 게
  // 왔지"를 되짚을 수 없고, 사용자가 말하지 않은 문자열이 조용히 수집을 정하게 된다.
  if (interest.query) lines.push(`query: ${interest.query}`);
  ...
  actions.forEach(action => {
    ...
    if (op === 'add') {
      ...
      interests.push({
        ...
        query: oneLine(action.query).slice(0, MAX_QUERY_CHARS) || null,
      });
      return;
    }

    if (op === 'reschedule') {
      // 예정일만 옮긴다. state·topic·reason·last_seen은 손대지 않는다.
      const target = findExactlyOne(interests, action.interestId, 'reschedule');
      target.reviewAfter = oneLine(action.reviewAfter) || null;
      return;
    }
    ...
    if (action.query !== undefined) {
      target.query = oneLine(action.query).slice(0, MAX_QUERY_CHARS) || null;
    }
