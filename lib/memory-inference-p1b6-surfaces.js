...
function validateSurfaceBatch(batch, options = {}) {
  ...
  const nameMatch = /^xion-local-memory-inference-p1b6-surface-batch-(\d{3})-v1$/u.exec(batch.name);
  const idMatch = /^p1b6-surface-batch-(\d{3})$/u.exec(batch.batchId);
  if (!nameMatch || !idMatch || nameMatch[1] !== idMatch[1]) {
