...
function createShadowRetriever({
  searchNotes,
  readChunks,
  maxNotes = DEFAULT_NOTE_RECALL_K,
} = {}) {
  if (typeof searchNotes !== 'function') throw new TypeError('노트 검색 함수가 필요합니다.');
  if (typeof readChunks !== 'function') throw new TypeError('청크 조회 함수가 필요합니다.');

  return async function retrieveShadowEvidence(testCase) {
    const candidates = await searchNotes(testCase);
    const selectedNotes = (Array.isArray(candidates) ? candidates : []).slice(0, maxNotes);
    const chunks = await readChunks(selectedNotes, testCase);
    return buildShadowRetrieval({
      query: testCase.query,
      queryEmbedding: testCase.queryEmbedding,
      noteCandidates: selectedNotes,
      chunks,
    });
  };
}

function createGlobalShadowRetriever({ searchNotes, readGlobalChunks } = {}) {
  if (typeof searchNotes !== 'function') throw new TypeError('노트 검색 함수가 필요합니다.');
  if (typeof readGlobalChunks !== 'function') {
    throw new TypeError('전역 청크 후보 조회 함수가 필요합니다.');
  }

  return async function retrieveGlobalShadowEvidence(testCase) {
    const noteCandidates = await searchNotes(testCase);
    const chunks = await readGlobalChunks(testCase);
