...
const ISOLATED_MANUAL_KINDS = new Set([
  'parserIssues',
  'unverifiableChunks',
]);

function isIsolatedManualOperation(operation) {
  return operation?.status === 'manual_review' && ISOLATED_MANUAL_KINDS.has(operation.kind);
}

function isolatedFilenamesForPlan(plan) {
  return new Set(plan.operations
    .filter(operation => ISOLATED_MANUAL_KINDS.has(operation.kind))
    .map(operation => operation.target?.filename)
    .filter(Boolean));
}

function isIsolatedOperation(operation, isolatedFilenames) {
  return isIsolatedManualOperation(operation)
    || isolatedFilenames.has(operation?.target?.filename);
}
...
function selectApprovedOperations(plan, approvedOperationIds = []) {
  ...
  const isolatedFilenames = isolatedFilenamesForPlan(plan);
  ...
  for (const operation of plan.operations) {
    ...
    if (operation.status === 'ready') {
      if (isIsolatedOperation(operation, isolatedFilenames)) continue;
...
function assertRepairResultIsSafe(initialPlan, selectedOperations, finalPlan) {
  const selectedIds = new Set(selectedOperations.map(operation => operation.id));
  const isolatedFilenames = isolatedFilenamesForPlan(initialPlan);
  const allowedRemainingIds = new Set(initialPlan.operations
    .filter(operation => isIsolatedOperation(operation, isolatedFilenames))
    .map(operation => operation.id));
  const finalIds = new Set(finalPlan.operations.map(operation => operation.id));

  for (const id of selectedIds) {
    if (finalIds.has(id)) throw new Error(`복구 작업이 적용 후에도 남아 있습니다: ${id}`);
  }
  for (const operation of finalPlan.operations) {
    if (!allowedRemainingIds.has(operation.id)) {
      throw new Error(`복구 후 새롭거나 미해결인 무결성 문제가 있습니다: ${operation.id}`);
    }
  }
  for (const id of allowedRemainingIds) {
    if (!finalIds.has(id)) {
      throw new Error(`격리된 무결성 문제의 감사 기록이 예상과 달라졌습니다: ${id}`);
    }
  }
}
...
async function applyTopicRepair({
  ...
  try {
    ...
    try {
      ...
      assertRepairResultIsSafe(migratedPlan, operations, finalPlan);
      db.exec('COMMIT');
    } catch (error) {
...
function formatTopicRepairResult(result) {
  ...
  return [
    ...
    `Final audit: ${result.finalAudit.healthy ? 'passed' : 'isolated findings preserved'}`,
  ].join('\n');
}
