...
// 검증기는 실패한 노트마다 파일명 한 줄과 `- 경고` 줄들을 낸다. 그 블록을 노트별로
// 쪼갠다. 반드시 redact 이전 원문에 대고 돌려야 한다 — 이름을 `[노트]`로 가린 뒤에는
// 어느 경고가 어느 노트 것인지 되살릴 수 없다.
function parseCodexValidationWarnings(error, filenames = [], maxPerNote = 8) {
  const text = typeof error === 'string'
    ? error
    : [error?.stderr, error?.stdout, error?.message].filter(Boolean).join('\n');
  const known = new Set((Array.isArray(filenames) ? filenames : []).map(String));
  const warnings = new Map();
  let current = null;
  for (const rawLine of String(text).split(/\r?\n/)) {
    const line = rawLine.trim();
    if (!line) continue;
    const basename = line.slice(line.lastIndexOf('/') + 1);
    if (known.has(basename)) {
      current = basename;
      if (!warnings.has(current)) warnings.set(current, []);
      continue;
    }
    if (!current || !line.startsWith('- ')) continue;
    const list = warnings.get(current);
    const message = line.slice(2).trim().slice(0, 200);
    if (message && list.length < maxPerNote && !list.includes(message)) list.push(message);
  }
  for (const [filename, list] of warnings) {
    if (list.length === 0) warnings.delete(filename);
  }
  return warnings;
}
