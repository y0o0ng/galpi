...
//
// **문턱이 정해지기 전에는 홈에 올리지 않는다.** `SURFACE_THRESHOLD`는 아직 실데이터로
// 정하지 못한 잠정값인데(analyze.js), 표본을 모으려면 수집을 켜야 하고 수집을 켜면
// 홈 노출도 함께 열린다. 그 둘을 떼는 것이 `config.surfaceEnabled`다 — 수집·판단·조회는
// 돌면서 홈만 조용하다. 문턱을 정한 뒤에 켠다.
//
