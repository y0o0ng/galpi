...
답할 때는 각 기사가 **어느 관심 때문에 왔는지** 함께 말한다. 사용자가 "왜 이걸 가져왔어?"라고 물으면 결과의 이유를 그대로 쓴다.
찾은 것이 없으면 지어내지 말고 없다고 답한다. 도구를 부르지 않은 채 뉴스 내용을 추측해서 답하지 않는다.`;
    ...
    execute(name, input = {}) {
      ...
      if (input.topic) {
        ...
        if (!matched) {
          ...
          return { isError: true, content: `추적 중이 아닌 주제입니다. 지금 지켜보는 것: ${known}` };
        }
        interestId = matched.interestId;
      }
      ...
      const results = rows
        .map(row => {
          const topics = (row.interestIds || []).map(id => byId.get(id)?.topic).filter(Boolean);
