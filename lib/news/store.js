  ...
  return {
    ...
    createReviewCandidate({ interestId, question, at }) {
      ...
      const open = db.prepare(`
        SELECT id FROM news_review_candidates
        WHERE interest_id = ? AND state IN ('pending', 'delivered')
      `).get(id);
      if (open) return null;
    ...
    /**
     * 이 기사를 데려온 관심들. 판단은 그중 노트에 아직 살아 있는 것으로 한다 —
     * 사라진 관심의 기사를 판단해봐야 보여줄 자리가 없다.
     */
    interestIdsForArticle(articleId) {
      return db.prepare(`
        SELECT interest_id AS interestId FROM news_article_interests
        WHERE article_id = ? ORDER BY first_seen_at ASC, interest_id ASC
      `).all(articleId).map(row => row.interestId);
    },
    ...
    /**
     * 홈에 올릴 후보. **topic은 여기서 못 채운다** — 관심 이름의 정본이 노트라
     * 이 표에는 `interest_id`밖에 없다. 호출부가 노트를 읽어 이름을 붙인다.
     * 그 불편이 곧 "노트가 정본"의 값이다(설계 22.1).
     */
    briefingArticles({ minRelevance = 0, minImportance = 0, limit = 5 } = {}) {
      const rows = db.prepare(`
        SELECT id, title, url, source, published_at AS publishedAt,
               summary, judgment_reason AS reason, relevance, novelty, importance
        FROM news_articles
        WHERE analysis_state = 'done'
          AND surface_state != 'dismissed'
          AND relevance >= @minRelevance
          AND importance >= @minImportance
        ORDER BY COALESCE(published_at, first_seen_at) DESC, id DESC
        LIMIT @limit
      `).all({ minRelevance, minImportance, limit });
      return rows.map(row => ({ ...row, interestIds: this.interestIdsForArticle(row.id) }));
    },
    ...
    searchArticles({ query, interestId, since, until, limit = 5 } = {}) {
      const clauses = ["analysis_state = 'done'"];
      const params = { limit: Math.min(Math.max(Number(limit) || 5, 1), 20) };
      if (query) {
        // LIKE 와일드카드는 이스케이프한다. `%` 한 글자가 전건 조회가 된다.
        params.query = `%${String(query).replace(/[\\%_]/g, '\\$&')}%`;
        clauses.push("(title LIKE @query ESCAPE '\\' OR summary LIKE @query ESCAPE '\\' OR judgment_reason LIKE @query ESCAPE '\\')");
      }
      if (Number.isFinite(since)) { params.since = Math.floor(since); clauses.push('COALESCE(published_at, first_seen_at) >= @since'); }
      if (Number.isFinite(until)) { params.until = Math.floor(until); clauses.push('COALESCE(published_at, first_seen_at) <= @until'); }
      if (interestId) {
        params.interestId = String(interestId);
        clauses.push('id IN (SELECT article_id FROM news_article_interests WHERE interest_id = @interestId)');
      }
      const rows = db.prepare(`
        SELECT id, title, url, source, published_at AS publishedAt,
               summary, judgment_reason AS reason, relevance, importance
        FROM news_articles
