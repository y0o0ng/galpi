...
function createNewsStore(db, options = {}) {
  ...
  return {
     ...
     */
    briefingArticles({ minRelevance = 0, minImportance = 0, limit = 5, interestIds = null } = {}) {
      const params = { minRelevance, minImportance, limit };
      let scope = '';
    ...
    searchArticles({ query, interestId, since, until, limit = 5 } = {}) {
      ...
      if (query) {
        ...
        clauses.push("(a.title LIKE @query ESCAPE '\\' OR ai.summary LIKE @query ESCAPE '\\' OR ai.judgment_reason LIKE @query ESCAPE '\\')");
      }
