...
// 뒤에 relevance 높은 후보에만 하기로 한 것이다(설계 17.1). 문턱은 아래에서 정해졌어도
// 원문 fetch는 별도 결정이라 아직 열지 않는다 — 기준 없이 전부 가져오면 비용만 늘고
// 판단은 그대로다.
//
...
// Pi 실데이터 (기사, 관심) 12쌍으로 정했다(2026-08-21, 관심 2개). 설계 13절이 요구한
// 30~50건에는 못 미치므로 표본이 쌓이면 다시 본다.
//
// **relevance 0.6** — 12쌍에서 값이 두 덩어리로 갈리고 그 사이가 비어 있다
// (`0.36` 아래 6건, `0.75` 위 6건). 관심을 하나 더해도 갭이 유지됐다.
//
// **importance 0.4** — relevance를 넘은 6건이 `0.40~0.70`에 몰려 있다. `0.5`로 두면
// 사용자가 보고 싶다고 한 제품 변경 소식(Claude 워터마킹, `0.43`·`0.45`)이 죽는다.
//
// **novelty 0.4** — 판단은 처음부터 novelty를 쟀지만 게이트가 쓰지 않아서, 기존 보고서
// 해설(`rel 0.75` · `nov 0.20` · `imp 0.40`)이 나머지 두 축만으로 통과했다. 프롬프트가
// "이미 알려진 것의 재보도, 총정리, 해설, 홍보는 낮다"고 정의해둔 값을 실제로 쓴다.
const SURFACE_THRESHOLD = { relevance: 0.6, novelty: 0.4, importance: 0.4 };
...
/**
 * 홈에 올릴 후보인가. 세 축을 모두 넘어야 한다 — 주제가 맞고(relevance), 달라진
 * 것이고(novelty), 이 사람의 판단을 바꾸는가(importance). 한 축이라도 빼면 그 축이
 * 낮은 기사가 나머지 둘만으로 올라온다.
 */
function meetsSurfaceThreshold(decision, threshold = SURFACE_THRESHOLD) {
  return decision.relevance >= threshold.relevance
    && decision.novelty >= threshold.novelty
    && decision.importance >= threshold.importance;
}
