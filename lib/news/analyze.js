...
// 판단 단계. 큐에서 기사 하나를 집어 관심과 견주고 relevance·novelty·importance와
...
function createNewsAnalyzer(options = {}) {
  ...
  async function analyzeOne(article, interestsById, nowSeconds) {
    const links = store.interestIdsForArticle(article.id);
    // 이 기사를 데려온 관심이 노트에서 사라졌으면 판단할 이유가 없다. 실패가
    // 아니라 대상이 아닌 것이므로 skipped다(설계 16).
    const interest = links.map(id => interestsById.get(id)).find(Boolean);
    if (!interest) {
      store.skipAnalysis({ id: article.id, leaseUntil: article.leaseUntil, code: 'NEWS_INTEREST_GONE' });
      return { id: article.id, outcome: 'skipped', code: 'NEWS_INTEREST_GONE' };
    }

    const output = await callModel({
      model,
      system: SYSTEM_PROMPT,
      input: buildPrompt({ interest, article, nowSeconds }),
      schema: DECISION_SCHEMA,
      schemaName: 'news_decision',
    });
    const decision = validateDecision(output);
    const settled = store.completeAnalysis({
      id: article.id,
      leaseUntil: article.leaseUntil,
      ...decision,
    ...
    return {
      id: article.id,
      outcome: settled ? 'done' : 'lost_lease',
  ...
  async function run() {
    ...
    for (const article of claimed) {
      try {
        ...
        outcomes.push(await analyzeOne(article, interestsById, nowSeconds));
      } catch (error) {
        const state = error?.retryable === false
          ? store.skipAnalysis({ id: article.id, leaseUntil: article.leaseUntil, code: error.code || 'NEWS_DECISION_INVALID' }) && 'skipped'
          : store.failAnalysis({
            id: article.id,
            leaseUntil: article.leaseUntil,
        ...
        outcomes.push({ id: article.id, outcome: state || 'lost_lease', code: error?.code || 'UNKNOWN' });
        // 제목·요약은 로그에 넣지 않는다. 오류 코드만 남긴다.
        onError(error, { articleId: article.id });
      }
    }
    return outcomes;
  }
