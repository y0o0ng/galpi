...
function kstDateTime(epoch) {
  const parts = kstParts(epoch);
  return `${formatDateParts(parts.year, parts.month, parts.day)}T${String(parts.hour).padStart(2, '0')}:${String(parts.minute).padStart(2, '0')}:${String(parts.second).padStart(2, '0')}+09:00`;
}
...
function preparedCreatePayload(normalized) {
  const due = normalized.dueKind === 'date'
    ? { kind: 'date', date: normalized.dueDate }
    : normalized.dueKind === 'datetime'
      ? { kind: 'datetime', at: kstDateTime(normalized.dueAt) }
      : { kind: 'none' };
  return {
    ...
    due,
    reminderAt: normalized.reminderAt === null ? null : kstDateTime(normalized.reminderAt),
  };
}
...
// 기한이 있는데 사용자가 알림을 걸지 않았을 때 대신 잡아주는 시각.
// 시각이 있으면 기한 10분 전, 날짜만이면 그날 아침 9시다. 기한이 없으면 잡을 자리가 없다.
const AUTO_REMINDER_LEAD_SECONDS = 10 * 60;
const AUTO_REMINDER_DATE_HOUR = 9;

function autoRemindAt(task) {
  if (task.dueKind === 'datetime') return task.dueAt - AUTO_REMINDER_LEAD_SECONDS;
  if (task.dueKind === 'date') {
    const { year, month, day } = parseKstDate(task.dueDate);
    return localKstEpoch({ year, month, day, hour: AUTO_REMINDER_DATE_HOUR });
  }
  return null;
}
...
function createAssistantTaskStore(db, options = {}) {
  ...
  const insertTask = db.prepare(`
    INSERT INTO assistant_tasks (
      ...
      due_kind, due_date, due_at, series_id, occurrence_date, created_at, updated_at
    ) VALUES (
      @clientRequestId, @createPayloadSha256, @title, @detail,
      @dueKind, @dueDate, @dueAt, @seriesId, @occurrenceDate, @now, @now
    )
  `);
  ...
  const insertReminder = db.prepare(`
    INSERT INTO assistant_reminders (
      task_id, remind_at, origin, occurrence_key, snoozed_from_id,
      snooze_request_key, created_at, updated_at
    ) VALUES (
      @taskId, @remindAt, @origin, @occurrenceKey, @snoozedFromId,
  ...
  const getNextPendingReminder = db.prepare(`
    ...
    LIMIT 1
  `);
  const getSeriesRule = db.prepare(`
    SELECT id, freq, by_weekday AS byWeekday, by_monthday AS byMonthday,
           time_kind AS timeKind, time_of_day AS timeOfDay, status
  ...
  // 기한이 있는 활성 일정은 살아있는 알림이 항상 하나다. 사용자가 정했으면 그것이고,
  // 아니면 이 기본 알림이다. 없으면 기한이 지나도 조용히 `overdue`로 넘어가기만 한다.
  //
  // 이미 지난 시각으로는 만들지 않는다. 기한 5분 전에 만든 일정에 "10분 전" 알림을
  // 지금 쏘는 것은 방금 그 일정을 적은 사람에게 아무것도 알려주지 않는다.
  function insertAutoReminder(taskId, task, reminderVersion, now) {
    const remindAt = autoRemindAt(task);
    if (remindAt === null || remindAt <= now) return false;
    insertReminder.run({
      taskId,
      remindAt,
      origin: 'auto',
      occurrenceKey: occurrenceKey(taskId, reminderVersion, remindAt),
      snoozedFromId: null,
  ...
  // 반복 회차는 목록에서 시리즈당 가장 이른 미완료 회차 하나로 접는다. 접지 않으면
  // 매일 반복 하나가 60행으로 목록을 통째로 먹는다. 가장 이른 것을 남기므로
  // 놓친 회차가 있으면 그것이 보인다.
  function collapseSeries(tasks) {
    const earliest = new Map();
    const total = new Map();
    for (const task of tasks) {
      if (task.seriesId === null) continue;
      total.set(task.seriesId, (total.get(task.seriesId) || 0) + 1);
      const current = earliest.get(task.seriesId);
      if (!current || compareDueTasks(task, current) < 0) earliest.set(task.seriesId, task);
    }
    const visible = new Map([...earliest.values()].map(task => [task.id, task.seriesId]));
    return {
      tasks: tasks.filter(task => task.seriesId === null || visible.has(task.id)),
      seriesFor(task) {
        if (task.seriesId === null) return null;
        const rule = getSeriesRule.get(task.seriesId);
        return rule ? { ...rule, remaining: (total.get(task.seriesId) || 1) - 1 } : null;
      },
    };
  }

  function activeTaskView() {
    const collapsed = collapseSeries(getTasksByLifecycle.all('active'));
    return {
      tasks: collapsed.tasks,
      decorate: task => ({ ...withLiveReminder(task), series: collapsed.seriesFor(task) }),
    };
  }
  ...
  function writeCurrentTask(current, next, eventType, now, actorType = 'user') {
    const values = {
      ...current,
      ...next,
      id: current.id,
  ...
  const createTransaction = db.transaction((normalized, now, series) => {
    ...
    const inserted = insertTask.run({
      ...normalized,
      seriesId: series?.id ?? null,
      occurrenceDate: series?.occurrenceDate ?? null,
      now,
    });
    ...
    if (normalized.reminderAt !== null) {
      ...
      insertAutoReminder(taskId, normalized, 1, now);
    }
  ...
  const updateTransaction = db.transaction((id, patch, now) => {
    ...
    const dueChanged = (
      next.dueKind !== current.dueKind || next.dueDate !== current.dueDate
      || next.dueAt !== current.dueAt
    );
    // 사용자가 정한 알림은 절대 시각이라 기한이 바뀌어도 건드리지 않는다. 기본 알림은
    // 기한에서 계산한 값이라 기한을 따라 옮겨야 한다.
    const autoNeedsMove = live?.origin === 'auto' && dueChanged;
    // `remove`는 사용자가 정한 시각을 지우는 것이다. 기본 알림에는 지울 것이 없어서
    // 아무 일도 하지 않는다. 그러지 않으면 취소하고 곧바로 다시 만들기를 반복해
    // 같은 요청이 매번 새 버전을 만든다.
    const removesUserReminder = reminderAction === 'remove' && live?.origin === 'user';
    const cancelReason = (
      reminderAction === 'replace' && live ? 'replaced'
        : removesUserReminder ? 'removed'
          : autoNeedsMove ? 'replaced' : null
    );
    const cancelsLive = live !== null && cancelReason !== null;
    // 살아있는 알림이 없어지거나 원래 없던 자리에 기한이 생기면 기본 알림을 다시 잡는다.
    const wantsAuto = (
      reminderAction !== 'replace'
      && (live === null || cancelsLive)
      && autoRemindAt(next) !== null
      && autoRemindAt(next) > now
    );
    const reminderChanged = reminderAction === 'replace' || cancelsLive || wantsAuto;
    ...
    if (cancelsLive) {
      cancelLiveReminders.run({ taskId: id, reason: cancelReason, now });
      onReminderResolved(live.id, now);
    }
    // 반복 회차를 직접 고치면 사용자가 손댄 회차로 표시한다. 시리즈 규칙이
    // 나중에 바뀌어도 이 회차는 재생성이 비껴간다.
    const overridden = current.seriesId === null ? current.overridden : 1;
    const updated = writeCurrentTask(current, { ...next, overridden, reminderVersion }, 'updated', now);
    if (reminderAction === 'replace') {
      ...
      insertAutoReminder(id, next, reminderVersion, now);
    }
    return currentResult(updated, { unchanged: false });
  });

  const transitionTransaction = db.transaction((id, action, expectedVersion, now, actorType) => {
    const current = requireTask(id);
    const alreadyTarget = (
    ...
    if (action === 'complete' || action === 'cancel') {
      ...
      // 종결될 때 알림이 모두 정리됐으므로 다시 열면 기본 알림도 다시 잡는다.
      // 그러지 않으면 다시 연 일정만 조용히 알림 없는 상태로 남는다.
      next = {
        status: 'active', lifecycle: 'active', completedAt: null,
        cancelledAt: null, closedAt: null,
        reminderVersion: current.reminderVersion + 1,
      };
      eventType = 'reopened';
      reopenedToActive = true;
    ...
    const updated = writeCurrentTask(current, next, eventType, now, actorType);
    if (reopenedToActive) {
      insertAutoReminder(updated.id, updated, updated.reminderVersion, now);
    }
    return currentResult(updated, { unchanged: false });
  });
  ...
  const snoozeTransaction = db.transaction((id, requestKey, now) => {
    ...
    const inserted = insertReminder.run({
      ...
      remindAt,
      // 미루기는 사용자가 그 알림을 보고 누른 것이라 기본 알림에서 미뤄도 사용자 것이 된다.
      origin: 'user',
  ...
  return {
    prepare(input, options = {}) {
      const rawCapturedAt = options.capturedAt === undefined ? captureNow() : Number(options.capturedAt);
      if (!Number.isFinite(rawCapturedAt)) {
        taskError('후보 기준 시각이 올바르지 않습니다.', 'INVALID_CAPTURED_AT');
      }
      const capturedAt = Math.floor(rawCapturedAt);
      return {
        capturedAt,
        timezone: KST_TIMEZONE,
        task: preparedCreatePayload(normalizeCreateInput(input, capturedAt)),
      };
    },

    // `series`는 반복 시리즈의 회차를 만들 때만 materializer가 넘긴다. 사용자 요청
    // 본문으로는 올 수 없는 자리라 HTTP route는 이 인자를 넘기지 않는다.
    create(input, options = {}) {
      ...
      const existing = input && typeof input === 'object' && !Array.isArray(input) &&
        ...
        : null;
      return createTransaction(normalizeCreateInput(input, now, !existing), now, options.series ?? null);
    },
    ...
    // `actorType`은 놓친 반복 회차를 서버가 정리할 때만 'system'이 된다. HTTP
    // route는 이 인자를 넘기지 않아 사용자 요청은 언제나 'user'로 남는다.
    transition(idValue, action, input, options = {}) {
      const id = validateId(idValue, '일정');
      const expectedVersion = validateExpectedVersion(input?.expectedVersion);
      const actorType = options.actorType === 'system' ? 'system' : 'user';
      return transitionTransaction(id, action, expectedVersion, captureNow(), actorType);
    },
    ...
    list(options = {}) {
      ...
      const { tasks, decorate } = activeTaskView();
      ...
      if (view === 'today') {
        const overdue = bucketed.filter(item => item.bucket === 'overdue').map(item => item.task)
          .sort(compareDueTasks).slice(0, limit).map(decorate);
    ...
    summary(options = {}) {
      ...
      const today = kstDate(now);
      // 달력의 날짜별 건수는 접지 않는다. 그날 몇 건인지가 달력이 답해야 하는
      // 질문이라 반복 회차도 하루에 하나씩 그대로 세어야 한다.
      const allTasks = getTasksByLifecycle.all('active');
      const { tasks, decorate } = activeTaskView();
      const bucketed = tasks.map(task => ({ task, bucket: bucketForTask(task, now, today, tomorrowEpoch) }));
...
module.exports = {
  ...
  // 반복 시리즈 store가 같은 검증기와 KST 변환을 쓴다. 두 벌을 두면 같은 입력을
  // 두 곳이 다르게 판정하고, 그 차이는 회차가 하루 어긋난 뒤에야 드러난다.
  internals: {
    taskError,
    normalizeText,
    validateRequestKey,
