...
function scheduleSystemPrompt(capturedAt) {
...
function createSchedulePrepareSession(store, { capturedAt, clientRequestId } = {}) {
  ...
  return {
    systemPrompt: scheduleSystemPrompt(Number(capturedAt)),

    getToolDefinitions() {
      return candidate ? [] : [SCHEDULE_PREPARE_TOOL];
    },
