...
function scheduleSystemPrompt(capturedAt, { persistImmediately = false } = {}) {
...
function createSchedulePrepareSession(store, {
  ...
  return {
    systemPrompt: scheduleSystemPrompt(Number(capturedAt), { persistImmediately }),

    getToolDefinitions() {
      return candidate ? [] : [toolDefinition];
    },
