...
function createAttachmentUploadService(db, {
  ...
  function cleanupOrphans() {
    ...
    const retiredTemporaryBlobs = db.prepare(`
      SELECT b.stored_path AS storedPath
      FROM attachment_blobs b
      WHERE b.storage_scope = 'temporary'
        AND b.status = 'deleted'
        AND NOT EXISTS (
          SELECT 1 FROM attachments a WHERE a.blob_id = b.id
        )
    `).all();
    for (const blob of retiredTemporaryBlobs) {
      if (!isInsideDirectory(resolvedTmpDir, blob.storedPath)) continue;
      try { fs.unlinkSync(blob.storedPath); } catch (error) {
        if (error.code !== 'ENOENT') throw error;
      }
    }
