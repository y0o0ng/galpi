...
async function auditNoteIndex({ db, vaultPath }) {
  ...
  const required = ['content_sha256', 'indexed_sha256', 'index_status'];
  const missingColumns = required.filter(column => !columns.has(column));
  if (missingColumns.length > 0) {
    throw new Error(`note index audit에는 schema v4가 필요합니다: ${missingColumns.join(', ')}`);
  }
  ...
  for (const row of rows) {
    ...
    const active = !Boolean(row.archived);
