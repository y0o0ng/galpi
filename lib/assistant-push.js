...
/**
 * provider가 준 거절 사유. web-push의 WebPushError는 본문(`body`)에 왜 거절했는지를
 * 담아 오는데, 지금까지 그것을 버려서 400이 나도 로그에 아무 흔적이 없었다.
 *
 * **본문에는 endpoint도 payload도 들어 있지 않다** — 거절 사유 문자열뿐이다. 그래도
 * 길이를 잘라 두어, provider가 언젠가 더 많은 것을 담아도 로그로 새지 않게 한다.
 */
/**
 * Web Push의 `Topic` 헤더로 쓸 값. **RFC 8030의 Topic은 URL-safe base64다.**
 *
 * base64 문자열의 길이는 4로 나눈 나머지가 1일 수 없는데(2·3·4글자가 각각 1·2·3바이트),
 * `mail-batch-${id}` 같은 뜻 있는 문자열은 id 자릿수가 늘어나는 순간 그 금지 길이에
 * 들어간다. 실제로 Apple이 `mail-batch-10`(13자)을 400 BadWebPushTopic으로 거절해
 * 메일 알림이 조용히 사라졌다. `mail-batch-9`(12자)까지는 우연히 통과했다.
 *
 * topic은 사람이 읽는 값이 아니라 **같은 대상의 이전 알림을 합치기 위한 라우팅 키**라
 * 뜻을 잃어도 된다. 해시로 바꾸면 같은 대상은 늘 같은 값이 되어 합치기는 그대로이고,
 * 길이가 22로 고정돼(22 % 4 === 2) 어떤 id에서도 규칙을 깨지 않는다.
 *
 * 도메인마다 길이를 맞추게 하지 않고 dispatcher 한 곳에서 처리한다 — 같은 규칙이
 * 세 곳에 흩어지면 새 도메인이 생길 때 또 틀린다.
 */
function normalizePushTopic(value) {
...
function createAssistantPushDispatcher(service, options = {}) {
  ...
  // 전달 루프(claim·retry·410 만료·lease·overlap guard)는 도메인과 무관하다. 도메인이
  // 정하는 것은 payload와 send option 둘뿐이라 그 둘만 주입받는다. 범용 notification
  // framework를 만들지 않고, hook을 주지 않은 호출자는 기존 일정 동작 그대로다.
  //
  // ttl은 hook 밖에 둔다. expiresAt에서 계산하는 값이라 도메인이 정할 것이 아니고,
  // hook이 덮으면 만료 계약이 조용히 깨진다.
  ...
  const buildSendOptions = typeof options.buildSendOptions === 'function'
    ? options.buildSendOptions
    : claim => ({ urgency: 'high', topic: `task-${claim.reminderId}`.slice(0, 32) });
  let timer = null;
  let activeTick = null;
  ...
  async function deliver(claim) {
    ...
    try {
      const sendOptions = buildSendOptions(claim);
      const topic = normalizePushTopic(sendOptions?.topic);
      const result = await transport.send({
        endpoint: claim.endpoint,
        keys: { p256dh: claim.p256dh, auth: claim.auth },
      }, payload, { ttl, ...sendOptions, ...(topic ? { topic } : {}) });
      ...
      // 재시도하지 않고 끝내는 실패다. 사유를 남기지 않으면 왜 잃었는지 영영 모른다.
      onError(error, { claim, httpStatus: status, reason: providerReason(error) });
