...
function validateCalibrationMetadata(pilotCase, metadata) {
  ...
  const expectedPrimarySource = (
    ...
    || metadata.capabilityProbe
  ) ? 'PROGRAMMATIC' : 'HUMAN';
  if (
    pilotCase.adjudication.state !== 'PRIMARY_ADJUDICATED'
    || pilotCase.adjudication.primary?.source !== expectedPrimarySource
  ) {
    throw new TypeError(`P1-B1 primary gold source는 ${expectedPrimarySource}이어야 합니다: ${pilotCase.caseId}`);
  }
