...
// 마크다운 링크와 맨 URL. 화면에서는 눌러서 원문으로 가는 길이지만 소리로는
// "에이치티티피에스 콜론 슬래시 슬래시 더블유…"가 된다.
const MARKDOWN_LINK_RE = /\[([^\]\n]*)\]\(\s*(?:https?:\/\/|\/)[^)\s]*\s*\)/g;
const BARE_URL_RE = /https?:\/\/\S+/g;
// 스트리밍 중에는 버퍼 끝이 아직 오는 중일 수 있다. 뒤에 공백이 온 URL만 끝난 것으로 본다.
const BARE_URL_TERMINATED_RE = /https?:\/\/\S+(?=\s)/g;

// 스킴 없는 주소(`axios.com`, `www.cnet.com/tech/x`). 모델이 "출처는 axios.com이야"라고
// 쓰면 TTS가 "액시오스 닷 컴"이라고 읽는다.
//
// **TLD를 열거한다.** "점 뒤 2~6글자 영문"으로 잡으면 `설정.json`·`파일.zip`처럼 주소가
// 아닌 것까지 지워 답을 훼손한다. 목록에 없는 TLD는 그냥 읽힐 뿐이라, 틀려도 지금보다
// 나빠지지 않는 방향으로 실패한다.
const HOST_TLDS = 'com|net|org|io|ai|dev|app|news|me|kr|co\\.kr|or\\.kr|ne\\.kr|go\\.kr|ac\\.kr|edu|gov|info|xyz';
const BARE_HOST_PATTERN = `\\b(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.)+(?:${HOST_TLDS})\\b(?:\\/\\S*)?`;
const BARE_HOST_RE = new RegExp(BARE_HOST_PATTERN, 'gi');
// 스트리밍 중에는 `axios.co` + `m`처럼 주소가 쪼개져 도착할 수 있다. 뒤에 공백이나
// 문장부호가 온 것만 끝난 주소로 보고 지운다. 나머지는 end()가 훑는다.
const BARE_HOST_TERMINATED_RE = new RegExp(`${BARE_HOST_PATTERN}(?=[\\s,.!?)\\]])`, 'gi');

/**
 * 소리로 나갈 텍스트에서 링크를 벗긴다. **화면 텍스트는 건드리지 않는다** —
 * 음성 턴의 답변도 화면에 그대로 저장되므로, 모델에게 링크를 쓰지 말라고 시키면
 * 사용자가 원문으로 들어갈 길이 함께 사라진다. 표시 글자는 남기고 주소만 지운다.
 *
 * URL 안의 점(`axios.com`)은 문장 끝으로도 오인돼 조각을 잘게 쪼갠다. 조각을
 * 나누기 전에 벗기면 그것도 함께 사라진다.
 *
 * @param {boolean} final 텍스트가 더 오지 않는가. 스트리밍 중이면 false.
 */
function stripSpokenMarkup(text, { final = true } = {}) {
  const source = String(text ?? '');
  if (!source) return '';
  return source
    .replace(MARKDOWN_LINK_RE, (match, label) => String(label).trim())
    .replace(final ? BARE_URL_RE : BARE_URL_TERMINATED_RE, '')
    .replace(final ? BARE_HOST_RE : BARE_HOST_TERMINATED_RE, '')
    // 링크를 들어낸 자리에 남는 빈 괄호·꺾쇠와 겹친 공백을 정리한다.
    .replace(/\(\s*[:：]?\s*\)/g, '')
    .replace(/<\s*>|(?:^|\s)[<>](?=\s|$)/g, ' ')
    .replace(/[ \t]{2,}/g, ' ')
    .replace(/\s+([,.!?])/g, '$1');
...
function selectSpokenText(fullText, { maxChars = DEFAULT_MAX_CHARS, maxSentences = 3 } = {}) {
  const text = stripSpokenMarkup(fullText).replace(/\s+/g, ' ').trim();
  if (!text) return '';
...
function createSpokenSegmenter({
  ...
  return {
    push(text) {
      ...
      buffer += String(text || '');
      // 조각을 나누기 **전에** 벗긴다. 그래야 URL 안의 점이 문장 끝으로 오인돼
      // 조각을 쪼개는 일도 없다. 아직 다 오지 않은 링크는 final=false가 남겨두고,
      // 완성되면 다음 push에서 벗겨진다.
      buffer = stripSpokenMarkup(buffer, { final: false });
    ...
    // 화면을 가리키며 닫고, 그냥 끝난 것이라면 꼬리를 마저 읽는다.
      ...
      // 더 올 것이 없으므로 끝에 걸쳐 있던 링크까지 벗긴다.
      const tail = stripSpokenMarkup(buffer).trim();
      buffer = '';
