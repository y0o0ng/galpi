...
function createSpokenSegmenter({
  ...
  // 상한에 닿으면 더 읽지 않고 화면을 가리키며 닫는다. selectSpokenText와 같은 문구다.
  function closeWithPointer(out) {
    closed = true;
    out.push('자세한 건 화면에 정리해뒀어.');
    return out;
  }
  ...
  return {
        ...
        if (SEGMENT_BREAK_CHARS.includes(char)) {
          ...
          if (!isDecimal && !isEllipsis) {
            ...
            if (candidate.length >= minSegmentChars) {
              ...
              index = 0;
              if (emittedSegments + 1 >= maxSegments || emittedChars + candidate.length >= maxChars) {
                out.push(take(candidate));
                return closeWithPointer(out);
              }
              out.push(take(candidate));
              continue;
            }
    ...
    // 스트림이 끝나면 남은 꼬리를 내보낸다. 짧아도 여기서는 버리지 않는다.
    end() {
      if (closed) return [];
      closed = true;
      ...
      return [take(tail.slice(0, Math.max(0, maxChars - emittedChars))), '자세한 건 화면에 정리해뒀어.'];
    },
  };
}
