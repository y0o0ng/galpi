...
function transcriptionError(message, code, status) {
  return new VoiceTranscriptionError(message, { code, status });
}
...
// 되묻기 문턱은 실제 오전사 표본이 쌓인 뒤에 정한다. 그때까지는 숫자만 모은다.
// 토큰 문자열은 전사 내용이므로 담지 않는다.
const LOW_CONFIDENCE_LOGPROB = -0.5;
// 자주 틀리는 고유명사만 좁게 적는다. 문장을 넣으면 전사가 그쪽으로 끌려간다.
const DEFAULT_TRANSCRIPTION_PROMPT = '시온, 시온아, 갈피';

function summarizeLogprobs(logprobs) {
  ...
  return {
    ...
    low: values.filter(value => value < LOW_CONFIDENCE_LOGPROB).length,
  };
}
...
function readVoiceTranscriptionUpload(req, {
...
function createOpenAITranscriptionProvider({
  ...
  return async function transcribeAudio({ audio, mimeType, durationMs }) {
    ...
    // 되묻기 문턱을 정할 근거를 모으기 위한 관측용이다. 동작은 바꾸지 않는다.
    form.append('include[]', 'logprobs');
    // 고유명사 힌트. 없으면 "시온아"가 "시오나"·"지온아"·"시훈아"로 흩어진다.
    if (resolvedPrompt) form.append('prompt', resolvedPrompt);
    ...
    if (!response.ok) {
      throw new VoiceTranscriptionError('보정 전사를 완료하지 못했습니다.', {
    ...
    return {
      ...
      confidence: summarizeLogprobs(body?.logprobs),
    };
  };
}
...
function createVoiceTranscriptionService({
  ...
  async function transcribe({
    ...
    const run = session.tail
      ...
      .catch(error => {
        // provider가 던진 빈 전사에도 같은 진단을 실어 서로 다른 턴의 오디오 동일성을 대사한다.
        if (error?.code === 'REALTIME_TRANSCRIPTION_EMPTY') {
          error.emptyAudioSha256 = audioSha256;
        }
        throw error;
      })
      .then(result => {
