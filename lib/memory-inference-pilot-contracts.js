...
const RESULT_CONTRACT_VERSION = 'xion-local-memory-inference-result-v2';
...
function validatePilotResult(value) {
  ...
  assertBoundedString(value.configuration.runnerVersion, 'configuration.runnerVersion');
  assertBoundedString(value.configuration.runtimeFamily, 'configuration.runtimeFamily');
  assertBoundedString(value.configuration.runtimeVersion, 'configuration.runtimeVersion');
  assertNullableBoundedString(value.configuration.modelId, 'configuration.modelId', { max: 240 });
  assertNullableBoundedString(
    value.configuration.artifactId,
    'configuration.artifactId',
    { max: 240 },
  );
  assertNullableBoundedString(value.configuration.quantization, 'configuration.quantization');
  assertBoundedString(value.configuration.promptVersion, 'configuration.promptVersion');
  assertBoundedString(
    value.configuration.taskContractVersion,
    'configuration.taskContractVersion',
  );
  assertBoundedString(
    value.configuration.taskSpecificationVersion,
    'configuration.taskSpecificationVersion',
  );
  assertBoundedString(
    value.configuration.outputSchemaVersion,
    'configuration.outputSchemaVersion',
  );
  assertBoundedString(value.configuration.commit, 'configuration.commit', { max: 64 });
  if (
    [POLICY_TYPES.LOCAL_ONLY, POLICY_TYPES.LOCAL_FIRST].includes(value.policyType)
    && (
      value.configuration.modelId === null
      || value.configuration.artifactId === null
      || value.configuration.quantization === null
    )
  ) {
    throw new TypeError('local runner에는 modelId, artifactId, quantization provenance가 필요합니다.');
  }
