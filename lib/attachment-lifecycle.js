...
// 문서 파싱은 채팅 요청 안에서 끝나야 해서 장수만큼 대기가 곱해진다. 여러 장을
// 붙이는 쪽은 이미지이므로 문서는 메시지당 하나로 묶어둔다.
const MAX_DOCUMENTS_PER_MESSAGE = 1;
const MAX_IMAGE_BYTES_PER_MESSAGE = 12 * 1024 * 1024;
const DOCUMENT_KINDS = new Set(['pdf', 'text', 'markdown']);
...
function normalizeAttachmentIds(value) {
  ...
  if (value.length > MAX_ATTACHMENTS_PER_MESSAGE) {
    throw lifecycleError(
      `한 메시지에는 첨부파일 ${MAX_ATTACHMENTS_PER_MESSAGE}개까지 연결할 수 있습니다.`,
      'ATTACHMENT_LIMIT',
    );
  }
...
function createAttachmentLifecycleService(db, {
  ...
  // 이미지는 도구로 필요할 때만 읽는 문서와 달리 매 턴 원본이 통째로 다시 실려
  // 나가므로 더 짧은 창을 쓴다. 값은 행 단위 컬럼이라 만료 계산은 그대로다.
  ...
  for (const turns of [replayWindowTurns, imageReplayWindowTurns]) {
    if (!Number.isInteger(turns) || turns < 1 || turns > 1000) {
      throw new TypeError('첨부 replay 사용자 턴 수가 올바르지 않습니다.');
    }
  }
  if (prepareAttachment !== null && typeof prepareAttachment !== 'function') {
    throw new TypeError('첨부 준비 함수가 올바르지 않습니다.');
  }

  const resolvedTmpDir = path.resolve(tmpDir);
  const activeAttachmentIds = new Set();
  const getAttachment = db.prepare(`
    SELECT a.id, a.session_id AS sessionId, a.scope, a.lifecycle_status AS lifecycleStatus,
           a.original_name AS filename, a.kind,
           b.id AS blobId, b.stored_name AS storedName, b.stored_path AS storedPath, b.mime_type AS mimeType,
  ...
  `);
  const deleteAttachmentChunks = db.prepare(`
    DELETE FROM attachment_chunks WHERE attachment_id = ?
  `);
  const deleteAttachmentDocument = db.prepare(`
  ...
  function assertMessageComposition(rows) {
    const documents = rows.filter(row => DOCUMENT_KINDS.has(row.kind));
    if (documents.length > maxDocumentsPerMessage) {
      throw lifecycleError(
        `한 메시지에는 문서 첨부를 ${maxDocumentsPerMessage}개까지 붙일 수 있습니다.`,
        'ATTACHMENT_DOCUMENT_LIMIT',
      );
    }
    const imageBytes = rows
      .filter(row => row.kind === 'image')
      .reduce((sum, row) => sum + Number(row.sizeBytes || 0), 0);
    if (imageBytes > maxImageBytesPerMessage) {
      throw lifecycleError(
        `한 메시지의 이미지 합계는 ${Math.floor(maxImageBytesPerMessage / (1024 * 1024))}MB까지입니다.`,
        'ATTACHMENT_IMAGE_BYTES_LIMIT',
        413,
      );
    }
  }
  ...
  async function beginChatRequest({ sessionId, attachmentIds }) {
    ...
    if (ids.length === 0) {
      return { attachmentIds: ids, attachments: [], release() {} };
    }
    if (!enabled) {
    ...
    assertMessageComposition(rows);

    // lease는 전부 잡히거나 하나도 안 잡힌다. 중간에 실패하면 이번 요청이 잡은
    // 것만 되돌리고, 다른 요청이 이미 들고 있던 것은 건드리지 않는다.
    const leased = [];
    try {
      ...
      for (const row of rows) {
        ...
        if (prepareAttachment) await prepareAttachment(row.id);
      }
    } catch (error) {
      for (const id of leased) activeAttachmentIds.delete(id);
      throw error;
    }
    ...
    return {
      attachmentIds: ids,
      attachments: rows.map(row => ({ id: row.id, filename: row.filename, kind: row.kind })),
      release() {
        if (released) return;
        released = true;
        for (const id of leased) activeAttachmentIds.delete(id);
  ...
  async function beginLibraryPromotion({ sessionId, attachmentId }) {
    const ids = normalizeAttachmentIds([attachmentId]);
    if (!enabled) {
      throw lifecycleError('첨부파일 기능이 비활성화되어 있습니다.', 'ATTACHMENT_DISABLED', 503);
    }
    const row = getAttachment.get(ids[0]);
    if (!row) throw lifecycleError('첨부파일을 찾을 수 없습니다.', 'ATTACHMENT_NOT_FOUND', 404);
    if (row.sessionId !== sessionId) {
      throw lifecycleError('다른 대화의 첨부파일입니다.', 'ATTACHMENT_SESSION_MISMATCH', 409);
    }
    if (row.scope === 'library' && row.lifecycleStatus === 'library') {
      return { attachment: row, alreadyLibrary: true, release() {} };
    }
    if (row.scope !== 'temporary' || row.lifecycleStatus !== 'attached_temporary') {
      throw lifecycleError('현재 대화에 연결된 임시 첨부만 저장할 수 있습니다.', 'ATTACHMENT_NOT_AVAILABLE', 409);
    }
    if (row.blobStatus !== 'ready' || !isInsideDirectory(resolvedTmpDir, row.storedPath)) {
      throw lifecycleError('첨부 원본을 사용할 수 없습니다.', 'ATTACHMENT_STORAGE_INVALID', 409);
    }
    if (activeAttachmentIds.has(row.id)) {
      throw lifecycleError('첨부파일이 다른 요청에서 사용 중입니다.', 'ATTACHMENT_BUSY', 409);
    }
  ...
  function cleanupExpired(attachmentIds) {
    ...
    for (const attachmentId of attachmentIds) {
      ...
      db.transaction(() => {
        deleteAttachmentChunks.run(row.id);
        deleteAttachmentDocument.run(row.id);
        if (!hasOtherReferences) {
  ...
  function attachToUserMessage({ sessionId, userMessageId, attachmentIds }) {
    ...
    return ids.map((id, position) => {
      ...
      insertMessageAttachment.run(
        ...
        position,
        originUserTurnIndex,
        row.kind === 'image' ? imageReplayWindowTurns : replayWindowTurns,
        timestamp,
      );
      return {
        attachmentId: id,
  ...
  function listForSession(sessionId) {
    return groupHistoryAttachments(listSessionAttachments.all(sessionId));
  }

  function listForMessageIds(messageIds) {
    ...
    `).all(...messageIds);
    return groupHistoryAttachments(rows);
  }

  function groupHistoryAttachments(rows) {
