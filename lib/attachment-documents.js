...
function createAttachmentDocumentService(db, {
  ...
  const getCandidate = db.prepare(`
    SELECT a.id AS attachmentId, a.original_name AS filename, a.kind, a.scope,
    ...
    LIMIT 1
  `);
  const getLibraryCandidate = db.prepare(`
    SELECT a.id AS attachmentId, a.original_name AS filename, a.kind, a.scope,
           d.page_count AS pageCount, d.line_count AS lineCount,
           d.char_count AS charCount, d.chunk_count AS chunkCount
    ...
    JOIN attachment_documents d ON d.attachment_id = a.id
    WHERE li.note_filename = @noteFilename
      AND d.parse_status = 'ready'
      AND a.scope = 'library'
      AND a.lifecycle_status = 'library'
    LIMIT 1
  `);
  ...
  function listCandidates({
    ...
    for (const noteFilename of Array.isArray(libraryNoteFilenames) ? libraryNoteFilenames : []) {
      const normalizedFilename = path.basename(String(noteFilename || '').trim());
      if (!normalizedFilename || normalizedFilename !== noteFilename) continue;
      const row = getLibraryCandidate.get({ noteFilename: normalizedFilename });
      if (!row || seen.has(row.attachmentId)) continue;
      candidates.push(row);
  ...
  function hasTemporaryCandidates({ sessionId, attachmentIds = [] } = {}) {
    if (!enabled) return false;
    const normalizedSessionId = String(sessionId || '').trim();
    if (!normalizedSessionId) return false;
    for (const attachmentId of Array.isArray(attachmentIds) ? attachmentIds : []) {
      const normalizedId = String(attachmentId || '').trim();
      if (!ATTACHMENT_ID_PATTERN.test(normalizedId)) continue;
      if (getCandidate.get({ attachmentId: normalizedId, sessionId: normalizedSessionId })) return true;
    }
    return listReplayCandidates.all(normalizedSessionId, normalizedSessionId).length > 0;
  }
