...
function createMailPushService(db, options = {}) {
   ...
   *
   * 여기가 집행하는 것은 알림 방향을 바꾸는 두 값, 억제와 승격이다. 분석을 아예
   * 건너뛰는 값은 LLM 앞에서 갈리므로 이 파일이 아니라 `analyze.js`의 일이다.
   */
  function effectiveMode(message) {
    ...
    // 억제가 승격을 이긴다. 사용자가 조용히 시킨 것을 다른 규칙이 되살리면
    // "껐는데 울린다"가 되고, 그 순간 알림 전체를 못 믿게 된다.
    if (matched.some(item => item?.action === 'suppress_notification')) return 'silent';
    if (!matched.some(item => item?.action === 'always_notify')) return message.mode;
    // 승격은 한 칸이다(설계 11.1). `silent`를 batch로 올릴 뿐이고, 즉시 알림은
    // 모델이 스스로 급하다고 본 메일에만 준다. 도메인 하나를 통째로 즉시 알림으로
    // 만들면 학교 공지 광고까지 폰을 울린다.
    if (message.mode === 'silent') {
      return message.category === 'urgent' || message.category === 'action_required'
        ? 'immediate'
        : 'batch';
    }
    if (message.mode === 'batch'
      && (message.category === 'urgent' || message.category === 'action_required')) {
      return 'immediate';
    }
    return message.mode;
  }
