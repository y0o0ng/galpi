...
/**
 * fetch 자체가 거절되는 경우를 agent가 아는 계약으로 바꾼다.
 *
 * 이 경계가 없으면 랜선이 잠깐 빠진 것만으로 code도 retryable도 없는 TypeError가
 * 그대로 올라가고, agent가 그것을 치명적 실패로 보고 계정을 status='error'로 세운다.
 * listDueAccounts는 active만 집으므로 그 계정은 영구히 멈춘다(naver.js의 같은 경계).
 *
 * 여기서 거절되는 것은 전부 전송 계층 실패다. HTTP 상태로 드러나는 실패는 응답이
 * 돌아온 뒤에 따로 판정하고, URL은 이 파일의 상수라 잘못된 주소로 거절될 일이 없다.
 * 그래서 코드 목록으로 가르지 않는다 — 목록은 낡는 순간 같은 영구 정지가 다시 난다.
 */
async function fetchOrThrow(fetchImpl, url, init) {
  try {
    return await fetchImpl(url, init);
  } catch (error) {
    if (error?.code && String(error.code).startsWith('MAIL_')) throw error;
    const normalized = gmailError(
      'Gmail 연결이 일시적으로 실패했습니다.', 'MAIL_HTTP_FAILED', { retryable: true },
    );
    // 원인 코드만 남긴다. 주소·토큰·본문은 로그에 넣지 않는다(설계 19절).
    normalized.httpCode = error?.cause?.code || error?.code || error?.name || null;
...
function createGoogleTokenSource(options = {}) {
  ...
  async function requestToken() {
    ...
    const response = await fetchOrThrow(fetchImpl, TOKEN_ENDPOINT, {
      method: 'POST',
...
function createGmailProvider(options = {}) {
  ...
  // 404의 뜻은 부르는 쪽마다 다르다. history에서는 커서 만료지만 메시지 재조회에서는
  // 그 메일이 사라진 것이다. 둘을 같은 코드로 던지면 재분석 실패가 전체 resync를 부른다.
  async function call(path, { retryOn401 = true, notFoundCode = 'MAIL_HISTORY_GONE' } = {}) {
    const token = await tokenSource.getAccessToken();
    const response = await fetchOrThrow(fetchImpl, `${API_BASE}${path}`, {
      headers: { authorization: `Bearer ${token}` },
    });
    if (response.status === 401 && retryOn401) {
      // 토큰이 우리 시계보다 먼저 죽은 경우다. 한 번만 강제로 갈고 다시 시도한다.
      await tokenSource.getAccessToken({ force: true });
      return call(path, { retryOn401: false, notFoundCode });
    }
    if (response.status === 404) {
      throw notFoundCode === 'MAIL_MESSAGE_GONE'
        ? gmailError('Gmail 메시지를 찾을 수 없습니다.', 'MAIL_MESSAGE_GONE', { status: 404 })
        : gmailError('Gmail history 커서가 만료됐습니다.', 'MAIL_HISTORY_GONE', { status: 404 });
    }
    if (!response.ok) {
      throw gmailError(
  ...
  async function baselineSync(reason) {
    // 커서를 잡기 직전 시각을 함께 들고 나간다. 이 시각 뒤에 도착한 메일은 최근
    // 목록에 섞여 들어와도 baseline이 아니다 — baseline으로 묻으면 커서가 H0라서
    // 다음 history에서 다시 만나는데, 그때는 identity dedup이 판단 상태를 건드리지
    // 않으므로 그 메일이 영원히 skipped로 남는다.
    const cursorCapturedAt = clock();
  ...
  return {
    ...
    /**
     * 분석에 쓸 원문을 다시 읽는다. 본문은 DB에 없으므로 재분석은 항상 여기를 거친다(설계 9.2).
     *
     * 라벨을 여기서 다시 본다. 저장된 뒤 사용자가 휴지통·스팸으로 옮겼을 수 있고, 그때는
     * 우리가 볼 메일이 아니다. 없어진 메일은 재시도로 풀리지 않으므로 retryable이 아니다.
     */
    async fetchRaw(gmailMessageId) {
      const id = String(gmailMessageId || '').trim();
      if (!id) throw gmailError('Gmail 메시지 ID가 필요합니다.', 'MAIL_MESSAGE_GONE');
      const message = await call(
        `/messages/${encodeURIComponent(id)}?format=raw`,
        { notFoundCode: 'MAIL_MESSAGE_GONE' },
      );
      if (!isCollectableMessage(message?.labelIds)) {
        throw gmailError('수집 대상이 아닌 라벨입니다.', 'MAIL_MESSAGE_EXCLUDED');
      }
      if (typeof message?.raw !== 'string' || !message.raw) {
        throw gmailError('Gmail 원문이 비어 있습니다.', 'MAIL_MESSAGE_GONE');
      }
      // 라벨은 분석 hint로 쓴다. IMPORTANT·CATEGORY_*는 구글이 이미 낸 판단이라
      // 우리가 다시 계산할 이유가 없다(설계 9.3 providerLabels).
      return { raw: Buffer.from(message.raw, 'base64url'), labels: message.labelIds || [] };
    },
