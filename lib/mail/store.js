...
function createMailStore(db, options = {}) {
  ...
  `);
  // 본문을 열 때 필요한 좌표(설계 10.2·23). 본문 컬럼은 없으므로 화면이 본문을
  // 요구하면 이 좌표로 Provider에서 다시 읽는다. 계정 주소까지 함께 주는 것은
  // 자격증명이 provider가 아니라 계정 단위로 풀리기 때문이다.
  const selectMessageLocator = db.prepare(`
    SELECT m.id, m.account_id AS accountId, a.provider AS provider,
           a.address AS accountAddress, a.status AS accountStatus,
           m.gmail_message_id AS gmailMessageId,
           m.imap_uid AS imapUid, m.imap_uid_validity AS imapUidValidity,
           m.subject, m.sender_name AS senderName, m.sender_address AS senderAddress,
           m.received_at AS receivedAt, m.has_attachments AS hasAttachments
    FROM mail_messages m
    JOIN mail_accounts a ON a.id = m.account_id
    WHERE m.id = ?
  `);
  ...
  return {
    ...
    findMessageLocator(id) {
      return selectMessageLocator.get(requireInteger(id, 'mailMessageId')) || null;
    },
