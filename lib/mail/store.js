...
function createMailStore(db, options = {}) {
  ...
  const insertMessage = db.prepare(`
    INSERT INTO mail_messages (
      ...
      @accountId, @provider, @identityKind, @identityKey,
    ...
    ON CONFLICT(account_id, identity_key) DO NOTHING
  `);
  const selectMessageByIdentity = db.prepare(`
    SELECT id, account_id AS accountId, identity_kind AS identityKind,
           identity_key AS identityKey, imap_uid AS imapUid,
           gmail_message_id AS gmailMessageId, is_baseline AS isBaseline,
           analysis_state AS analysisState, received_at AS receivedAt
    FROM mail_messages
    WHERE account_id = ? AND identity_key = ?
  `);
  ...
  return {
    ...
    // 같은 메일을 다시 만나면 조용히 무시한다. cursor replay가 안전한 이유가 이것이다.
    ...
    saveMessage(input, nowValue) {
      ...
      const row = selectMessageByIdentity.get(accountId, identityKey);
      return { message: row, inserted: result.changes === 1 };
    },
