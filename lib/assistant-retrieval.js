  ...
  const termDocumentFrequency = new Map(termGroups.map(group => [
    ...
    normalizedNotes.filter(note => (
      group.variants.some(variant => (
        note.titleLower.includes(variant) ||
        note.bodyLower.includes(variant) ||
        note.tagsLower.includes(variant)
      ))
    )).length,
  ]));
...
function rankChunkCandidates({
  ...
  const termDocumentFrequency = new Map(termGroups.map(group => [
    ...
    normalizedChunks.filter(chunk => (
      group.variants.some(variant => (
        chunk.titleLower.includes(variant) || chunk.contentLower.includes(variant)
      ))
    )).length,
  ]));
