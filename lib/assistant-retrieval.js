...
const GLOBAL_SHADOW_SEARCH_STOP_WORDS = new Set([
  ...
  '나', '나는', '저는', '우리', '우리는', '지금', '아냐', '아니',
  '맞아', '그럼', '보통', '원래', '솔직히', '너무', '안', '하고', '해서',
  '하면', '같은', '있고', '있는지', '언제', '혹시', '방법', '사려고',
  '이걸', '이걸로', '그걸', '그걸로', '거', '건', '거야', '거지',
  '찾아봐줘', '찾아봐줄', '찾아봐줄래', '봐줘', '봐줄래',
...
const LONG_TITLE_NEAR_MATCH_WEIGHT = 20;
const MULTI_EVIDENCE_QUERY_PATTERN = /(?:함께|같이|동시에|둘\s*다|각각|[\p{L}\p{N}]+(?:와|과)\s+[\p{L}\p{N}]+)/u;
...
const DEFAULT_SHADOW_RETRIEVAL_LIMITS = Object.freeze({
  ...
  globalSemanticOnlyMinScore: 0.80,
  globalLexicalMinEmbeddingScore: 0.30,
  globalMaxScoreGap: 0.11,
});
...
function extractQueryTermGroups(
  ...
  for (const term of rawTerms) {
    ...
    if (/^[a-z]$/i.test(term) || /^\d{1,2}$/.test(term)) continue;
...
function getNotePrior(notePriors, filename) {
  ...
  return {
    ...
    titleMatched: value?.titleMatched === true,
...
function rankNoteCandidates({
  ...
  for (const note of normalizedNotes) {
    ...
    for (const group of termGroups) {
      ...
      const titleTermFrequency = maxVariantScore(group.variants, variant => (
        Math.max(
          countOccurrences(note.titleLower, variant) * 5,
          hasNearLongTitleMatch(note.titleLower, [variant])
            ? LONG_TITLE_NEAR_MATCH_WEIGHT
            : 0,
        )
      ));
      if (titleTermFrequency > 0) titleMatchedTerms += 1;
      ...
      titleKeywordScore += titleTermFrequency * inverseDocumentFrequency;
...
function rankChunkCandidates({
  ...
  lexicalMinEmbeddingScore = null,
  strongNotePriorThreshold = null,
  strongNoteMinFinalScore = null,
  allowNoteKeywordAnchor = false,
  allowAutomaticStrongNotePrior = false,
  semanticOnlyUsesBaseScore = false,
  maxScoreGap = null,
  maxNotes = Infinity,
  includeParticleVariants = false,
  stopWords = DEFAULT_SEARCH_STOP_WORDS,
  ...
  for (const chunk of normalizedChunks) {
    let keywordScore = 0;
    let questionKeywordScore = 0;
    let questionMatchedTerms = 0;
    for (const group of termGroups) {
      const documentFrequency = termDocumentFrequency.get(group.term) || 1;
      const inverseDocumentFrequency = Math.log(
        (documentCount + 1) / (documentFrequency + 1)
      ) + 1;
      const questionOccurrences = maxVariantScore(group.variants, variant => (
      ...
      if (questionOccurrences > 0) questionMatchedTerms += 1;
      questionKeywordScore += questionOccurrences
        * Math.max(1, questionKeywordWeight)
    ...
    if (notePriorWeight > 0) {
      ...
      const minimumQuestionMatches = termGroups.length >= 3 ? 2 : 1;
      const hasLexicalAnchor = questionMatchedTerms >= minimumQuestionMatches
        || (allowNoteKeywordAnchor && prior.keywordMatched);
      const hasStrongNotePrior = Number.isFinite(strongNotePriorThreshold)
        && (
          allowAutomaticStrongNotePrior
          || prior.explicit
          || prior.titleMatched
        )
      ...
      if (
        ...
        (
          semanticOnlyUsesBaseScore
            ? baseScore < semanticOnlyMinScore
            : embeddingScore === null || embeddingScore < semanticOnlyMinScore
        )
      ) continue;
      if (
        hasLexicalAnchor &&
        embeddingScore !== null &&
        Number.isFinite(lexicalMinEmbeddingScore) &&
        embeddingScore < lexicalMinEmbeddingScore
      ) continue;
    ...
    scored.push({
      ...
      questionMatchedTerms,
      embeddingScore,
      notePrior: prior.score,
      noteExplicit: prior.explicit,
    });
  }
  ...
  for (const chunk of scored) {
    ...
    if (
      ...
      Number.isFinite(maxScoreGap) &&
      topScore - chunk.score > maxScoreGap
    ) continue;
...
function buildGlobalShadowRetrieval({
  ...
  const requestsMultipleEvidence = MULTI_EVIDENCE_QUERY_PATTERN.test(String(query || ''));
  const queryTermGroups = extractQueryTermGroups(
    query,
    GLOBAL_SHADOW_SEARCH_STOP_WORDS,
    { includeParticleVariants: true },
  );
  const queryTermCount = queryTermGroups.length;
  const hasSpecificTitleTerm = queryTermGroups.some(group => (
    group.variants.some(variant => compactSearchText(variant).length >= 5)
  ));
  ...
  for (const candidate of Array.isArray(noteCandidates) ? noteCandidates : []) {
    ...
    notePriors.set(candidate.filename, {
      ...
      titleMatched: existingPrior?.titleMatched === true || (
        queryTermCount > 0
        && hasSpecificTitleTerm
        && candidate.titleKeywordScore >= 15
        && candidate.titleMatchedTerms >= Math.min(2, queryTermCount)
      ),
      explicit: existingPrior?.explicit === true,
    });
  }
  ...
  const rankedChunks = rankChunkCandidates({
    ...
    chunks,
    limit: limits.maxChunks ?? DEFAULT_SHADOW_RETRIEVAL_LIMITS.maxChunks,
    maxChunksPerNote: requestsMultipleEvidence
    ...
    lexicalMinEmbeddingScore: limits.globalLexicalMinEmbeddingScore
      ?? DEFAULT_SHADOW_RETRIEVAL_LIMITS.globalLexicalMinEmbeddingScore,
    ...
    allowNoteKeywordAnchor: limits.globalAllowNoteKeywordAnchor === true,
    allowAutomaticStrongNotePrior:
      limits.globalAllowAutomaticStrongNotePrior === true,
    semanticOnlyUsesBaseScore: limits.globalSemanticOnlyUsesBaseScore === true,
    maxScoreGap: requestsMultipleEvidence
      ? Infinity
      : limits.globalMaxScoreGap
        ?? DEFAULT_SHADOW_RETRIEVAL_LIMITS.globalMaxScoreGap,
  });
  const context = buildChunkContext(rankedChunks, {
    maxChunks: limits.maxChunks ?? DEFAULT_SHADOW_RETRIEVAL_LIMITS.maxChunks,
    maxCharsPerChunk: limits.maxCharsPerChunk
