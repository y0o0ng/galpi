...
const migrations = [
  ...
  },
  {
    ...
    up(db) {
      ...
      db.exec(`
        ...
        -- 자격증명이 provider당 한 세트라 같은 provider의 두 번째 계정은 결국 첫 계정의
        -- 사서함을 읽는다. v1 범위는 Gmail 1 + Naver 1이므로 DB에서 막는다. 다계정을
        -- 열려면 이 인덱스를 지우는 마이그레이션과 account-scoped credential이 함께 필요하다.
        CREATE UNIQUE INDEX idx_mail_accounts_one_per_provider
  ...
  {
    version: 20,
    name: 'mail_push_message_target',
    up(db) {
      // 개별 Mail Push의 대상을 Attention에서 메일 자체로 옮긴다.
      //
      // 이유: `notification_mode`와 Attention은 서로 다른 축이다. 전자는 "알릴 가치가
      // 있는가"이고 후자는 "잊으면 안 되는 후속 행동이 있는가"다. 그런데 delivery가
      // Attention을 대상으로 잡고 있어서 한 축이 다른 축의 존재를 강제했다 —
      // `important/batch`인데 Attention이 없는 메일은 넣을 target_id가 없어 전달
      // 경로에 들어올 수조차 없었다. 그 조합은 예외가 아니라 정상이고 fixture
      // 기대값에도 둘(`subscription-renewal-notice`·`meeting-confirmed-datetime`),
      // 실제 메일에도 둘 있었다.
      //
      // SQLite는 CHECK를 ALTER로 바꿀 수 없어 표를 다시 만든다. 이 저장소의 첫 표
      // 재작성이다. 아래 전체가 runDatabaseMigrations의 db.transaction() 안에서
      // 돌므로 중간에 실패하면 반쪽짜리 스키마가 남지 않는다. `mail_push_deliveries`를
      // 참조하는 표가 없어서(자식 표다) foreign_keys를 끄지 않아도 안전하다.
      //
      // **무엇을 버리는가.** 버리는 것은 `mail_attention` 행이 아니라, 새 message
      // target으로 **해석할 수 없는 `mail_push_deliveries` 행**뿐이다. 기존
      // `attention` delivery는 `mail_attention.mail_message_id`를 통해 message
      // target으로 변환하고, 참조할 Attention이 없는 orphan delivery는 복사하지
      // 않는다. `mail_push_deliveries`는 기기별 전달 기록이며 메일·Attention의
      // 정본이 아니므로, 해석 불가능한 전달 기록 하나를 보존하려고 migration 전체를
      // 실패시키는 쪽이 더 나쁘다. `message`·`batch` delivery는 그대로 보존한다.
      db.exec(`
        CREATE TABLE mail_push_deliveries_new (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          -- attention → message. 대상은 이제 mail_messages.id다.
          target_kind TEXT NOT NULL CHECK (target_kind IN ('message', 'batch')),
          target_id INTEGER NOT NULL,
          notify_seq INTEGER NOT NULL DEFAULT 1 CHECK (notify_seq >= 1),
          subscription_id INTEGER NOT NULL,
          status TEXT NOT NULL DEFAULT 'pending'
        ...
        -- batch는 그대로. 대상 표가 바뀌지 않았다.
        INSERT INTO mail_push_deliveries_new (
          id, target_kind, target_id, notify_seq, subscription_id, status,
          attempt_count, next_attempt_at, expires_at, lease_until, last_attempt_at,
          last_http_status, last_error_code, accepted_at, created_at, updated_at
        )
        SELECT
          id, target_kind, target_id, notify_seq, subscription_id, status,
          attempt_count, next_attempt_at, expires_at, lease_until, last_attempt_at,
          last_http_status, last_error_code, accepted_at, created_at, updated_at
        FROM mail_push_deliveries
        WHERE target_kind = 'batch';

        -- attention은 INNER JOIN으로 해석 가능한 것만 옮긴다. join이 안 되는 행은
        -- 조용히 빠진다 — 그것이 위에서 말한 orphan delivery다.
        INSERT INTO mail_push_deliveries_new (
          id, target_kind, target_id, notify_seq, subscription_id, status,
          attempt_count, next_attempt_at, expires_at, lease_until, last_attempt_at,
          last_http_status, last_error_code, accepted_at, created_at, updated_at
        ...
        SELECT
          d.id, 'message', a.mail_message_id, d.notify_seq, d.subscription_id, d.status,
          d.attempt_count, d.next_attempt_at, d.expires_at, d.lease_until, d.last_attempt_at,
          d.last_http_status, d.last_error_code, d.accepted_at, d.created_at, d.updated_at
        FROM mail_push_deliveries d
        JOIN mail_attention a ON a.id = d.target_id
        WHERE d.target_kind = 'attention';

        DROP TABLE mail_push_deliveries;
        ALTER TABLE mail_push_deliveries_new RENAME TO mail_push_deliveries;
  ...
  {
    version: 21,
    name: 'mail_accounts_works_provider',
    // 아래 주석대로 pragma를 트랜잭션 밖에서 다뤄야 한다.
    ownsTransaction: true,
    up(db) {
      // 네이버 웍스 계정을 받기 위해 provider CHECK를 넓힌다. SQLite는 CHECK를
      // ALTER로 못 바꾸므로 표를 다시 만든다.
      //
      // **이번에는 부모 표다.** `mail_messages`·`mail_sync_state`·`mail_preferences`가
      // 이 표를 참조한다. v20에서 재작성한 delivery는 자식이 없는 잎이라 그냥 해도
      // 됐지만, 여기서는 DROP이 암묵적인 `DELETE FROM`을 돌려 자식이 전부 위반이 된다.
      //
      // `defer_foreign_keys`로는 못 넘긴다 — 그것은 위반 **개수**를 세는 방식이라
      // 새 표에 같은 id를 다시 넣어도 이미 센 위반이 지워지지 않는다(실측으로 확인).
      // 그래서 SQLite 문서의 절차를 그대로 쓴다: 트랜잭션 밖에서 FK를 끄고, 재작성을
      // 자체 트랜잭션으로 묶고, 커밋 전에 `foreign_key_check`로 직접 확인한다.
      //
      // 데이터는 하나도 버리지 않는다. 넓히는 변경이라 옮기지 못할 행이 없다.
      const current = db.prepare(`
        SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'mail_accounts'
      `).get()?.sql || '';
      // 자기 트랜잭션을 갖는 migration은 중간에 죽으면 다시 돈다. 이미 넓혀진 DB에서는
      // 아무 일도 하지 않아야 한다.
      if (current.includes("'works'")) return;
      ...
      try {
        ...
        CREATE TABLE mail_accounts_new (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          -- gmail · naver · works. 없앤 것이 아니라 하나 늘렸다.
          provider TEXT NOT NULL CHECK (provider IN ('gmail', 'naver', 'works')),
          address TEXT NOT NULL CHECK (length(address) BETWEEN 1 AND 320),
          status TEXT NOT NULL DEFAULT 'active'
            CHECK (status IN ('active', 'auth_required', 'error', 'disabled')),
          next_sync_at INTEGER NOT NULL DEFAULT 0,
        ...
        INSERT INTO mail_accounts_new (
          id, provider, address, status, next_sync_at, last_sync_at,
          last_error_code, last_error_at, auth_alert_sent_at, created_at, updated_at
        )
        SELECT id, provider, address, status, next_sync_at, last_sync_at,
               last_error_code, last_error_at, auth_alert_sent_at, created_at, updated_at
        FROM mail_accounts;
        ...
        CREATE INDEX idx_mail_accounts_due
          ON mail_accounts(status, next_sync_at);
        -- 자격증명이 provider당 한 세트라는 사실은 그대로다. 웍스가 늘었을 뿐
        -- 같은 provider의 두 번째 계정은 여전히 첫 계정의 사서함을 읽게 된다.
        CREATE UNIQUE INDEX idx_mail_accounts_one_per_provider
          ON mail_accounts(provider);
        `);
        const violations = db.pragma('foreign_key_check');
        if (violations.length) {
          throw new Error(`mail_accounts 재작성이 참조를 끊었습니다: ${JSON.stringify(violations)}`);
        }
        db.exec('COMMIT');
      } catch (error) {
        db.exec('ROLLBACK');
  ...
  },
  {
    ...
    name: 'news_agent',
    up(db) {
      // News Agent의 표를 한 번에 만든다. N4가 실제로 쓰는 것은 앞의 넷뿐이지만,
      // v19에서 배운 대로 한 기능의 스키마를 단계별로 쪼개면 나중에 CHECK나 FK를
      // 붙이려고 표를 다시 만드는 마이그레이션이 생긴다. 뒤의 둘은 v1.1까지
      // NEWS_AGENT_ENABLED 뒤에서 빈 채로 기다린다.
      //
      // **관심 자체는 여기 없다.** 관심 본문의 정본은 Markdown 노트이고
      // (`xion-news-context.md`), 이 표들이 드는 것은 worker 상태와 외부에서
      // 가져온 것뿐이다(설계 22.1). 그래서 `interest_id`에는 FK가 없다 —
      // 참조 대상이 DB에 없기 때문이고, 없다는 사실이 경계 그 자체다.
      //
      // 설계 단일 기준은 docs/xion-news-agent-design.md 8·11·12절이다.
      db.exec(`
        CREATE TABLE news_articles (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          -- 같은 기사를 다시 만나면 반드시 같은 값이 나와야 한다. 재료는
          -- canonical_url 하나이고 계산은 lib/news/normalize.js 한 곳에서 한다.
          identity_key TEXT NOT NULL UNIQUE CHECK (length(identity_key) = 64),
          canonical_url TEXT NOT NULL,
          -- 사용자가 실제로 여는 주소. 추적 파라미터가 붙어 있어도 그대로 둔다.
          url TEXT NOT NULL,
          title TEXT NOT NULL CHECK (length(title) BETWEEN 1 AND 400),
          source TEXT,
          -- 파싱된 epoch와 원문 표기를 함께 둔다. 파싱 실패를 NULL 하나로 접으면
          -- 날짜가 이상한 기사들이 서로 같은 값을 갖게 된다.
          ...
          -- **판단은 여기 없다.** 같은 기사가 두 관심에 걸릴 수 있고 relevance와
          -- "왜 가져왔는지"는 어느 관심에서 보느냐에 따라 다르다. 기사에 하나만
          -- 저장하면 한쪽 기준 판단이 다른 쪽 설명으로 새어 사용자에게 틀린 이유를
          -- 말하게 된다. 판단과 큐는 news_article_interests에 있다.

          -- 노출 (N5). 치우기는 기사 단위다 — "이 기사 안 볼래"는 관심별 판단이
          -- 아니라 그 기사 자체에 대한 것이다.
          surface_state TEXT NOT NULL DEFAULT 'none'
            CHECK (surface_state IN ('none', 'home', 'dismissed')),
          surfaced_at INTEGER
        );
        CREATE INDEX idx_news_articles_surface
          ON news_articles(surface_state, published_at);

        -- 어느 관심이 이 기사를 데려왔는지. "왜 가져왔어?"의 근거가 여기 있다
        -- (설계 15). 같은 기사가 두 관심에 걸리면 행이 둘이고 기사는 하나다.
        CREATE TABLE news_article_interests (
          article_id INTEGER NOT NULL REFERENCES news_articles(id) ON DELETE CASCADE,
          interest_id TEXT NOT NULL CHECK (length(interest_id) BETWEEN 1 AND 80),
          query TEXT,
          first_seen_at INTEGER NOT NULL,

          -- 분석 큐. 상태 이름과 전이는 메일 분석 큐와 같다(설계 16). 키가 기사가
          -- 아니라 쌍이라, 한 기사가 두 관심에 걸리면 판단도 두 번 돈다.
          analysis_state TEXT NOT NULL DEFAULT 'pending'
            CHECK (analysis_state IN ('pending', 'analyzing', 'done', 'failed', 'skipped')),
          analysis_attempt_count INTEGER NOT NULL DEFAULT 0,
          analysis_next_attempt_at INTEGER NOT NULL DEFAULT 0,
          analysis_lease_until INTEGER,
          analysis_error_code TEXT CHECK (analysis_error_code IS NULL OR length(analysis_error_code) <= 80),

          -- 판단 (N4b). 프롬프트 버전과 모델을 판단 행에 함께 남긴다 — 없으면
          -- 나중에 "프롬프트를 바꿔서 나빠졌나"를 되짚을 수 없다(설계 18).
        ...
        CREATE INDEX idx_news_article_interests_interest
          ON news_article_interests(interest_id, first_seen_at);
        CREATE INDEX idx_news_article_interests_queue
          ON news_article_interests(analysis_state, analysis_next_attempt_at);

        -- 관심별 수집 상태. 노트가 정본인 관심의 그림자라 노트에서 사라진
        -- interest_id의 행은 남아 있어도 아무 일도 하지 않는다.
        CREATE TABLE news_interest_polls (
          interest_id TEXT PRIMARY KEY CHECK (length(interest_id) BETWEEN 1 AND 80),
        ...
        -- 뉴스 전용 크레딧 하위 한도. 검색 예산은 채팅 웹 검색과 한 통이라
        -- 이것이 없으면 뉴스 폴링이 사용자의 채팅 검색을 굶길 수 있다.
        CREATE TABLE news_search_usage (
        ...
        -- 아래 둘은 v1.1(N3)의 것이다.
        CREATE TABLE news_review_candidates (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          interest_id TEXT NOT NULL CHECK (length(interest_id) BETWEEN 1 AND 80),
          kind TEXT NOT NULL DEFAULT 'interest_review' CHECK (kind IN ('interest_review')),
          state TEXT NOT NULL DEFAULT 'pending'
            CHECK (state IN ('pending', 'delivered', 'resolved', 'dismissed', 'expired')),
          question TEXT NOT NULL,
          created_at INTEGER NOT NULL,
          delivered_at INTEGER,
          resolved_at INTEGER
        );
        CREATE INDEX idx_news_review_candidates_state
          ON news_review_candidates(state, created_at);
        -- 관심 하나에 살아 있는 질문은 하나뿐이다. SELECT 뒤 INSERT로는 겹치는 두
        -- 턴이 둘 다 통과할 수 있어서, 이 불변식은 DB가 든다. 두 개가 열리면
        -- 서버는 첫 번째만 보므로 나머지는 답을 받을 수 없는 채로 남는다.
        CREATE UNIQUE INDEX idx_news_review_candidates_open
          ON news_review_candidates(interest_id)
          WHERE state IN ('pending', 'delivered');

        -- 재확인 질문의 기기별 전달. 전달 루프(claim·retry·410 만료·lease)는
        -- createAssistantPushDispatcher를 그대로 쓰고, 상태표만 도메인이 갖는다
        -- (메일 push.js와 같은 선례). 뉴스에는 batch도 snooze도 없어서 대상은
        -- candidate 하나뿐이다.
        CREATE TABLE news_push_deliveries (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          candidate_id INTEGER NOT NULL REFERENCES news_review_candidates(id),
          subscription_id INTEGER NOT NULL REFERENCES assistant_push_subscriptions(id),
          status TEXT NOT NULL DEFAULT 'pending'
            CHECK (status IN (
              'pending', 'sending', 'retry', 'accepted', 'failed', 'expired', 'skipped'
            )),
          ...
          created_at INTEGER NOT NULL DEFAULT (strftime('%s','now')),
          updated_at INTEGER NOT NULL DEFAULT (strftime('%s','now')),
          -- 한 기기에 같은 질문을 두 번 만들지 않는다.
          UNIQUE (candidate_id, subscription_id),
          CHECK (expires_at >= created_at),
          CHECK (
            (status = 'sending' AND lease_until IS NOT NULL) OR
            (status != 'sending' AND lease_until IS NULL)
        ...
        CREATE INDEX idx_news_push_deliveries_due
          ON news_push_deliveries(status, next_attempt_at);

        -- proactive 표시는 messages가 아니라 여기 있다(설계 11.4). UNIQUE 둘이
        -- "같은 candidate가 메시지를 두 번 만들지 않는다"를 조건문이 아니라 DB
        -- 제약으로 잠근다.
        CREATE TABLE news_proactive_messages (
          message_id INTEGER NOT NULL UNIQUE REFERENCES messages(id),
          candidate_id INTEGER NOT NULL UNIQUE REFERENCES news_review_candidates(id),
          interest_id TEXT NOT NULL CHECK (length(interest_id) BETWEEN 1 AND 80),
          created_at INTEGER NOT NULL
        );
      `);
  ...
  {
    version: 23,
    name: 'retrieval_trace_active_notes_input',
    up(db) {
      // NULL은 이 migration 전의 historical row처럼 input이 관측되지 않았다는 뜻이다.
      // 새 row의 []는 실제 retrieval input에 active note가 없었다는 exact observation이다.
      // 기존 notes_json은 retrieval output 의미를 유지하고 조용히 재해석하지 않는다.
      addColumnIfMissing(
        db,
  ...
  {
    version: 24,
    name: 'local_memory_inference_research_ledger',
    up(db) {
      db.exec(`
        ...
        CREATE INDEX IF NOT EXISTS idx_memory_inference_observations_time
        ON research_memory_inference_observations(occurred_at);

        CREATE INDEX IF NOT EXISTS idx_memory_inference_observations_workload_time
        ON research_memory_inference_observations(workload_type, occurred_at);
      `);
    },
  },
  {
    ...
    up(db) {
      addColumnIfMissing(
        ...
        'can_continue',
        'INTEGER NOT NULL DEFAULT 0 CHECK (can_continue IN (0, 1))',
      );
    },
  },
  {
    version: 26,
    name: 'retrieval_query_resolution_trace',
    up(db) {
      // NULL은 이 migration 전의 historical row처럼 해석 단계가 없었다는 뜻이다.
      // 기존 query_sha256은 계속 사용자 발화 원문의 hash이고 재해석하지 않는다.
      // retrieval_query_sha256은 실제로 검색에 쓰인 질의의 hash이며,
      // no_retrieval·ambiguous처럼 corpus 검색이 없었던 턴에서는 NULL이다.
      addColumnIfMissing(db, 'assistant_retrieval_shadow_runs', 'resolution_outcome', 'TEXT');
      addColumnIfMissing(db, 'assistant_retrieval_shadow_runs', 'retrieval_query_sha256', 'TEXT');
    },
  },
  {
    version: 27,
    name: 'independent_ddays',
    up(db) {
      db.exec(`
        CREATE TABLE ddays (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        CREATE INDEX idx_ddays_target_date ON ddays(target_date);
      `);
    },
  },
];
...
function runDatabaseMigrations(db) {
  ...
  for (const migration of migrations) {
    ...
    // 부모 표를 다시 만드는 migration은 자기 트랜잭션을 직접 연다. SQLite에서 부모
    // 재작성은 `PRAGMA foreign_keys=OFF`를 요구하는데 그 pragma는 트랜잭션 안에서
    // 무시되기 때문이다. 원자성은 그 migration이 BEGIN/COMMIT으로 직접 지키고,
    // 여기서는 버전 기록만 뒤이어 남긴다. 중간에 죽으면 다음 기동에서 다시 도는데,
    // 그런 migration은 이미 적용된 DB에서 아무 일도 하지 않아야 한다.
...
module.exports = {
  ...
  // 표 재작성 migration은 "어디까지 올린 DB"에서 시작해야 검증할 수 있다. 테스트가
  // 중간 버전을 만들 수 있게 배열을 연다.
  migrations,
