...
function loadRuns(db, { sinceEpoch = null, allModes = false } = {}) {
  ...
  const queryHashColumn = columns.has('query_sha256')
    ...
    : 'NULL AS querySha256';
  // schema v26 이전 row는 해석 단계 자체가 없었다. NULL을 `pass`로 읽지 않는다.
  const resolutionColumn = columns.has('resolution_outcome')
    ? 'resolution_outcome AS resolutionOutcome'
    : 'NULL AS resolutionOutcome';
  const retrievalQueryHashColumn = columns.has('retrieval_query_sha256')
    ? 'retrieval_query_sha256 AS retrievalQuerySha256'
    : 'NULL AS retrievalQuerySha256';
  const where = clauses.length > 0 ? `WHERE ${clauses.join(' AND ')}` : '';
  return db.prepare(`
    SELECT id, session_id AS sessionId, mode, ${queryHashColumn},
           ${resolutionColumn}, ${retrievalQueryHashColumn},
...
function loadChunkReviewData(db, chunkIds) {
  ...
  const rows = db.prepare(`
    SELECT c.chunk_id AS chunkId, c.note_filename AS noteFilename, c.content
    FROM note_chunks c
...
function buildReviews(db, rows, limit) {
  ...
  return selectedGroups.map(([, groupRows]) => {
    ...
    return {
      ...
      resolutionOutcome: representative.resolutionOutcome || null,
...
function buildRetrievalShadowReport({
  ...
  // corpus 검색을 아예 하지 않은 턴이다. 중단(검색은 돌았는데 고른 것이 없음)과
  // 섞으면 A2 회수 품질 지표가 조용히 희석된다.
  const withoutCorpusRetrieval = normalized.filter(row => (
    row.resolutionOutcome === 'no_retrieval' || row.resolutionOutcome === 'ambiguous'
  ));
  const byResolution = Object.entries(
    Object.groupBy(normalized, row => row.resolutionOutcome || 'unrecorded'),
  )
    .map(([outcome, outcomeRows]) => ({ outcome, runs: outcomeRows.length }))
  ...
  const report = {
    ...
    byMode,
    byResolution,
    runsWithoutCorpusRetrieval: withoutCorpusRetrieval.length,
    // 원문과 다른 질의로 검색이 나간 실행. 질의 본문은 어디에도 저장하지 않는다.
    rewrittenQueryRuns: normalized.filter(row => (
      row.retrievalQuerySha256 && row.retrievalQuerySha256 !== row.querySha256
    )).length,
    ...
    abstentions: normalized.filter(row => (
      row.chunks.length === 0
      && row.resolutionOutcome !== 'no_retrieval'
      && row.resolutionOutcome !== 'ambiguous'
    )).length,
...
function formatRetrievalShadowReport(report) {
  ...
  const lines = [
    ...
    `질의 해석: ${report.byResolution.map(item => `${item.outcome} ${item.runs}`).join(', ') || '없음'} · corpus 미검색 ${report.runsWithoutCorpusRetrieval}건 · 원문과 다른 질의 ${report.rewrittenQueryRuns}건`,
  ...
  for (const review of report.reviews || []) {
    lines.push('', `[${review.traceIds.join(', ')}] ${formatKst(review.createdAt)} · ${review.modes.join(', ')} · 해석 ${review.resolutionOutcome || '기록 없음'}`);
