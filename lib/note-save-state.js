...
function createNoteSaveStateReader(db) {
  ...
  const listSessionMessages = db.prepare(`
    ${sessionMessageSelect}
    WHERE m.session_id = ?
    ORDER BY m.created_at ASC, m.id ASC
  `);
  const listNewestSessionMessages = db.prepare(`
    ${sessionMessageSelect}
    WHERE m.session_id = ?
    ORDER BY m.created_at DESC, m.id DESC
    LIMIT ?
  `);
  const listOlderSessionMessages = db.prepare(`
    ${sessionMessageSelect}
    WHERE m.session_id = ?
      AND (
        m.created_at < ?
  ...
  function listSessionMessagePage(sessionId, { limit, beforeCreatedAt = null, beforeId = null }) {
    const rows = beforeCreatedAt === null
      ? listNewestSessionMessages.all(sessionId, limit + 1)
      : listOlderSessionMessages.all(
        sessionId,
        beforeCreatedAt,
        beforeCreatedAt,
        beforeId,
