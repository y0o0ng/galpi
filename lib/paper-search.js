...
const S2_FIELDS = 'title,abstract,year,authors,citationCount,externalIds,url,tldr,openAccessPdf';
...
function normalizePaper(item) {
  ...
  const authors = (Array.isArray(item.authors) ? item.authors : [])
    .map(author => sanitizePaperText(typeof author === 'string' ? author : author?.name, 160))
    .filter(Boolean)
    .slice(0, 30);
  ...
  const citationCount = Number.isFinite(Number(item.citationCount))
    ...
    : 0;
  const doi = sanitizePaperText(item.doi || item.externalIds?.DOI, 240) || null;
  const arxivId = sanitizePaperText(item.arxivId || item.externalIds?.ArXiv, 120) || null;
  const openAccessPdfUrl = normalizeHttpUrl(item.openAccessPdfUrl || item.openAccessPdf?.url) || null;
  ...
  return {
    ...
    authors,
    citationCount,
    tldr: sanitizePaperText(typeof item.tldr === 'string' ? item.tldr : item.tldr?.text, 1200) || null,
...
function retryDelayMs(retryIndex, randomImpl) {
  return RETRY_BASE_DELAY_MS * (2 ** retryIndex)
    + Math.floor(randomImpl() * RETRY_JITTER_MS);
}
  ...
  if (!mockEnabled) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt += 1) {
      ...
      try {
        ...
        if (response.ok) {
          throw new PaperSearchError(
            'Semantic Scholar가 올바르지 않은 응답을 보냈습니다. 잠시 후 다시 시도해주세요.',
            502,
            'invalid_response',
          );
        }
