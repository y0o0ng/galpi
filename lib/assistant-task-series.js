...
function normalizePatchInput(input, current, now) {
  ...
  if (Object.prototype.hasOwnProperty.call(input, 'recurrence')) {
    ...
    // 시작일 범위는 사용자가 anchor를 **실제로 옮길 때만** 본다. 보내온 값이 지금과
    // 같으면 그냥 규칙을 그대로 다시 적은 것이다. 있다는 이유만으로 검사하면,
    // 규칙 전체를 담아 보내는 시온의 후보 카드가 한 달 된 반복에서 전부 거절된다.
...
// 카드와 컨텍스트가 같은 문장을 쓰도록 규칙을 한 줄로 적는다.
function describeRule(series) {
  ...
  if (series.freq === 'weekly') {
    const days = String(series.byWeekday || '').split(',').filter(Boolean)
      .map(day => WEEKDAY_NAMES[Number(day) - 1]).filter(Boolean);
...
function createAssistantTaskSeriesStore(db, options = {}) {
  ...
  const selectPastOccurrences = db.prepare(`
    ...
    -- 회차 날짜가 아니라 실제 기한 순이다. 사용자가 어떤 회차만 뒤로 옮겼으면
    -- 가장 최근 놓친 회차는 그 옮긴 회차이고, 남겨야 하는 것도 그것이다.
    ORDER BY t.series_id ASC, t.due_date ASC, t.due_at ASC, t.id ASC
  `);
  ...
  return {
    ...
    // 아래 두 함수는 아무것도 쓰지 않는다. 시온이 만든 후보를 확인 카드에 띄우기
    // 위한 것이고, 저장은 사용자가 카드를 눌렀을 때 위의 create·update·end가 한다.
    prepare(input, options = {}) {
      const capturedAt = Math.floor(
        options.capturedAt === undefined ? captureNow() : Number(options.capturedAt)
      );
      if (!Number.isFinite(capturedAt)) {
        taskError('후보 기준 시각이 올바르지 않습니다.', 'INVALID_CAPTURED_AT');
      }
      const normalized = normalizeCreateInput(input, capturedAt);
      return {
        capturedAt,
        ...
        series: {
          ...
          recurrence: {
            ...
            byWeekday: normalized.byWeekday,
            byMonthday: normalized.byMonthday,
            startDate: normalized.startDate,
            endDate: normalized.endDate,
            timeKind: normalized.timeKind,
            timeOfDay: normalized.timeOfDay,
            reminderLeadSeconds: normalized.reminderLeadSeconds,
          },
        },
      };
    },

    // 대상을 서버가 직접 확인한다. 없는 회차, 이미 끝난 회차, 반복이 아닌 일정,
    // 종료된 시리즈는 모두 여기서 막힌다. 모델이 대상을 못 고르면 후보가 생기지
    // 않고 오류가 돌아가므로 되묻게 된다.
    prepareOverride(input, options = {}) {
      const capturedAt = Math.floor(
        options.capturedAt === undefined ? captureNow() : Number(options.capturedAt)
      );
      if (!Number.isFinite(capturedAt)) {
      ...
      if (!input || typeof input !== 'object' || Array.isArray(input)) {
        taskError('반복 변경 요청 본문이 올바르지 않습니다.', 'INVALID_OVERRIDE_INPUT');
      }
      const action = input.action;
      if (!['skip', 'reschedule', 'series_update', 'end'].includes(action)) {
        taskError('action은 skip, reschedule, series_update, end 중 하나여야 합니다.', 'INVALID_OVERRIDE_ACTION');
      }

      if (action === 'skip' || action === 'reschedule') {
        const task = taskStore.get(validateId(input.taskId, '회차')).task;
        if (task.seriesId === null) {
          taskError('반복 회차가 아닌 일정입니다.', 'NOT_AN_OCCURRENCE', 409);
        }
        if (task.status !== 'active' || task.lifecycle !== 'active') {
          taskError('이미 종결된 회차는 바꿀 수 없습니다.', 'OCCURRENCE_NOT_ACTIVE', 409);
        }
        const series = requireSeries(task.seriesId);
        const base = {
          capturedAt,
          timezone: KST_TIMEZONE,
          kind: 'override',
          override: {
            ...
            recurrenceLabel: describeRule(series),
          },
        };
        if (action === 'skip') return base;

        const due = validateDue(input.due, capturedAt);
        if (due.dueKind !== series.timeKind) {
          taskError(
            series.timeKind === 'datetime'
              ? '이 반복은 시각이 있는 회차라 날짜와 시각을 함께 정해야 합니다.'
              : '이 반복은 날짜만 있는 회차라 시각을 정할 수 없습니다.',
            'OVERRIDE_DUE_KIND_MISMATCH'
          );
        }
        base.override.due = due.dueKind === 'date'
          ? { kind: 'date', date: due.dueDate }
          : { kind: 'datetime', at: kstDateTime(due.dueAt) };
        return base;
      }

      const series = requireSeries(validateId(input.seriesId, '반복 일정'));
      if (series.status !== 'active') {
        taskError('이미 종료된 반복입니다.', 'SERIES_NOT_EDITABLE', 409);
      }
      const override = {
        action,
        seriesId: series.id,
      ...
      if (action === 'series_update') {
        const patch = normalizePatchInput(
          { expectedVersion: series.version, recurrence: input.recurrence },
          series,
          capturedAt
        );
        if (!patch.recurrence) {
          taskError('바꿀 반복 규칙이 필요합니다.', 'INVALID_RECURRENCE');
        }
        override.recurrence = patch.recurrence;
        override.nextRecurrenceLabel = describeRule({
          ...patch.recurrence,
          byWeekday: patch.recurrence.byWeekday === null ? null : patch.recurrence.byWeekday.join(','),
        });
      }
      return { capturedAt, timezone: KST_TIMEZONE, kind: 'override', override };
    },
