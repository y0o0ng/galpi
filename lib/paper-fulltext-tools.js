...
function paperCandidateFromNote(note) {
  ...
  return {
    ...
    sourceUrl: String(metadata.open_access_pdf_url || '').trim() || arxivPdfUrl(metadata.arxiv_id),
  };
}
...
function createPaperFullTextTools({
  ...
  async function ensurePaperIndex(candidate) {
    ...
    if (
      ...
      && (embeddingsReady || !candidate.sourceUrl)
    ) {
    ...
    const run = (async () => {
      const downloaded = await downloadPdf(candidate.sourceUrl);
