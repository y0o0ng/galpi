...
function loadRows(db, startEpoch, endEpoch) {
  ...
  return db.prepare(`
    SELECT workload_type AS workloadType, occurred_at AS occurredAt,
           ...
           ledger_schema_version AS ledgerSchemaVersion,
           instrumentation_version AS instrumentationVersion
...
function observationContract(rows) {
  const counts = new Map();
  for (const row of rows) {
    const key = JSON.stringify([row.ledgerSchemaVersion, row.instrumentationVersion]);
    const current = counts.get(key) || {
      ledgerSchemaVersion: row.ledgerSchemaVersion,
      instrumentationVersion: row.instrumentationVersion,
      rows: 0,
    };
    current.rows += 1;
    counts.set(key, current);
  }
  const observed = [...counts.values()].sort((left, right) => (
    left.ledgerSchemaVersion - right.ledgerSchemaVersion
    || left.instrumentationVersion.localeCompare(right.instrumentationVersion)
  ));
  return {
    current: {
    ...
    observed,
    coherent: observed.length <= 1,
    matchesCurrent: observed.every(item => (
      item.ledgerSchemaVersion === LEDGER_SCHEMA_VERSION
      && item.instrumentationVersion === INSTRUMENTATION_VERSION
    )),
  };
}
...
function buildMemoryInferencePilotReport({
  ...
  if (coverageStatus === COVERAGE_STATUSES.COMPLETE && instrumentationFailureCount !== 0) {
    throw new TypeError('COMPLETE coverage에는 외부 확인된 instrumentation failure count가 정확히 0이어야 합니다.');
  }
  const days = listKstDays(startEpoch, endEpoch);
  const rows = loadRows(db, startEpoch, endEpoch);
  const contract = observationContract(rows);
  if (
    coverageStatus === COVERAGE_STATUSES.COMPLETE
    && (!contract.coherent || !contract.matchesCurrent)
  ) {
    throw new TypeError('COMPLETE coverage에는 current observation contract로 일관된 window가 필요합니다.');
  }
  ...
  return {
    ...
    coverage: {
      ...
      reason: coverageStatus === COVERAGE_STATUSES.COMPLETE
        ? 'EXTERNALLY_VERIFIED_ZERO_FAILURE_CURRENT_CONTRACT_WINDOW'
        : instrumentationFailureCount > 0
          ? 'INSTRUMENTATION_FAILURES_OBSERVED'
          : !contract.coherent
            ? 'MIXED_OBSERVATION_CONTRACTS'
            : !contract.matchesCurrent
              ? 'NON_CURRENT_OBSERVATION_CONTRACT'
    ...
    instrumentationFailures: {
      ...
      note: instrumentationFailureCount === null
        ...
        : instrumentationFailureCount === 0
          ? 'An independently verified instrumentation log review found zero failed writes.'
          : 'An independently verified instrumentation log review found failed writes; missing incidence was not estimated.',
    },
    ledgerRows: rows.length,
    workloads,
  };
...
function formatMemoryInferencePilotReport(report) {
  const lines = [
    ...
    `Observation contract current: schema ${report.observationContract.current.ledgerSchemaVersion} · ${report.observationContract.current.instrumentationVersion}`,
    `Observation contracts observed: ${report.observationContract.observed.length === 0
      ? 'none'
      : report.observationContract.observed.map(item => (
        `schema ${item.ledgerSchemaVersion} · ${item.instrumentationVersion} · rows ${item.rows}`
      )).join('; ')} · coherent: ${report.observationContract.coherent ? 'yes' : 'no'} · current-only: ${report.observationContract.matchesCurrent ? 'yes' : 'no'}`,
