...
function createVoiceShortcutService(db, options = {}) {
  ...
  const claimTransaction = db.transaction((credentialIdValue, turn) => {
    ...
    const currentTime = now();
    const existing = getReceipt.get(credentialId, turn.requestId);
    if (existing) {
      ...
      return { kind: 'new', ...getReceipt.get(credentialId, turn.requestId), retry: true };
    }
    ...
    return { kind: 'new', ...getReceipt.get(credentialId, turn.requestId), retry: false };
  });
  ...
  return {
    ...
    completeRequest({
      ...
      return getReceipt.get(credentialId, requestId);
    },
  };
}
