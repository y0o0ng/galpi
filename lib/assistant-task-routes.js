...
function registerAssistantTaskRoutes({ app, store, enabled }) {
  ...
  app.get('/api/tasks/summary', requireEnabled, (_req, res) => {
    try {
      return res.json(store.summary());
    } catch (error) {
      return sendError(res, error);
    }
  });
  ...
  for (const action of ['complete', 'cancel', 'reopen', 'delete', 'restore']) {
    app.post(`/api/tasks/:id/${action}`, requireEnabled, requireJson, (req, res) => {
      try {
        return res.json(store.transition(req.params.id, action, req.body));
      } catch (error) {
        return sendError(res, error);
      }
    });
