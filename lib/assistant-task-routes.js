...
function registerAssistantTaskRoutes({
  app,
  store,
  enabled,
  seriesStore = null,
  seriesEnabled = false,
  onTaskMutation = async () => {},
}) {
  ...
  if (seriesEnabled && !seriesStore?.create) throw new TypeError('반복 일정 저장소가 필요합니다.');

  // 반복은 일정 위에 얹히므로 일정 자체가 꺼져 있으면 함께 꺼진다.
  const seriesReady = Boolean(enabled && seriesEnabled && seriesStore);
  ...
  function requireSeriesEnabled(_req, res, next) {
    if (!seriesReady) {
      return res.status(503).json({
        error: '반복 일정 기능이 아직 활성화되지 않았습니다.',
        code: 'TASK_SERIES_DISABLED',
      });
    }
    return next();
  }
  ...
  app.get('/api/task-series', requireSeriesEnabled, (req, res) => {
    try {
      return res.json(seriesStore.list({ status: req.query.status }));
    } catch (error) {
      return sendError(res, error);
    }
  });

  app.get('/api/task-series/:id/occurrences', requireSeriesEnabled, (req, res) => {
    try {
      return res.json({ occurrences: seriesStore.listOccurrences(req.params.id) });
    } catch (error) {
      return sendError(res, error);
    }
  });

  app.post('/api/task-series', requireSeriesEnabled, requireJson, async (req, res) => {
    try {
      const result = seriesStore.create(req.body);
      await onTaskMutation();
      return res.status(result.replayed ? 200 : 201).json(result);
    } catch (error) {
      return sendError(res, error);
  ...
  app.patch('/api/task-series/:id', requireSeriesEnabled, requireJson, async (req, res) => {
    try {
      const result = seriesStore.update(req.params.id, req.body);
      await onTaskMutation();
      return res.json(result);
    } catch (error) {
      return sendError(res, error);
  ...
  app.post('/api/task-series/:id/end', requireSeriesEnabled, requireJson, async (req, res) => {
    try {
      const result = seriesStore.end(req.params.id, req.body);
      await onTaskMutation();
      return res.json(result);
    } catch (error) {
      return sendError(res, error);
  ...
  app.use((error, req, res, next) => {
    const isTaskPath = /^\/api\/(?:tasks|task-series|reminders)(?:\/|$)/.test(req.path);
