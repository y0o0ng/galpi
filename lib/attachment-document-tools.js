...
function createAttachmentDocumentTools({ documentService } = {}) {
  ...
  function createSession({ sessionId, attachmentIds = [] } = {}) {
    const candidates = documentService.listCandidates({
      sessionId,
      attachmentIds,
    ...
    function getToolDefinitions() {
      ...
      if (phase === 'search') {
        ...
        return [{
          ...
          description: `Search only the temporary attachments available to this chat turn. Use focused for a specific question and overview for a summary or broad review. Available attachments: ${available}`,
          input_schema: {
            type: 'object',
