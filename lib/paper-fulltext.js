...
function createPaperFullTextService({
  ...
  const getActivePaper = db.prepare(`
    ...
    FROM notes
    WHERE paper_id = ? AND note_type = 'paper' AND archived = 0
    LIMIT 1
  `);
  ...
  function searchPaper({ paperId, query, queryEmbedding = null, limit = 4 } = {}) {
    const normalizedPaperId = String(paperId || '').trim();
  ...
  return { getDocument: paperId => getDocument.get(paperId) || null, indexPaper, searchPaper };
}
