...
function buildTopicRepairPlan(report) {
  ...
  for (const finding of findings.duplicateFileQaIds || []) {
    ...
    const signaledOwners = uniqueSorted(Object.values(ownerSignals).flat());
    const ownerCandidate = signaledOwners.length === 1 ? signaledOwners[0] : null;
    ...
    operations.push({
      ...
      evidence: {
        ...
        ownerSignals,
        identicalFileContent: uniqueSorted(locations.map(item => item.contentSha256)).length === 1,
      },
      recommendation: {
        action: 'resolve_duplicate_qa_id',
        preserveQaIdIn: ownerCandidate,
        assignNewQaIdIn: ownerCandidate
  ...
  for (const finding of findings.dbOnlyChunks || []) {
    ...
    operations.push({
      ...
      kind: 'db_only_chunk',
      status: hasContentMatch ? 'manual_review' : 'ready',
      target: { chunkId: finding.chunkId, filename: finding.filename },
      reason: hasContentMatch
      ...
      recommendation: {
        action: hasContentMatch ? 'reconcile_chunk_with_file_qa' : 'mark_source_missing',
      },
    });
  }
