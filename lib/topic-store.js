...
function topicQaEntryToChunkContent(entry) {
  const content = comparableQaEntryText(entry?.content);
  if (!content) throw new Error(`Q&A 청크 본문을 만들 수 없습니다: ${entry?.qaId || 'unknown'}`);
  return content;
}
...
function findQaEntryStarts(body) {
...
function parseQaLog(raw) {
  ...
  const starts = findQaEntryStarts(body);
  let match;
...
function qaEntryContent(entry) {
  return normalizeForHash(typeof entry === 'string' ? entry : entry?.content);
}

function restoreNewlines(value, newline) {
  return newline === '\r\n' ? value.replace(/\n/g, '\r\n') : value;
}

function validateQaLogMutation(next, expectedQaIds) {
  const parsed = parseQaLog(next);
  if (!parsed.parseable) {
    const detail = parsed.issues.map(item => `${item.code}: ${item.message}`).join('; ');
    throw new Error(`QA-LOG 수정 결과가 올바르지 않습니다: ${detail}`);
  }
  const actualQaIds = parsed.entries.map(entry => entry.qaId);
  if (
    actualQaIds.length !== expectedQaIds.length
    || actualQaIds.some((qaId, index) => qaId !== expectedQaIds[index])
  ) {
    throw new Error('QA-LOG 수정 결과의 Q&A 순서가 계획과 다릅니다.');
  }
  return next;
}

function appendQaLogEntries(raw, entries) {
  const original = String(raw || '');
  const parsed = parseQaLog(original);
  if (!parsed.parseable) throw new Error('QA-LOG를 안전하게 수정할 수 없습니다.');

  const additions = (entries || []).map(qaEntryContent).filter(Boolean);
  if (additions.length === 0) return original;

  const newline = original.includes('\r\n') ? '\r\n' : '\n';
  const normalized = original.replace(/\r\n?/g, '\n');
  const end = normalized.indexOf(QA_LOG_END, normalized.indexOf(QA_LOG_START) + QA_LOG_START.length);
  const before = normalized.slice(0, end).replace(/[ \t\n]*$/, '');
  const next = `${before}\n\n${additions.join('\n\n')}\n\n${normalized.slice(end)}`;
  const expectedQaIds = [
    ...parsed.entries.map(entry => entry.qaId),
    ...additions.map(content => {
      const match = content.match(/<!--\s*qa_id:\s*([^>]+?)\s*-->/);
      return match?.[1]?.trim() || '';
    }),
  ];
  return restoreNewlines(validateQaLogMutation(next, expectedQaIds), newline);
}

function replaceQaLogEntries(raw, entries) {
  const original = String(raw || '');
  const parsed = parseQaLog(original);
  if (!parsed.parseable) throw new Error('QA-LOG를 안전하게 수정할 수 없습니다.');

  const replacements = (entries || []).map(qaEntryContent).filter(Boolean);
  const newline = original.includes('\r\n') ? '\r\n' : '\n';
  const normalized = original.replace(/\r\n?/g, '\n');
  const start = normalized.indexOf(QA_LOG_START);
  const bodyStart = start + QA_LOG_START.length;
  const end = normalized.indexOf(QA_LOG_END, bodyStart);
  const body = replacements.length > 0
    ? `\n\n${replacements.join('\n\n')}\n\n`
    : '\n';
  const next = normalized.slice(0, bodyStart) + body + normalized.slice(end);
  const expectedQaIds = replacements.map(content => {
    const match = content.match(/<!--\s*qa_id:\s*([^>]+?)\s*-->/);
    return match?.[1]?.trim() || '';
  });
  return restoreNewlines(validateQaLogMutation(next, expectedQaIds), newline);
...
function removeQaLogEntry(raw, { qaId, expectedContentSha256 } = {}) {
  if (!qaId || !expectedContentSha256) {
    throw new TypeError('qaId와 expectedContentSha256이 필요합니다.');
  }

  const original = String(raw || '');
  const newline = original.includes('\r\n') ? '\r\n' : '\n';
  const normalized = original.replace(/\r\n?/g, '\n');
  const parsed = parseQaLog(normalized);
  if (!parsed.parseable) throw new Error('QA-LOG를 안전하게 수정할 수 없습니다.');

  const matches = parsed.entries.filter(entry => entry.qaId === qaId);
  if (matches.length !== 1) {
    throw new Error(`삭제할 qa_id가 정확히 1개여야 합니다: ${qaId}`);
  }
  if (matches[0].contentSha256 !== expectedContentSha256) {
    throw new Error(`Q&A 본문 hash가 계획과 다릅니다: ${qaId}`);
  }

  const markerStart = normalized.indexOf(QA_LOG_START);
  const bodyStart = markerStart + QA_LOG_START.length;
  const bodyEnd = normalized.indexOf(QA_LOG_END, bodyStart);
  const body = normalized.slice(bodyStart, bodyEnd);
  const starts = findQaEntryStarts(body);
  const targetIndex = starts.findIndex(entry => entry.qaId === qaId);
  if (targetIndex < 0) throw new Error(`QA-LOG 범위를 찾지 못했습니다: ${qaId}`);

  const removeStart = bodyStart + starts[targetIndex].index;
  const removeEnd = bodyStart + (starts[targetIndex + 1]?.index ?? body.length);
  const next = normalized.slice(0, removeStart) + normalized.slice(removeEnd);
  return newline === '\r\n' ? next.replace(/\n/g, '\r\n') : next;
}
...
async function auditTopicStore({ db, vaultPath }) {
  ...
  `).all();
  const autoSaveColumns = tables.has('auto_save_decisions')
    ? columnNames(db, 'auto_save_decisions')
    : new Set();
  const autoSaveRows = tables.has('auto_save_decisions')
    ? db.prepare(`
      SELECT
        ...
        qa_id AS qaId,
        note_filename AS noteFilename,
        ${autoSaveColumns.has('session_id') ? 'session_id' : 'NULL'} AS sourceSession,
        source_user_message AS sourceUserMessage,
        source_assistant_message AS sourceAssistantMessage,
        decision,
        action
  ...
  for (const chunk of chunkRows) {
    ...
    if (rowArchived(note)) {
      observations.archivedChunks.push({ chunkId: chunk.chunkId, filename: chunk.noteFilename });
      continue;
    }
  ...
  return {
    ...
    summary: {
      ...
      archivedChunks: observations.archivedChunks.length,
...
function buildTopicRepairPlan(report) {
  ...
  for (const [index, finding] of (findings.fileOnlyQa || []).entries()) {
    const decisions = decisionsById.get(finding.qaId) || [];
    const saveDecisions = decisions.filter(item => item.decision === 'save');
    const matchingDecisions = saveDecisions.filter(item => item.noteFilename === finding.filename);
    const ambiguousProvenance = saveDecisions.length > 1
      || (saveDecisions.length === 1 && matchingDecisions.length !== 1);
    operations.push({
      id: `file_only_qa:${finding.qaId}:${index + 1}`,
      kind: 'file_only_qa',
      status: ambiguousProvenance ? 'manual_review' : 'ready',
      target: { qaId: finding.qaId, chunkId: null, filename: finding.filename },
      reason: ambiguousProvenance
        ? '자동 저장 출처가 여러 개이거나 현재 Markdown 배정과 달라 자동 재색인할 수 없습니다.'
        : 'Markdown에 정본 Q&A가 있지만 검색 청크가 없습니다.',
      evidence: {
        finding: { ...finding },
        fileEntries: fileEntriesById.get(finding.qaId) || [],
        dbChunks: dbChunksById.get(finding.qaId) || [],
        autoSaveDecisions: decisions,
      },
      recommendation: {
        action: ambiguousProvenance ? 'inspect_file_qa_provenance' : 'reindex_file_qa',
      },
    });
  }
...
function formatTopicStoreAudit(report) {
  ...
  if (observations.archivedChunks?.length > 0) {
    lines.push(
      '',
      `Archived note chunks (${observations.archivedChunks.length})`,
      ...formatList(
        observations.archivedChunks,
