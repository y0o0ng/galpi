...
- **메일 에이전트가 다음 제품 작업이고 MAIL-1은 2026-08-17 실계정 인수를 마쳤다**(설계는 `docs/xion-mail-agent-design-final.md`, 로드맵은 V 단계가 아닌 독립 트랙 `MAIL-1~4`). 다음은 MAIL-2(분석 큐 + Attention)다. **로컬 `.env`에만 자격증명이 있고 Pi에는 아직 배포하지 않았다.** **`V5-C`라고 부르지 않는다 — 그 이름은 외부 캘린더 일정 에이전트가 이미 쓰고 있다.** **이 항목은 진행 중에만 둔다 — 인수가 끝나면 여기서 지우고 상세는 설계 문서와 git에 남긴다.** 되짚기 쉬운 계약만 적는다. **IMAP UID는 locator이지 identity가 아니다** — dedup은 `Message-ID`(없으면 헤더+본문 digest fingerprint)로 하고, UID로 하면 `UIDVALIDITY`가 바뀌는 순간 메일함 전체가 새 메일이 된다. **sync와 LLM 분석은 같은 tick에서 끝나지 않는다** — `analysis_state` + lease + backoff + 상한이 있어야 Codex queue에서 겪은 좌초가 재발하지 않는다. **Push 전달 정본은 기기별 `mail_push_deliveries`다** — 메시지 행의 상태 컬럼 하나로는 기기 둘을 표현할 수 없고, 기존 `assistant_push_deliveries`는 건드리지 않고 delivery loop만 공유한다. 잠금화면 미리보기는 기본 `숨김`이고 문구는 서버가 만든다. **Google OAuth는 `In production`으로 게시하되 인증은 제출하지 않는다** — 개인 사용 면제가 공식 문서에 있고, 제출하면 restricted scope라 CASA 보안 평가가 붙는다. 클라이언트 유형은 `웹 애플리케이션`이다. MIME 파서는 `postal-mime` + 자체 HTML→text이고 `mailparser`는 발송 스택(`nodemailer`)을 끌고 와서 쓰지 않는다. v1은 읽기 전용이라 삭제·이동·읽음 처리·답장을 하지 않고, 새 Dashboard·메일 상세 페이지·범용 Notification Framework를 만들지 않는다. 전부 `MAIL_AGENT_ENABLED` 기본 `false` 뒤다.
