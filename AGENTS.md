...
- 남은 단일 미해결은 **NDX100 과거 구성원**이고 이것이 플랜을 가른다. 있으면 EOD+Fundamentals 1개월 $79.98, 없으면 EOD $19.99 + 구성원·실적일 자체 구축(12.4가 지시한 경로, 지수 공고와 SEC EDGAR 8-K는 공개 자료라 영구 보존 가능)이 낫다. 실적 `published_at`은 살 필요 없다 — 관측된 마지막 실제 실적일 + 91일로 추정하고 `confidence='estimated'`로 두면 point-in-time 안전하다.
