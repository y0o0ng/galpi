...
- V4-B 음성·Realtime 상태: `docs/voice-realtime-design.md`를 상세 단일 기준으로 삼아 R0 raw WebRTC를 2026-07-30 Pi 운영에 활성화했다. `lib/realtime-session.js`의 서버 소유 unified SDP proxy와 `public/voice-realtime.js`의 media/data channel 상태 머신, `gpt-realtime-2.1-mini`·`marin`, `gpt-4o-mini-transcribe` 한국어 자막, semantic VAD 끼어들기, 5분 hard cap, mute·종료·공용 cleanup을 기능 flag 뒤에 뒀다. tool·DB·Vault·task 쓰기 경로는 없고 코드 기본 flag는 `false`, Pi에서만 `true`다. 공식 multipart 일반 필드 형식과 socket 주소 기반 rate-limit key로 실제 호출·Tailscale proxy 경계를 보정했다. Mac·Pi HTTPS에서 `201`, connected peer, open data channel, remote audio, 완료 자막을 확인했고 로컬/Pi Realtime 7/7, Pi 전체 214/214, DB·Vault·task 불변과 SQLite 무결성을 통과했다. 백업은 `20260730-1744`, 코드 복구본은 `code-v4b-r0-pre-20260730-174417.tar.gz`, 최종 PID는 `124693`이다. iPhone 실제 마이크 한국어 10턴·끼어들기 5회·종료 인수 전에는 R1 A2 기억·활성 일정 read-only를 시작하지 않는다. 텍스트 GPT-5.6 모델 picker와 Realtime 모델은 별도 경계이고 상시 마이크·호출어·백그라운드 지속 대화·원본 오디오 보관은 범위 밖이다.
