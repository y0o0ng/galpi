...
- **다음은 남은 미커버 14구간이다.** 옛 계열을 못 찾은 것(`ACT`·`MI`·`MICC`·`DV`), 구간 시작이 시리즈 생성보다 이른 것(`DISCK`·`LMCK`), 트래킹 주식(`LILA`·`LILAK`), 구간을 쪼개야 하는 것(`FOX`·`FOXA` 4구간, `_drop_same_day_reuse`가 2019-03-19 쌍을 상쇄해 21세기폭스와 폭스코퍼레이션이 이어져 있다), 그리고 `COV`·`HET`이다. 그것까지 비워야 `SURVIVORSHIP_BIASED`가 풀리고 그 뒤가 워크포워드와 14.7 판정이다.
