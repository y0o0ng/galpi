...
- 다음 개발 시작점: `docs/voice-halfduplex-design.md`의 H2 저장(음성 턴을 `shared-main`으로 옮기고 메인 채팅 영역에 렌더링)부터 간다. 그 뒤 H3 검증·되묻기 → H4 일정·메모 확인 카드 → H5 Realtime 정리다. 지연 개선은 스트리밍 TTS(약 1,000ms) → 침묵 1200→1000ms(200ms) → earcon 순서이며 아직 미구현이다. 폰↔Pi 왕복 3회를 1회로 줄이는 통합은 설계만 남겼고 스트리밍 TTS 구현 시 함께 검토한다. 더 빠른 텍스트 모델로 내리는 선택은 하지 않는다.
