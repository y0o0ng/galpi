...
- **다음은 미커버 57구간 중 "편출일 지연"을 정상으로 인정하는 판정이다.** 성격이 셋으로 갈린다 — 진짜 티커 재사용·부재 24구간(22심볼), **편출일 지연 22구간**, 나머지 11구간. 지연은 결함이 아니다. 스냅샷이 "늦어도 이 판"이라 회사가 이미 거래를 멈춘 뒤로 구간이 며칠 더 이어지고(`ABC` 28일·`BDK` 23일·`ERTS` 12일) 그 기간에 바가 없는 것이 맞으며, 바가 없으면 백테스트가 못 고르므로 fail-close다. `uncovered_intervals`가 이 셋을 구분하지 못하는 것이 지금의 한계이고, 인정 판정을 넣으면 57 → 35구간이 되어 `SURVIVORSHIP_BIASED` 해제가 눈앞에 온다. 그 뒤가 워크포워드와 14.7 판정이다.
