...
- **남은 blocker 하나: 수리본의 새 blind HUMAN 재검토다.** 패킷 빌더는 끝났고(`scripts/build-memory-inference-p1b6-surface-repair-human-review-packet.js`, 생성만 하고 커밋하지 않는다) 행은 `reviewRowId`+`selectedBundle` 둘뿐이며 **source-audit 이유는 의미 판단을 오염시키므로 넣지 않는다.** 재검토 자체는 아직 안 했고, accepted pool 30·HUMAN gold·HELD_OUT·학습은 그대로다. **P1-B6은 아직 완료가 아니다.**
