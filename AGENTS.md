...
- **누출 검사는 batch-003 전용이다** — pilot·batch-001·batch-002·batch-002 후속본과 batch-003 내부에 대해 정규화 대화 재사용, 12자 이상 정확 턴 재사용, 식별자 재사용, family의 split 교차를 거부한다. 템플릿 재사용 진단은 게이트가 아니라 사람이 볼 보고서다.
- **다음 실제 게이트는 304행 전체의 fresh source audit이고 아직 실행하지 않았다.** 범용 빌더로 패킷 생성만 확인했다(관례대로 커밋 안 함). batch-003의 HUMAN 재검토·수용·gold 동결·HELD 반복·FINAL 선정·학습은 하나도 열리지 않았다.
