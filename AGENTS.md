...
- 다음은 **EODHD 어댑터**(JSON → `bars_daily`, 무료 키의 최근 1년으로 검증)와 **7.2 증권 종류 필터**(`Type`·`Exchange`)다. 그다음 실데이터 1년으로 loop를 처음 돌려보고, 통과하면 EOD $19.99를 결제해 15년 가격을 받는다. **결제는 파이프라인이 끝까지 도는 것을 본 뒤에 한다.**
