...
- C1e 로컬 개발 상태: schema v8에 `notes.owner_agent`와 일정 월별 projection generation outbox를 추가했다. 활성 task DB는 별도 LLM 없이 단일 Claude·의회 공통 `<schedule>` 컨텍스트로 합성하고, 완료·취소는 일반 검색 가능한 월별 `schedule_history` 노트로 투영한다. 다시 열기·삭제·복원은 같은 월을 재생성하며 DB만 정본이다. 일정별 노트·저장 버튼·과거 일정 질문 분류기는 만들지 않았다. 전체 테스트 166/166을 통과했고 Pi에는 아직 배포하지 않았다.
