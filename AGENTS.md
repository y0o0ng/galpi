...
- 첨부 U0a 서버 기반은 로컬 구현 완료·Pi 미배포다. schema v12와 인증 임시 업로드·60분 orphan 정리가 있고 flag 기본값은 `false`다. 다음은 U0b 메시지 연결·`CONTEXT_N` snapshot·모델 호출 전 만료이며, library 승격은 명시적 승인 뒤에만 한다. 강의는 미구현이다.
