...
- composer는 위 입력·아래 도구의 두 줄 구조다. 보조 기능과 첨부는 `+` 메뉴에 두고, 오른쪽 주 액션은 빈 입력에서 시온 마크의 반이중 음성·입력값이 생기면 전송으로 전환한다. 활성 음성 루프의 종료 버튼은 입력값보다 우선한다. 음성 버튼의 시각 원은 데스크톱 36px·모바일 38px이고 실제 타깃은 40px·44px로 유지한다.
