...
- **Local Memory Inference Study Pilot P0와 P1-A는 `COMPLETE`이고, P1-B1 synthetic model-size screening은 ~4B까지 완료해 preregistered rule에 따라 automatic size progression을 중단했다. P1-B2a extraction decision-boundary diagnostic이 active next step이다.** production memory 판단·retrieval·model routing은 그대로이며 정본·결과는 `docs/Memory research/local-memory-inference-study-design.md`; private natural replay는 `UNOPENED`이고 별도 승인이 필요하다.
