...
- **다음은 미커버 구간 23개다. 이것이 `SURVIVORSHIP_BIASED`를 잡고 있는 마지막 항목이다.** (1) 분사인데 편입 기록이 표에 없어 floor부터 구성원으로 잡힌 6개(`PM`·`DPS`·`SNI`·`SW`·`CFN`·`DISCK`) — **가격이 아니라 재구성 문제**라 `CORRECTIONS`에 편입 행을 넣으면 된다. (2) `FOXA`·`FOX`는 2019-03-19 같은 날 인계라 `_drop_same_day_reuse`가 구간을 붙여놔 21세기폭스(`TFCFA`)를 가를 자리가 없다. (3) 나머지 10개는 벤더에 그 시절 계열이 없다(`MICC`는 OTC 계열뿐이라 쓰지 않았다). 그 뒤가 워크포워드와 14.7 판정이다.
