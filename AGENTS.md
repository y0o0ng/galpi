...
- C2 1차 보정 상태: 넷을 각각 고쳤다. 음량은 서버에서 조각마다 RMS 0.14를 목표로 맞추고 피크 0.97·이득 상한 6배로 클리핑과 잡음 증폭을 막는다. 재려면 조각 전체가 필요해 스트리밍 대신 버퍼링하며 조각이 짧아 비용이 작다. 끊김은 자르기 대신 음성 턴에서 결론을 먼저 말하게 하는 지시문으로 바꿨다. 앞 2~3문장만 들어도 답이 되게 하고 앞부분에 표·목록·제목 기호를 쓰지 않게 한다. 소리로 읽으면 기호가 그대로 읽힌다. 전사는 `gpt-transcribe`의 `prompt`에 `시온, 시온아, 갈피`만 좁게 넣었고 같은 오디오에서 `시오나` → `시온아`로 바뀌는 것을 확인했다. 문장을 넣으면 전사가 그쪽으로 끌려가므로 이름만 둔다. 로컬 회귀 323/323이다.
