...
- **ZERO에서 J126이 무너진다.** 총수익 +19.5%(J63) vs +4.7%(J126), 기대값 +0.040 vs **-0.841**로 순위가 둘 다 뒤집힌다. **폐지 건수 때문이 아니다** — J126은 폐지가 6건(J63 9건)인데 ZERO 손실이 -$21,643(J63 -$11,109)으로 건당 3배다. **이 민감도는 더 큰 달러 포지션과 관련돼 있고, `R/2ATR` 구조상 저변동 종목 선택과의 상호작용이 한 원인일 가능성이 있다. 다만 J126 진입 전체의 ATR·notional 분포는 재지 않았으므로 원인을 단정하지 않는다.**
...
- 다음 후보: **변환 구조 자체**(수량 `R/2ATR` · 청산 · 노출)가 다음 축이다 — 신호 층 우위가 K·슬롯·J에서 연속으로 번역되지 않았고, 개별 손잡이가 아니라 변환이 막는다는 가설이 쌓였다 · `jt-k42`에 비용 스트레스와 인접값을 돌려 blocker 둘을 없앤다 · `RX` 하나를 손으로 확정하면 민감도 아티팩트가 사라진다 · BULL-only 게이트와 레짐 조건부 J는 여전히 별도 PR이다.
