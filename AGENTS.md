...
- **벤더는 재사용된 티커의 옛 회사를 `_OLD` 아래 따로 갖고 있다.** `unlisted_symbols`가 0을 주는데도 구간이 비었던 이유다 — 현재 티커는 목록에 멀쩡히 있고 그 시절 회사는 `CEG_OLD`(CONSTELLATION ENERGY GROUP INC)·`DELL_OLD`·`EMC_OLD`처럼 접미사가 붙은 코드에 있다. 규칙 밖의 코드도 있다(`SUN → SUN1`, `NIHD → NIHDQ`, `FOXA → TFCFA`). 미커버 구간이 82 → **23개**가 됐다.
- **찾은 계열을 기본 심볼로 합치지 않는다. 다른 회사는 다른 심볼이다.** `CEG_OLD`는 2006~2012, 새 `CEG`는 2022~라 한 계열로 묶으면 10년 구멍과 가격 점프가 있는 하나가 되고 **새 회사의 첫 252세션이 옛 회사 바로 SMA·ATR·모멘텀을 계산한다.** 그래서 `universe_membership`의 그 구간 심볼 자체를 벤더 계열로 바꾼다. 구성원 수 불변식은 구간 단위라 깨지지 않는다.
- **매핑은 `universe/reused-tickers.csv`에 따로 둔다.** `universe/*.csv`는 S&P·Nasdaq 공고를 옮긴 공개 기록이라 벤더 코드가 섞이면 안 된다. `universe` 단계가 매번 그 매핑을 적용하고, `remap` 단계는 새로 찾은 것만 더하며 손으로 넣은 줄과 `vendor_name`의 근거를 보존한다. 이름·`CIK_OVERRIDES`는 지수 티커로 묶여 있으므로 EDGAR는 접미사를 떼고 찾는다(`MON_OLD`가 `MON`의 지정을 물려받는다). EDGAR는 CSV 티커가 아니라 **적재된 구성원 심볼**로 돌아야 `securities`·`earnings_calendar`가 멤버십과 같은 키를 갖는다.
- **남은 23개 구간은 셋으로 갈린다.** (1) 분사 회사인데 편입 기록이 표에 없어 floor부터 구성원으로 잡힌 것(`PM` 2008-03-17·`DPS` 2008-05-07·`SNI` 2008-06-12·`SW`·`CFN`·`DISCK`) — **가격이 아니라 재구성 문제**라 `CORRECTIONS`에 편입 행을 넣어야 한다. (2) `FOXA`·`FOX`는 2019-03-19 같은 날 티커 인계라 `_drop_same_day_reuse`가 구간을 하나로 붙여서 21세기폭스(`TFCFA`)와 폭스코퍼레이션을 가를 자리가 없다. (3) 나머지(`BEAM`·`DV`·`IACI`·`LIFE`·`LILA`·`LMCK`·`MI`·`SAI`·`SE`·`COV`)는 벤더에 그 시절 계열이 없다. `MICC`는 OTC 계열(`MIICF`)만 있어 NASDAQ 시세가 아니라 쓰지 않았다.
...
- **하나로 좁혀지지 않으면 고르지 않는다.** 후보 검증은 넷을 다 요구한다 — 제출 이력이 멤버십 구간과 겹칠 것, 8-K를 낼 것, SIC가 있을 것, **구간 안에 Item 2.02 실적 제출이 있을 것**. 마지막 것이 지주회사·중간 법인·신탁을 걸러낸다. 조용히 첫 번째를 고르면 `LEHMAN BROTHERS HOLDINGS`의 첫 매치가 신탁(`0001382976`)이라 자신 있게 틀린다. 최종 실측: 미해결 223 → **3개**(`FMCN`·`LMCA`·`LMCK`), 2층 48개·3층 127개·선행 병합 33개·`CIK_OVERRIDES` 36개, 실적 130,824행·섹터 901/907, 실적을 모르는 구간 4.3%.
...
- **`CIK_OVERRIDES` 36개는 전부 지수를 떠난 회사다.** 자동 경로가 못 찾는 것이 폐지 종목 쪽으로 기울어 있어서, 손으로 채우지 않으면 그대로 생존편향이 된다. 후보는 SEC 회사 검색(`browse-edgar?company=`)으로 찾고 두 가지로 확인했다 — `formerNames`의 날짜로 **그 멤버십 구간 동안** 그 CIK가 쓰던 이름, 그리고 구간 안의 Item 2.02 건수. 주석의 이름이 그 구간의 이름이고 그것이 근거다. 자동 검증을 통과하지 못하는 정당한 경우가 있다 — `YHOO`(Altaba가 펀드로 전환해 `sic`이 비었다)·`ACAS`가 그래서 명시 지정이 필요했다. 반대로 검색 1순위가 함정인 경우도 있다 — `MON`의 `0000067686`은 **옛** Monsanto(현 Pharmacia)이고, `ACAS`의 `AGNC Investment`는 그 회사가 운용하던 다른 회사다.
- **`AGN`·`GAS`는 한 구간 안에서 회사가 바뀌어 어느 CIK도 전체를 덮지 못한다.** 구간 시작을 덮는 쪽을 골랐고 나머지 절반은 실적을 모른 채 fail-close로 남는다(`AGN`은 2015-03-23부터의 Allergan plc, `GAS`는 2011-12-12부터의 AGL Resources).
- **실적일을 만들 수 없는 구조적 부류가 둘이다.** (1) 외국 발행사는 8-K가 아니라 20-F/6-K로 제출해 Item 2.02라는 개념이 없다(`ASML`·`AZN`·`BIDU`·`CHKP`·`INFY`·`JD`·`PDD`·`TCOM`·`VOD` 등 20종목, 전부 NDX100). (2) 주 인가 은행은 SEC가 아니라 **FDIC**에 제출한다(`FRC`·`SBNY`). CIK를 아무리 잘 찾아도 안 나오고 6-K는 Item 코드가 없어 제목 휴리스틱이 필요한데 그건 틀리면 조용히 틀린다. **이 제외는 생존편향과 성격이 다르다** — 결과를 낙관적으로 만들지 않고 기회를 좁힐 뿐이라 14.7 판정을 막을 이유가 없다. 유니버스에서 빼지도 않는다(NDX100 구성원 수 불변식이 깨진다). 실적 0건 종목 24개가 대부분 이 둘이다.
- **1층이 준 CIK가 구간 앞을 못 덮으면 선행 등록인을 찾아 합친다(바꾸지 않는다).** `XOM`·`GOOGL`·`DIS`·`MDT`처럼 지주회사로 재편되면 티커는 새 법인을 가리키고 그 법인에 과거 제출이 없다. 1층 CIK는 티커 기반이라 뒷부분은 그쪽이 맞으므로 버리지 않고 합친다. 실측 33종목이 붙어 `gaps`가 140 → 110개, 실적을 모르는 구간이 8.7% → 6.6%가 됐다.
- **선행 법인 판정은 `formerNames`의 날짜로 한다. "그 구간에 이 CIK의 이름이 무엇이었나"를 묻는다.** 이름이 언젠가 스쳤다는 이유로 고르면 엉뚱한 시대의 법인이 온다 — `DNB`의 후보 `0000030419`는 1994~1998에만 `DUN & BRADSTREET CORP`였고 2008년 구간에는 `R H DONNELLEY CORP`였다. `ADT`의 후보 `0000833444`도 `ADT LIMITED`는 1995~1997뿐이고 2012년 구간에는 `TYCO INTERNATIONAL LTD`였다. 둘 다 이름·SIC·제출이력·승계는 통과하고 이 검사에만 걸린다. 여기에 승계 검사를 더한다 — 선행이 후속 시작 뒤로도 400일 넘게 제출하면 재편이 아니라 별개 회사다(`MPC`의 후보 `MARATHON OIL CORP`는 13년을 더 냈다).
- **이 검사가 못 넘는 것은 어순이다.** SEC는 `DISNEY WALT CO`·`BARD C R INC`처럼 성을 앞세워 적는데 위키는 `The Walt Disney Company`·`CR Bard`로 적어서 정규형이 안 맞는다. 토큰을 정렬해 맞추면 매칭은 늘지만 무관한 회사가 섞이므로 하지 않았다. 남은 `gaps` 110개는 이 부류와 외국 발행사, 그리고 실제로 최근 상장한 회사가 섞여 있다.
