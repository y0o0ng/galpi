...
- **P1-B6 smoke calibration/reconciliation은 30 accepted / 2 dataset-rejected로 닫혔고, adaptive-growth batch-002 64건과 source-audit attempt-001 packet이 준비됐다.** audit은 아직 실행하지 않았고 accepted pool은 30 그대로다. 다음은 fresh strong-model session에서 batch-002 source-audit attempt 001을 실행하는 것이며 final gold는 동결되지 않았고 training·raw episode·private replay는 `UNOPENED`, production은 그대로다. 정본은 `docs/Memory research/local-memory-inference/local-memory-inference-p1b6-design.md`와 run receipt다.
