...
- 실제 변경 전 무엇을 왜 바꾸는지, 변경 범위·영향·트레이드오프·검증 계획을 짧게 사전등록하고 그 범위를 한 번 컨펌받는다.
- 승인된 범위 안의 편집·테스트·`main` 확인·fetch/pull·스테이징·커밋·푸시는 추가 컨펌 없이 진행한다. 의미 계약 변경, 범위 확대, 파괴적 작업처럼 결과를 바꾸는 새 선택만 다시 컨펌받는다.
...
- **P1-B6 batch-002 수리·재조정 주기는 닫혔다 — 수용 63 accepted / 1 rejected / 0 unresolved, 누적 accepted surface pool은 93이다.** 정본은 `fixtures/local-memory-inference-p1b6-batch-002-acceptance.json`, 63행 후속본은 `…-surface-effective-current-batch-002.json`(51 상속 + 12 수리, `050` 없음)과 `…-primary-human-accepted-current-batch-002.json`(**63 KEEP / 39 CLEAR / 24 ESCALATE**)이다. 재조정은 effective-current 카탈로그 기준 **63 match / 0 mismatch**다.
- **앞으로의 authoring이 읽는 의미 권위는 historical Exact56(불변, `772f07…`)이 아니라 `fixtures/local-memory-inference-p1b6-skeleton-effective-current.json`(34 CLEAR / 22 ESCALATE)이다.**
- **`CONSERVATIVE_PRAGMATIC_INTERPRETATION`은 앞으로의 판단에도 계속 적용된다: 경쟁 해석은 보이는 근거가 적극적으로 licensing할 때만 인정한다.** 논리적으로 양립 가능하다는 것만으로는 모호성이 유지되지 않는다.
...
- **batch-003 adaptive growth의 계획·프로토콜·검증기는 고정돼 있다.** 수용 seed 93에서 부족분 287, 트랜치 304(버퍼 17), split 195/47/62 · label 141/163 · language 213/61/30 · fragment 55/78/99/56/16 · discourse 각 38. HELD는 필수 59(45+14)에 버퍼 +2/+1로 47/15다. 계획 정본은 `fixtures/local-memory-inference-p1b6-surface-batch-003-authoring-protocol.json`, 도출기는 `scripts/build-memory-inference-p1b6-batch-003-authoring-plan.js`다.
...
- **batch-003 표면 304건 authoring이 끝났다.** 정본은 `fixtures/local-memory-inference-p1b6-surface-batch-003.json`(304 item / 304 episode), 원문은 `scripts/data/memory-inference-p1b6-batch-003-content.js`이고 오프셋·ID는 materializer가 기계적으로 만든다. 고정된 계획·프로토콜은 안 고쳤고, 모든 marginal과 skeleton별 배정이 계획과 정확히 일치한다. 무커버 skeleton `aebbf047…`은 신규 실현 5건을 받았다(`050` 수리 아님).
- **첫 materialization은 구조적으로는 맞았지만 게이트 전 의미 구성 검토에서 ESCALATE 표면 붕괴가 나왔고, 감사 전에 53건을 재authoring했다.** 보이는 근거만으로 두 지위가 licensing되지 않고 하나로 풀리던 것들이다 — skeleton 8개(`135ab779`·`2da4e54e`·`38c426bb`·`5229ea23`·`59c8f518`·`a19bb9e9`·`be0efa30`·`cc054a42`). **이건 authoring 결함이지 HUMAN 재라벨도 감사 판정도 아니다.** 수리는 원문·선택 근거·anchor만 바꾸고 slot 계약(skeleton·split·ID·family·언어·담화 패턴·fragment 수)은 전부 보존하며 오프셋은 기계로 다시 계산한다. 붕괴 유형별 수리는 receipts 문서의 표가 정본이다.
- **두 번째 검토에서 잔여 15건을 감사 전에 또 고쳤다.** `be0efa30` 6건은 두 규칙이 **겹치지 않고 직교**해서(TV 소등 vs 거실 조명, 말하는 언어 vs 문서 언어, 간식 횟수 vs 간식 시각, 온도 vs 습도, 배송 위치 vs 수령 확인, 시간 vs 강도) 그냥 공존할 뿐 대상 해석을 못 바꿨고, `cc054a42` 7건은 정렬 표현이 보고 전체를 술어로 받아 전체 채택을 풀어 버렸다. `079`·`147`도 같이 손봤다. 고친 방향은 **같은 차원 위의 교차(nesting 아님) 범주**, **어휘로 범위가 고정되지 않는 행위·결정 신호**, 예외 안팎을 각각 지지하는 두 속성, 하나의 열거절 안에 나란히 놓인 두 지시 대상이다. **나머지 38건은 문체 정리 목적으로 다시 열지 않는다.**
...
- **대규모 배치의 전수 HUMAN 검토는 명시적으로 폐기됐다.** 저장소 주인의 결정이고 정본은 `fixtures/local-memory-inference-p1b6-large-batch-review-authority-amendment.json`이다. batch-003부터 **전향적으로만** 적용되고 batch-001·002의 HUMAN 이력은 그대로 유효·불변이다. **검토 안 된 일치 행은 HUMAN gold가 아니다**(`CATALOG_STRONG_MODEL_CONFIRMED`).
- **현재 의미 권위는 semantic contract v3다 — `fixtures/local-memory-inference-p1b6-skeleton-effective-current-v3.json`, 42 CLEAR / 14 ESCALATE.** v2(45/11)·v1(34/22)·Exact56(32/24)은 덮어쓰지 않은 역사 계보이고 v3는 **v2의 정확한 원본 바이트에서** 파생된다. 계약 핵심: **주어진 불확실한 상태는 CLEAR, 추가 정보 없이는 '모른다'로만 답할 수 있는 TARGET 상태는 ESCALATE**(v2의 `unknownIsNotAmbiguity`를 대체), **TARGET 자체를 판정하고 하류 추론(자격·원인·지속성)은 보지 않는다**, **전제를 보태야 성립하는 경쟁 읽기는 인정하지 않는다**(유지). v2 개정 중 `2da4e54e`·`5269c91f`·`b8e64a03`을 ESCALATE로 되돌렸고, 실현 불가능한 `f58debd8`·`28736b74`는 은퇴시켜 같은 자리에 새 불투명 ID `53ab6351`·`0768ea20`(TRAIN/APPROXIMATION·RANGE/ESCALATE)을 넣었다 — **옛 표면·HUMAN 판정은 넘어가지 않는다.** mixed skeleton은 `5fc872af`·`be0efa30` 둘이 남았다. `aebbf047`·`869c7127`의 CLEAR는 그대로이고 최종 수용 전 anchor 품질 후속 작업이 따로 있다. 정본은 P1-B6 설계의 `Semantic contract v3` 절이다.
- **최종 코퍼스의 `190 CLEAR / 190 ESCALATE`와 split별 라벨 하한은 전향적으로 폐기됐고 새 비율로 대체하지 않는다.** 라벨 수는 이제 의미적으로 타당한 표면들의 **결과**이지 authoring·선정의 목표가 아니다. 380 총량·240/60/80 split·언어·fragment·감사·split 격리는 그대로다. held-out ESCALATE가 적은 것은 **보고할 측정력 한계**이지 재라벨로 메울 대상이 아니다.
- **301행 strong-model 의미 검토는 외부에서 실행됐고 원본 결과 바이트로 v2에 대해 reconcile했다 — 267 일치 / 34 HUMAN 라우팅.** 영수증은 `…-strong-model-semantic-review-batch-003-attempt-001.json`이고 원본 아티팩트 SHA로 묶여 있다(아티팩트 자체는 커밋 안 함, 리뷰어 모델 정보는 없어서 안 지어냄). **이미 발행된 301행 blind packet 바이트는 그대로다** — packet 출처와 현재 reconcile 권위는 별개다. 34건 중 20건이 v2의 네 mixed skeleton에 몰렸다.
- **66행 blind HUMAN 판정·보정(attempt-001)과 그 26건 해결은 닫혔다.** 정본은 P1-B6 설계의 `HUMAN adjudication/calibration result — attempt 001`·`Batch-003 26-row resolution` 절, 해결 영수증은 `fixtures/local-memory-inference-p1b6-batch-003-resolution-receipt.json`이다. **보정 일치는 `HUMAN_ADJUDICATED`로 승격하지 않고 표본 밖 235건으로 외삽하지 않는다.** FINAL_HELD_OUT은 반복 HUMAN 검토 대신 선정 후 독립 2차 strong-model 검토를 받는다.
- **다음은 수용이 아니라 새 게이트다.** ① 수리 후보 `162`·`214`(`fixtures/local-memory-inference-p1b6-surface-repair-candidate-batch-003.json`)의 새 source audit과 v3 기준 새 blind HUMAN 검토 — source audit은 **2/2 PASS로 끝났고**(`fixtures/local-memory-inference-p1b6-batch-003-repair-source-audit-attempt-001.json`, packet `b32f57a4…`) v3 전용 HUMAN 프로토콜(`fixtures/local-memory-inference-p1b6-batch-003-repair-human-review-protocol.json`, `bb7c79d4…`)과 2행 blind 패킷(`2d58d919…`, 커밋 안 함)으로 **2026-09-23 HUMAN 검토를 마쳤다 — 2 KEEP·ESCALATE, v3 reference와 2/2 일치**(`fixtures/local-memory-inference-p1b6-batch-003-repair-human-review-attempt-001.json`). 저장소 주인이 수리 해결에 참여했으므로 독립 확인은 아니고 **수용은 아직 없다**, ② 대체 skeleton 두 개의 새 표면 authoring과 그 감사·검토, ③ 되돌린 세 skeleton 위의 나머지 batch-003 표면 24건(`073`·`076`·`143`·`144`의 v2 기준 `HUMAN_ADJUDICATED` CLEAR 포함)을 v3에 맞춰 다시 reconcile하는 것 — 계획(`fixtures/local-memory-inference-p1b6-batch-003-v3-rereconciliation-plan.json`, 내부용)과 중립 v3 strong-model 프로토콜(`…-targeted-v3-strong-model-review-protocol.json`)을 사전등록했고 24행 blind 패킷(`665755e2…`, 커밋 안 함)의 **외부 strong-model 검토를 마쳤다 — 19 일치 / 5 HUMAN**(`075`·`077`·`080`·`082`·`143`, 전부 모델 CLEAR vs v3 ESCALATE), calibration 3(`076`·`144`·`235`). 영수증은 `fixtures/local-memory-inference-p1b6-targeted-v3-strong-model-review-batch-003-attempt-001.json`이다. **8행 blind HUMAN 검토(2026-09-23)는 7 FIX / 1 KEEP ESCALATE였고**(`…-batch-003-targeted-v3-human-review-attempt-001.json`), **`2da4e54e`·`5269c91f`는 TARGET 경계 실현 실패로 닫았다** — 계약 실패가 아니라 표면이 TARGET을 판단 근거 사실에 둔 실패라 v3·두 skeleton은 그대로 active이고, batch-003의 두 skeleton 표면 16건(12+4)이 전부 현재 ineligible이다(`fixtures/local-memory-inference-p1b6-batch-003-target-boundary-resolution-receipt.json`, 역사 증거 불변). **두 skeleton의 새 표면은 TARGET을 적용 상태·범주 소속·규칙 결과 자체에 둬야 한다**(불변식은 선언된 `targetAnchorRole`만 확인). 실현 가능성 증명 후보 `p1b6-item-tb1-001`·`-002`(`…-surface-target-boundary-candidate.json`)는 새 source audit을 **2/2 PASS**로 통과했고(`fixtures/local-memory-inference-p1b6-target-boundary-source-audit-attempt-001.json`) 외부 v3 검토와 HUMAN 검토(2026-09-26)에서 **둘 다 KEEP ESCALATE로 v3와 일치**해 두 skeleton이 TARGET을 상태 자체에 두면 실현 가능함을 보였다(`…-target-boundary-human-review-attempt-001.json`). 둘 다 calibration 행이라 `CATALOG_STRONG_MODEL_CONFIRMED`/`PROVISIONAL`에 머물고 `HUMAN_ADJUDICATED`는 없으며 독립 확인도 아니다, 수량은 1:1 대체가 아니라 top-up이 정한다. source audit은 바이트 불변·기계적 PASS 확인 뒤에만 물려받았고 v2 의미·HUMAN 판정은 v3 판정으로 넘기지 않는다. `be0efa30` 거절 5건(`242`·`244`·`246`·`248`·`249`)은 현재 표면만 거절했고 대체 표면은 빚지지 않는다.
