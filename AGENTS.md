...
- 다음 개발 시작점: C2 LLM 스트리밍이다. `/api/chat`의 progress stream이 단계 이벤트만 보내고 부분 답변 텍스트를 보내지 않으므로 모델 호출부를 스트리밍으로 바꿔야 실제 약 1,000ms가 준다. 텍스트 채팅과 공유하는 경로라 회귀 위험이 있다. H3 되묻기는 실제 오전사 표본이 쌓인 뒤로 미룬다. 음성 턴이 텍스트 답변과 겹칠 때의 `isLoading` 거절 경로는 일부러 만들기 애매해 계속 미검증으로 남을 수 있다. 그 뒤 `docs/voice-halfduplex-design.md`의 H3 검증·되묻기 → H5 Realtime 정리다. 음성 턴이 텍스트 답변과 겹칠 때의 `isLoading` 거절 경로는 실기기 검증 전이다. 지연 개선은 스트리밍 TTS → 침묵 1200→1000ms → earcon 순서로 아직 미구현이다. 지연 개선은 스트리밍 TTS(약 1,000ms) → 침묵 1200→1000ms(200ms) → earcon 순서이며 아직 미구현이다. 폰↔Pi 왕복 3회를 1회로 줄이는 통합은 설계만 남겼고 스트리밍 TTS 구현 시 함께 검토한다. 더 빠른 텍스트 모델로 내리는 선택은 하지 않는다.
