...
- H0 지연 스파이크 결과: 2026-08-01 Pi 실측 median으로 STT 674ms, LLM 첫 바이트 372ms, LLM 전체 1797ms, TTS 첫 오디오 545ms, TTS 전체 1627ms다. 말 끝나고 첫 소리까지는 직렬 4777ms, 스트리밍 3333ms이며 폰↔Pi 왕복·재생 시작·첫 문장 대기가 빠져 있어 현실 보정치는 4.0~4.5초다. 병목은 API가 아니라 침묵 감지로, 1.8초 침묵이 스트리밍 총합의 54%를 차지한다. 따라서 침묵 임계값을 1200ms(되묻기 응답 800ms)로 낮추고 스트리밍 TTS를 필수로 둔다. 스파이크 코드는 `/home/pi/latency-spike.js`이며 폐기 대상이다.
- 다음 개발 시작점: `docs/voice-halfduplex-design.md`의 H1 핵심 루프부터 구현한다. 되묻기·저장 없이 LISTENING→CAPTURING→TRANSCRIBING→THINKING→SPEAKING→COOLDOWN만 돌리고, 5턴 연속 대화에서 마이크 잠금·에코 재인식·상태 고착 0회와 첫 소리까지 5초 이내를 통과 기준으로 한다. 그 뒤 H2 저장 → H3 검증·되묻기 → H4 일정·메모 확인 카드 → H5 Realtime 정리 순서다. 턴 폐기는 신규 구현하지 않고 `isPersistableUserTurn`을 재사용하며, `voice-turn-recorder.js`·보정 전사 라우트·`dbSaveChatExchange`도 그대로 쓴다. R2c-1의 `realtime_turn_receipts`는 전사가 답변보다 먼저 확정되므로 불필요해지지만 schema v10은 무해하므로 제거하지 않고 미사용으로 둔다. 음성 출처 턴은 topic 자동 저장에서 제외한다. 구현 전 열린 결정은 Realtime 코드 제거 범위, TTS voice, flag 이름(`VOICE_HALFDUPLEX_ENABLED` 제안), 빈 전사 25% 원인이다. 헛기침 거리 의존성 원인 판별(`near_field`·`far_field`·비활성 동일 거리 비교)은 R2c와 독립된 별도 표본으로 다루고, `server_vad` threshold와 `interrupt_response:false`, 자체 sound classifier는 그 표본 뒤 별도 컨펌으로만 연다. R2c 전에는 R2b 보정본도 휘발 상태이며 message·topic·task write는 계속 0회다. R1의 기존 A2 기억 5개·무관 질문 4개 정량 parity는 별도 운영 표본으로 남아 있고, mini 보정 뒤에도 반복적으로 부족할 때만 full `gpt-realtime-2.1`을 검토한다. 새 `chat:gpt-single-v1:a2` 실제 질문의 과회수·최신성·abstention 관찰과 V4.5-C 잠금화면 Web Push 10회 표시 기준도 계속 채운다. 유지보수·평가 세션이 자동 topic 저장되지 않는 명시적 경계는 별도 후속이다. 제품 승격 순서는 V4.5→V4-B 음성→V5-A 딜 스카우트→V5-B 주식 분석을 유지한다. 강의 Phase 0은 병행 가능하고 전체 구현은 V5-B 전 또는 V5-B PAPER 관찰 기간에 진행할 수 있다. 단, 딜 스카우트 Phase -1과 게시·LLM·갈피 DB/vault 접근이 없는 Phase 0은 별도 컨펌과 공식 API 키 또는 승인 전 예외 검토 뒤 격리해 병행할 수 있다. 논문 10+10 품질 평가는 별도 컨펌 전이며 Pi `.env`는 `PAPER_SEARCH_MOCK=false`, `npm audit` 기존 경고는 자동 수정하지 않는다.
