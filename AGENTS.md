...
- **9.2 하드 한도는 아직 집행하지 않는다.** 동시보유·섹터·총 계획위험·60일 상관·일일/주간 손실·낙폭 축소는 다음 리스크 엔진 단계다. 크기 계산만으로 9.2를 지킨다고 오해하면 안 된다. 섹터 한도와 7.2 증권 종류 필터는 종목 분류 데이터가 필요해 그 단계에서 `securities` 표와 함께 한다.
- 다음은 리스크 엔진과 PolicyVersion(9.2) → 체결·비용(10.1·10.4) → 청산(7.5) → 성과 지표(14.7)다. 미체결은 일봉에서 재산정을 모델링할 수 없으므로 취소하고 `NO_FILL` 건수만 기록한다.
