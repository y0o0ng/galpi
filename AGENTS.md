...
- 트레이딩 전략 구축 계약: 전략 family마다 로드맵이 하나다. **연구 예산·종료 조건·Phase별 사전등록이 거기 있다.** 실험 산출물 색인은 `trading/runs/README.md`다.
  ...
  - `quality-value` (Phase 0 진행 중 — Step 4 PIT shares/ME까지 구현했고 다음은 Step 5다): `docs/trading/strategies/quality-value-roadmap.md`. **Step 4 구현 정본은 `docs/trading/strategies/qv-step4-shares-me-design.md`이고 P2 same-regime selector·C3 continuity·identity manifest가 거기서 얼어붙었다.**
    ...
    - **표지 제목은 filing 관측이지 자동으로 production prose alias가 아니고(B1), 탄생 증거만으로는 `effective_to = null`이 되지 않는다(B2).** 둘 다 사용자 결정으로 CLOSED다. 5A-2 법적 증거 공급기(`backtest/qv_identity_legal_evidence.py`)가 SEC governing instrument에서 **명시 사실만** 읽고, 자동 class 연결의 anchor는 `N1(표지 제목) == N1(governing class 이름)` 하나다. 생성기와 승격기가 `class_evidence_from_legal_proof` **같은 함수**를 쓴다. 계약은 `docs/trading/strategies/qv-step5-phase0-materialization-design.md`의 5A-2 절이다.
...
- **Local Memory Inference Study P1-B2c는 `CLOSED / COMPLETE / FAIL_FRESH_SYNTHETIC_VALIDATION`이고 P1-B2d HUMAN gold는 5/5/5로 동결됐다. 첫 P1-B2d 시도는 endpoint unavailable로 model output이 0개였던 infrastructure-invalid 실행이며, runner는 fail-close readiness preflight를 갖췄고 valid run은 아직 pending이다.** fresh-validated triage contract 전까지 composition은 막혀 있고 later raw-context 평가는 별도·deferred이며 production memory 판단·retrieval·model routing은 그대로다. private natural replay는 `UNOPENED`이고 정본은 `docs/Memory research/local-memory-inference-study-design.md`다.
