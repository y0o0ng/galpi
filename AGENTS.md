...
- H6 잠금화면 단축어 입구는 H6a 서버 단일 턴까지 로컬 구현했다. schema v11의 활성 Push 구독 연계 hash-only credential·durable receipt와 exact route가 공용 턴 코어·`shared-main`을 쓰며, topic 자동 저장과 일정·노트·Codex 쓰기를 막는다. flag 기본값은 `false`이고 Pi·iPhone 인수 전이다.
...
- 다음 음성 작업은 H6a Pi 배포·활성 iPhone credential 발급 뒤 H6b 잠금화면 단일 턴 10회 인수다. 이를 통과한 뒤에만 bounded 3턴을 연다. 침묵 1200→1000ms, earcon, H3 되묻기는 각각 별도 변경이며 더 빠른 텍스트 모델로 내리지 않는다.
