...
- **P1-B6 batch-002 24건 mismatch의 item별 semantic adjudication은 끝났다 — 10 `SKELETON_SEMANTICS_NEEDS_REVISION` / 11 `SURFACE_COLLAPSES_AMBIGUITY` / 3 `HUMAN_DECISION_NEEDS_REREVIEW` / 0 `UNRESOLVED`.** 정본은 `fixtures/local-memory-inference-p1b6-pragmatic-adjudication-batch-002-receipt.json`이고 행은 `itemId`+`outcome`뿐이다. **adjudication은 라우팅이지 HUMAN 재라벨링이 아니다** — HUMAN 결정·frozen Exact56·surface는 하나도 안 바뀌었고 reconciliation은 여전히 `RECONCILIATION_NEEDS_FIX`, `humanReviewCompleted`는 false다.
...
- **수리본 12건의 새 source audit은 `COMPLETE_PASS` 12/12로 끝났다.** 정본은 `fixtures/local-memory-inference-p1b6-surface-repair-source-audit-batch-002-attempt-001.json`이고, 원시 결과 파일은 관례대로 커밋하지 않되 receipt가 파일명과 SHA로 고정한다. **모델·추론 설정은 결과 파일에서 복구되지 않아 추측하지 않고 `NOT_RECOVERABLE_FROM_RESULT_ARTIFACT`로 적었다.** 옛 batch-002 `PASS`는 하나도 승계되지 않았다.
- **batch-002 수리·재조정 주기는 닫혔다 — 수용 63 accepted / 1 rejected / 0 unresolved, 누적 accepted pool은 93이다.** 정본은 `fixtures/local-memory-inference-p1b6-batch-002-acceptance.json`이고 63행 후속본은 `…-surface-effective-current-batch-002.json`·`…-primary-human-accepted-current-batch-002.json`이다. 계약은 설계 문서의 `Batch-002 Finalization and Acceptance` 절이다.
- **후속본은 51 상속 + 12 수리이고 `050`은 없다.** 상속 행은 역사 64행 배치에서, 수리 행은 수리 후보에서 각각 바이트 그대로 온다. **옛 HUMAN 결정은 수리된 원문으로 절대 넘어가지 않는다**(빌더가 행마다 `humanDecisionSource`로 구조적으로 강제). 재조정은 Exact56이 아니라 effective-current 카탈로그 기준 **63 match / 0 mismatch**이고, 효력 HUMAN은 **63 KEEP / 39 CLEAR / 24 ESCALATE**다.
- **`050`은 대체본이 없다.** 현재 표면 실현만 폐기됐고 skeleton은 그대로다. 커버리지 슬롯이 필요해지면 별도 신규 authoring 작업이다.
- **수리본 HUMAN 재검토는 리뷰어 독립적 확인이 아니다.** 리뷰어가 제시된 모집단 전체가 수리본이고 목적이 모호성 복원임을 알았다. 또 receipt의 `reviewerAuthoredTheRepairs: true`는 부정확한 기록이라 — 소유자는 최종 판단을 했지만 12건 표면 원문을 직접 쓰지는 않았다 — `…-provenance-correction.json`에 좁게 정정했다. **역사 receipt 바이트와 12건 판정은 그대로다. 정정은 독립성을 만들지 않는다.**
- **다음 단계는 아직 하나도 열리지 않았다.** 최종 corpus HUMAN gold 동결, 반복 HELD 검토, FINAL_HELD_OUT 공개, 결정론적 FINAL 선정, 학습 전부 UNOPENED다. **380항목 P1-B6 corpus는 완료가 아니고, 구축은 accepted pool 93에서 이어진다.**
