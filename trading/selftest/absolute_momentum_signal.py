...
쓴다. 날짜마다 유효 관측 수가 달라 둘이 정확히 같지 않은 것은 **오류가 아니다.**
...
def _checksum(primary: dict) -> list[str]:
    return [
        ...
        "",
        "**두 값이 다른 것은 오류가 아니다.** 날짜마다 유효 관측 수가 달라서다 — 관측 가중은"
        " 관측이 적은 날짜에 가중치를 덜 주고 날짜 가중은 모든 날짜를 같게 본다.",
        "",
    ]
...
def _binding(primary: dict) -> list[str]:
    ...
    if binding["label"] == LABEL_NON_BINDING:
        lines += [
            ...
            "```",
            "",
            "이고, `SPY > SMA200`인 날짜에는 **시장의 126일 모멘텀 자체가 대개 양수**다."
            " 그러면 RS 상위 종목은 `RS > 0`과 `ABS(시장) > 0`이 겹쳐 **ABS도 필연적으로"
            " 양수**가 된다.",
            "",
            ...
            "",
            "**TOP5의 ABS가 0에 가까이 간 적조차 없다** — 최솟값이 +0.19다. 그래서 문턱을"
            " 0 근처에서 조금 올려도 결과가 달라지지 않는다. **다만 그것을 확인하려고 문턱"
            " 사다리를 돌리지 않는다** — 사전등록이 금지한 것이고, 이 표만으로도 `ABS > 0`이"
            " 이 구조에서 **구속력이 없는 조건**이라는 것이 드러난다.",
            "",
