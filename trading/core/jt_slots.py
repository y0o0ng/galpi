...
> **이 파일의 "거래당 위험"은 sizing 단위다.** 이 코어들도 `FIXED_HOLD`라 손절을 집행하지
> 않으므로 그 퍼센트는 거래당 실제 최대손실이 아니다 — 이유는 `jt.py`의 "0.25%는 집행되는
> 손절 위험이 아니다"에 있다. **표의 수치와 처치의 불변식은 그대로다**(PR #19에서 표를
> 다시 쓰지 않았다. 이 실험의 불변식은 `슬롯 × 거래당 예산 = 1.25%`라는 관계이고 그 관계는
> 이름과 무관하게 성립한다).
