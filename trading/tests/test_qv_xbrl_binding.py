...
def document(facts, *, cik=CIK):
    return parse_instance(cover_instance(facts, default_cik=cik), "cover.xml")
...
class BindingFixture:
    ...
    def derive(self, facts, *, accession="0000000042-24-000001", cik=CIK,
               instant=INSTANT, filing_usable=FILING_USABLE):
        return derive_bindings(
            self.connection, document(facts, cik=cik),
            cik=cik, accession=accession, instance_document_name="cover.xml",
            issuer_id=ISSUER, identity_source_version=IDV,
            filing_historical_usable_session=filing_usable,
            default_instant=instant,
        )

    def store(self, bindings):
        return store_bindings(
            self.connection, bindings,
            filing_source_version=FSV, identity_source_version=IDV,
            instance_sha256=SHA,
        )

    def lookup(self, accession, member_local="CommonClassAMember", **kwargs):
...
class FilingLocalityTest(BindingFixture, unittest.TestCase):
    ...
    def test_the_same_accession_cannot_bind_one_qname_to_two_classes(self):
        self.klass("cls-a", "AAA")
        self.klass("cls-x", "XXX")
        self.prose("cls-a", "class a common stock")
        self.prose("cls-x", "reorganized common stock")
        ...
        self.store(bound)
        # 같은 자연키에 다른 class를 넣으면 조회가 그것을 그대로 드러낸다.
        self.connection.execute(
            "UPDATE qv_xbrl_class_bindings SET class_id = 'cls-x'"
            " WHERE accession = '0000000042-20-000001'"
        )
        self.connection.commit()
        self.assertEqual(self.lookup("0000000042-20-000001"), ("cls-x", RESOLVED))
        del clash
...
class StorageTest(BindingFixture, unittest.TestCase):
    def test_the_binding_row_keeps_reproducible_provenance(self):
        ...
        self.assertEqual(row["instance_sha256"], SHA)
    ...
    def test_an_ambiguous_stored_binding_is_reported_not_guessed(self):
        self.klass("cls-a", "AAA")
        self.prose("cls-a", "class a common stock")
        bound, _ = self.derive(
            title_facts("CommonClassAMember", "Class A Common Stock", "AAA")
        ...
        self.store(bound)
        # 같은 자연키의 다른 instance 문서가 다른 class를 가리키는 상태를 만든다.
        self.klass("cls-x", None, listed=False)
        self.connection.execute(
            "INSERT INTO qv_xbrl_class_bindings"
            " SELECT cik, accession, 'other.xml', axis_key, member_key,"
            "  filing_source_version, identity_source_version, issuer_id, 'cls-x',"
        ...
        self.connection.commit()
        self.assertEqual(self.lookup("0000000042-24-000001"), (None, AMBIGUOUS))
