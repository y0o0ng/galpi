...
상세는 `results.md`이고 여기에는 결론만 둔다.

**재현은 정확했다.** J63 514거래·+35.39%·0.229R·MDD 9.4%, J126 512거래·+35.63%·0.233R·
MDD 10.8%로 PR #13과 네 항목 모두 일치했다. 신호 층도 TOP5 +42 초과수익이 J63 +0.484% ·
J126 +1.032%로 PR #12의 +0.48% · +1.03%를 그대로 냈다.

**퍼널.** TOP5 22,135 → ACCEPTED 554/560(2.5%) → FILLED 519/517 → CLOSED 514/512.

|층|단위|J63|J126|J126 − J63|
|---|---|---:|---:|---:|
|TOP5 초과|유니버스 대비 %|+0.48%|+1.03%|+0.55%p|
|ACCEPTED|″|+0.23%|+0.58%|+0.35%p|
|FILLED|″|**−0.20%**|+0.53%|+0.73%p|
|raw 거래 수익률|raw %|+0.89%|+1.80%|+0.91%p|
|net-notional|notional %|+1.23%|+2.02%|+0.78%p|
|실현 R|R|0.229|0.233|**0.003**|
|거래당 달러|$|$63|$69|**$6**|

**Q1 `NO_CLEAR_STAGE` · Q2 `INCONCLUSIVE`.** 모든 층에서 부호가 유지됐고 사전등록 규칙은
부호만 보기 때문이다.

**사전등록 규칙의 한계를 그대로 남긴다.** PATTERN S가 요구하는 "R 또는 달러에서 축소·역전"을
`classify_pattern`은 **역전(`≤ 0`)으로만** 구현했다. 이번 데이터가 정확히 그 틈에 떨어졌다 —
raw 층 +0.91%p가 R 층 0.003 · 달러 층 $6로 **부호는 유지한 채 크기만 무너진다.** 결과를 본 뒤
문턱을 고치면 사전등록이 아무것도 보증하지 못하므로 규칙을 바꾸지 않았고, 크기 변화는
`results.md` §20에 탐색적 진단으로 분리했다. **`NO_CLEAR_STAGE`를 "층 사이에 아무 일도
없었다"로 읽지 않는다.**

**J63/J126의 전체 sizing 분포 차이는 사전등록한 5% median 기준에서 지지되지 않았다.**
정규화 ATR 중앙이 −3.0%, 진입 notional 비중 중앙이 +4.1%로 둘 다 문턱 미만이다(기준 G·H).
**문턱 미달을 "차이가 없다"의 증명으로 읽지 않는다** — 중앙값 하나로 재는 기준이라 꼬리에
있는 차이는 애초에 잡지 못한다. 다만 이 결과로 "J126이 저변동 종목을 골라 더 큰 포지션이
된다"를 **사실처럼 쓸 근거는 없다.**

**data-exit의 위치 구조는 두 J 공통이지만 ZERO 손실 규모는 설명하지 못한다.** 폐지·정체불명
거래가 low-vol · high-notional 꼬리에 위치하는 구조는 J63과 J126 **모두에서** 관찰됐다
(진입 notional 비중 97~98백분위 · 정규화 ATR 1~3백분위). 따라서 그 구조 자체는 J126 고유
현상이 아니다. **다만 J126의 더 큰 ZERO 손실 규모(−$21,643 vs −$11,109, 6건 vs 9건이라
건당 약 3배)는 이 진단만으로 설명되지 않는다** — 위치가 같아도 건당 손실 크기가 왜 다른지는
재지 않았고, 표본이 한 자릿수라 이 진단으로 답할 수 있는 질문도 아니다.

**구속 제약은 `RISK`다** — J63 96% · J126 97%. 그래서 `R/2ATR`가 실제로 수량을 정한다고 말할
수 있고, `corr(정규화 ATR, 진입 notional 비중)` Spearman이 −0.999 · −0.998로 현재 sizing은
실측상 거의 순수한 inverse-normalized-vol 가중이다.

**allocation × return 정렬 진단(사전등록 아님, `results.md` §20.4).** 결과를 보고 추가했고
**공식 `NO_CLEAR_STAGE / INCONCLUSIVE`를 바꾸지 않는다.** 기존 LAST_CLOSE trace의 체결 거래를
다시 집계할 뿐 새 실행·새 코어·수량 변경이 없다. 물은 것은 "높은 퍼센트 수익률을 낸 거래일수록
실제 포지션 비중이 작았는가"다.

|값|J63|J126|
|---|---:|---:|
|ρ(비중, net 수익률)|+0.058|+0.003|
|ρ(정규화 ATR, net 수익률)|−0.057|−0.005|
|동일가중 net 수익|+1.23%|+2.02%|
|배분가중 net 수익|+1.48%|+1.61%|
|정렬 격차|**+0.24%**|**−0.41%**|

J126 − J63 우위는 동일가중 **+0.78%p** → 배분가중 **+0.13%p**로 83%가 지워지고, 절대값 상위
1%를 뺀 표본에서는 +0.29%p → **−0.28%p**로 부호까지 바뀐다. trim은 **위·아래 각각 1%가 아니라
절대값이 큰 전체 1%**라 양·음 어느 쪽이든 극단값이 후보가 된다.

**판정 X3 (mixed · INCONCLUSIVE).** 순위 상관은 거의 0이고 부호가 희석 가설과 **반대**인데
배분가중은 우위를 실제로 압축한다. 두 지표가 어긋나는 이유는 Evidence 5에 있다 — 수익률
양쪽 꼬리가 중간보다 **비중이 낮고 정규화 ATR이 높아** 관계가 단조가 아니라 U자다. Spearman은
단조만 보고 가중평균은 크기를 본다. **한쪽만 인용해 결론을 만들지 않는다.** trim이 방향을
바꾼다는 것 자체가 소수 거래에 기댄 관계라는 신호다. **sizing intervention 근거가 부족하다.**

**탐색적 관찰(사전등록 아님).** `R ≈ net_notional_return / stop_fraction`이 실측으로
성립하고(J63 0.231 vs 실제 0.229), J126/J63 배율이 raw 2.03× → net 1.64× → **R 1.02×**로
떨어지며, J126의 raw 상위 10거래 `stop_fraction` 중앙이 12.40%로 전체 6.90%의 1.8배다.
**이것은 다음 PR에서 따로 사전등록할 가설이지 이 PR의 결론이 아니다** — 이 구조가 실제로
변환 손실을 만드는지는 수량 규칙을 한 번 바꿔봐야 알 수 있고, 꼬리 관찰은 J당 10건이다.
