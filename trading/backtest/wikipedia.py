...
EXCLUDED_CHANGES = (
    ...
    # 아래 넷은 2026-08-10에 미커버 구간을 판정하며 확인했다. 벤더 심볼 목록 110,399개를
    # 이름으로 뒤져 **어느 코드에도 그 시절 계열이 없다**는 것을 확인한 것만 넣는다.
    # 넷 다 생존편향을 만들지 않는다 — Marshall & Ilsley는 인수, DeVry는 개명해 살아남았고
    # LiLAC은 6개월짜리 트래킹 주식이라, 빠지는 것은 기회이지 나쁜 결과가 아니다.
    (
        "2011-07-05", "SP500", "remove", "MI",
        "Marshall & Ilsley(2011-07 BMO 인수)의 계열이 벤더에 없다. 이름 검색 'Ilsley'가"
        " 0건이고 `MI1`은 Marshall Industries라는 **다른 회사**로 바 0행이다. 현재 `MI`는"
        " NFT Limited(2015-11-25~)라 그대로 두면 다른 회사 가격을 받는다",
    ),
    (
        "2012-10-01", "SP500", "remove", "DV",
        "DeVry의 벤더 계열 `DV_OLD`가 2011-03-31부터라 구간 앞 21개월이 빈다. 개명 후"
        " 티커인 Adtalem(`ATGE`)은 EODHD US 목록에 아예 없고, 현재 `DV`는"
        " DoubleVerify(2021-04-21~)다. 구간 시작을 못 덮으면 첫 252세션 워밍업이 불가능해"
        " 진입 자체가 안 되므로 부분 계열로 남기지 않는다."
        " **짝이 되는 2009-06-10 편입은 스냅샷이 낸 것이라 여기 행으로 넣을 수 없다** —"
        " `_snapshot_dropped`가 심볼로 거르므로 이 한 줄이면 양쪽이 다 빠진다",
    ),
    (
        "2015-07-02", "NDX100", "add", "LILA",
        "2015-07~2018 리버티글로벌의 LiLAC 트래킹 주식이다. 벤더 이름에 'LiLAC'이 하나도"
        " 없고 `LILAV`·`LILKV`는 8행·7행짜리 when-issued 라인이다. 현재 `LILA`는 2018년"
        " 분사한 Liberty Latin America(2018-01-02~)로 **다른 실체**다",
    ),
    (
        "2015-12-21", "NDX100", "remove", "LILA",
        "위 편입 행과 한 쌍이다",
    ),
    (
        "2015-07-02", "NDX100", "add", "LILAK",
        "같은 트래킹 주식의 Class C다. 근거는 `LILA` 항목과 같다",
    ),
    (
...
# 같은 날 같은 티커로 **회사가 바뀐** 자리. `_drop_same_day_reuse`가 상쇄하면 안 된다.
#
# 상쇄하는 것이 기본값인 이유는 그 티커가 하루도 지수를 벗어나지 않았기 때문이고, 그것은
# 개명이나 계열사 승계에는 맞는 말이다. 하지만 실체가 **다른 회사**면 가격 계열도 갈라야
# 하고 그러려면 구간이 둘이어야 한다. 한 구간으로 두면 앞시절이 신설 법인의 가격을 받는다.
#
# 넣는 조건은 하나다 — 변경 이력 표의 그 행이 편출과 편입에 **서로 다른 회사 이름**을
# 적을 것. 이름이 같으면 개명이므로 `SYMBOL_RENAMES`나 상쇄로 간다.
IDENTITY_CHANGES = (
    (
        "2019-03-19", "SP500", "FOXA",
        "표의 같은 행이 편출을 '21st Century Fox Class A', 편입을 'Fox Corporation Class A'"
        " 로 적는다. 21세기폭스는 디즈니에 인수됐고 폭스코퍼레이션은 그날 새로 상장한"
        " 분사 법인이다. 앞 구간은 벤더 TFCFA(2006-01-03~2019-03-20)에서 읽는다",
    ),
    (
        "2019-03-19", "SP500", "FOX",
        "같은 날 같은 사건의 Class B다. 앞 구간은 TFCF(2006-01-03~2019-03-20)에서 읽는다",
    ),
    (
        "2019-03-19", "NDX100", "FOXA",
        "Nasdaq-100에서도 같은 날 같은 교체가 일어난다. 지수마다 구간이 따로라 따로 적는다",
    ),
    (
        "2019-03-19", "NDX100", "FOX",
        "같은 날 같은 사건의 Class B다",
    ),
)


def identity_changes_for(index_name: str) -> frozenset[tuple[str, str]]:
    """`reconstruct`에 넘길 `(날짜, 심볼)`. 그 지수 것만 고른다."""
    return frozenset(
        (date_, symbol)
        for date_, index, symbol, _ in IDENTITY_CHANGES
        if index == index_name
    )
