...
class Study:
    ...
    def add_date(
        ...
        **유니버스 평균은 그날 것만 쓴다.** 다른 날 평균과 섞으면 "같은 날 같은 집합에서
        ...
        for seed, picks in (random_picks or {}).items():
            for horizon in self.horizons:
                values = [
                ...
                if values and horizon in universe_mean:
                    self._cell(self.random_draws, (seed, horizon)).add(
                        statistics.fmean(values) - universe_mean[horizon]
                    )


def random_spread(study: Study, horizon: int) -> tuple[float, float, float] | None:
    """무작위 시드들의 초과수익 `(최소, 중앙, 최대)`. 시드가 없으면 None이다."""
    values = [
        cell.mean
        for (seed, other), cell in study.random_draws.items()
