...
|C2d ✅|`TaskPanel` 반복 작성 카드, 회차 `이번만 \| 이후 전체`, 시리즈 화면|UI 계약 테스트와 1440×900·390×844 실브라우저|
...
C2b 구현 완료(2026-08-08, 로컬 전체 490/490). override 두 가지는 코드를 더하지 않았다 — 회차 건너뛰기는 기존 `cancel`, 회차 시간 변경은 기존 `PATCH`가 그대로 하고, 회차를 고칠 때 `overridden = 1`을 붙이는 두 줄만 들어갔다. 시리즈 수정은 현재 규칙 위에 준 필드만 얹은 뒤 결과를 통째로 다시 검증하므로 주기만 바꾸고 필요한 열을 빠뜨린 요청이 조용히 통과하지 않고, 주기가 바뀌면 그 주기가 쓰지 않는 열을 함께 비운다. 이미 돌고 있는 시리즈의 anchor는 과거에 있는 것이 정상이라 시작일 범위는 사용자가 이번 요청에서 직접 준 경우에만 본다.

놓친 회차 정리는 `actor_type = 'system'`으로 남고, 사용자가 같은 순간에 그 회차를 건드려 버전이 어긋나면 이번 tick을 건너뛰고 다음 tick에서 다시 본다.

배포 전 자체 리뷰에서 버그 둘을 찾아 고쳤다(2026-08-08, 로컬 전체 525/525).

- **시온의 "이후 전체 변경" 후보가 오래된 반복에서 전부 거절됐다.** 후보 payload에 규칙 전체가 담기므로 `startDate`도 함께 오는데, 서버가 그 값이 **있다는 이유만으로** 과거 검사를 했다. 한 달 넘게 돌아간 반복은 anchor가 과거라 `RECURRENCE_START_IN_PAST`로 막힌다. 이제 보내온 시작일이 지금 값과 **다를 때만** 범위를 본다. 시작일을 실제로 과거로 옮기는 것은 계속 막힌다.
- **놓친 회차 정리가 회차 날짜 순으로 골랐다.** 사용자가 어떤 회차만 뒤로 옮겼으면 가장 최근 놓친 회차는 그 옮긴 회차인데, 회차 날짜로 세면 엉뚱한 것을 남긴다. 실제 기한 순으로 바꿨다.

둘 다 합성 테스트가 아니라 **시작일이 과거로 내려간 상태**와 **회차를 옮긴 상태**를 만들어야 드러난다. 처음 테스트들이 모두 오늘 시작하는 시리즈를 썼기 때문에 가려져 있었다.

C2f 구현 완료(2026-08-08, 로컬 전체 522/522). 컨텍스트 접기는 새로 만들 것이 없었다 — C2c가 `list`에 접기를 넣었고 컨텍스트가 그 결과를 쓰므로 이미 접혀 있었다. 남은 일은 **접혔다는 사실을 모델에게 알리는 것**이었다. 반복 표시에 남은 회차 수를 적지 않으면 그 반복이 한 번만 남은 것처럼 읽힌다. 매일 반복 61회차가 컨텍스트에서 한 줄이다.

월별 노트는 같은 시리즈의 회차를 한 항목으로 묶고 날짜만 나열한다(`**반복 4회 · 19:30** · 운동` / `날짜: 10·12·14·17일`). 묶는 열쇠는 시리즈 id와 **제목**이라, 사용자가 그 회차만 제목을 고쳤으면 따로 묶여서 그 결정이 노트에서도 남는다. 묶인 항목은 그 시리즈의 가장 이른 회차 자리에 놓이므로 들어오는 순서가 달라도 같은 글이 나온다 — projection이 재생성이라 이 결정성이 계약이다.

C2e 구현 완료(2026-08-08, 로컬 전체 517/517). 두 도구 모두 아무것도 쓰지 않고, 실브라우저에서 반복 생성 카드의 `등록`과 회차 건너뛰기 카드의 `적용`이 각각 시리즈와 회차를 실제로 저장하는 것까지 확인했다.

구현이 정한 것들이다.

- **`due`와 `recurrence`는 정확히 하나만 받는다.** 둘 다 없으면 기한 없는 일정이 조용히 만들어지고, 둘 다 있으면 무엇이 정본인지 알 수 없다. 그래서 `required`를 `['title']`로 낮추는 대신 `execute`가 둘을 세어 오류를 돌려주고 모델이 다시 부른다.
- **화면 없는 단축어(H6d)에는 반복도 override도 열지 않는다.** `persistImmediately`인 세션은 `recurrence` 속성 자체가 없는 도구를 받고 `due`가 계속 필수다. 즉시 저장 예외는 지금처럼 단발 일정 생성에만 남는다.
- 카드가 부르는 경로는 서버가 준 문자열이 아니라 `action`에서만 나온다. 서버가 URL을 만들어 내려주면 카드에 보인 것과 다른 곳을 부를 수 있다.
- `expectedVersion`은 후보를 만든 시점 값이다. 그 사이에 다른 변경이 들어오면 적용이 `409`로 막히고 카드가 오류를 보여준다. 이미 있는 생성 카드가 시각이 지나면 거절되는 것과 같은 성질이다.
- 음성의 좁은 어휘에 `적용`·`바꿔` 계열을 더했다. 반복 변경 카드의 주 버튼이 `등록`이 아니라 `적용`이라 그 말을 못 받으면 말로 끝낼 수 없다.

C2d 구현 완료(2026-08-08, 로컬 전체 504/504). 반복 탭·작성 카드·시리즈 화면·스코프 선택을 붙이고 1440×900과 390×844 실브라우저에서 확인했다. 390px에서 회차 카드와 시리즈 카드의 넘침이 0이고 모든 행동 버튼이 44px이며 가로 스크롤이 없다. UI로 반복 생성 → 시리즈 시각 07:00→08:00 수정 → 회차 재생성 → `이번 회차 건너뛰기`까지 끝까지 돌렸다.

**배포돼 있던 버그를 하나 찾아 고쳤다.** `.task-form-field`·`.task-form-grid`가 `display: grid`를 쓰는데 **작성자 스타일의 `display`가 UA의 `[hidden] { display: none }`을 이긴다.** 그래서 `hidden` 지정이 한 번도 먹지 않았고, 마감이 `없음`인데도 마감 날짜·시각 칸이 계속 보이고 있었다. `.task-form [hidden] { display: none; }` 한 줄로 못박았다. 반복 UI는 요일과 날짜 칸의 배타적 표시를 `hidden`에 기대므로 이것을 고치지 않으면 성립하지 않는다.
...
- **재생성이 걷어내는 회차는 취소가 아니라 물리 삭제다.** `series:{id}:{날짜}`가 이미 쓰인 ID로 남으면 같은 날짜에 새 규칙의 회차를 만들 수 없어 규칙 변경이 시리즈의 회차를 통째로 없앤다. 대상은 규칙에서 파생됐을 뿐 사용자가 손댄 적 없는 미래 회차뿐이라 감사 이력에 남길 값이 없고, 아무도 본 적 없는 행을 취소로 남기면 월별 노트의 취소 구역이 하지도 않은 일로 찬다. 발화된 알림이나 delivery가 하나라도 붙은 회차는 대상에서 제외해 이 조건을 코드가 강제한다.
- **materializer는 그 날짜에 회차가 이미 있으면 건드리지 않는다.** 기존 create의 멱등에 기대면 새 규칙의 payload가 달라 `409`가 난다. 보존이 성립하는 자리가 여기다.
- **시각을 앞당기면 오늘 회차가 사라질 수 있다.** 19:30 반복을 낮에 07:00으로 바꾸면 오늘 07:00은 이미 지나서 손대지 않은 오늘 회차가 지워진 뒤 다시 만들어지지 않는다. 지나간 시각의 회차를 만들지 않는다는 규칙이 그대로 적용된 결과다.
- 제목·설명만 바꾸면 회차를 다시 만들지 않는다. 이미 만든 회차의 제목은 그대로 두고 새로 만드는 회차부터 새 제목을 쓴다. 회차마다 사용자가 따로 고쳤을 수 있어서 일괄로 덮어쓰면 그 결정을 지운다.
- 반복 종료도 같은 삭제를 쓴다. 종료 뒤에는 지난 기록만 남는다.
...
- 작성 카드에 반복 선택을 더한다. 규칙 넷과 시각, 선택적 종료일, 선택적 알림 offset이고, 제출 전 요약은 `매주 월·수·금 07:00 · 2026. 8. 9.부터 / 회차 알림 기본`처럼 **규칙을 문장으로** 보여준다.
- **다음 회차 날짜를 미리 계산해 보여주지는 않는다.** 그러려면 규칙 엔진이 클라이언트에도 있어야 하고, 두 벌이 되면 화면과 실제 회차가 어긋난 뒤에야 그 사실이 드러난다. 만들어진 회차는 서버 응답으로 확인한다.
- 이미 만들어진 단발 일정을 반복으로 바꾸는 길은 열지 않는다. 회차를 뒤늦게 붙이면 그 일정의 기한과 시리즈 규칙 중 무엇이 정본인지 알 수 없다.
