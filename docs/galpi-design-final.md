...
- **첨부 U0a 로컬 구현·강의 미구현:** schema v12의 blob/upload 분리, 인증된 단일 파일 임시 업로드, 엄격한 형식 검증과 60분 orphan 정리까지 로컬에서 구현했고 전체 회귀 356/356을 통과했다. flag 기본값은 `false`이며 Pi에는 아직 배포하지 않았다. 다음 U0b에서 temporary 첨부의 `CONTEXT_N=10` 사용자 턴 snapshot·메시지 연결·모델 호출 전 만료를 구현한다. library는 승인 후에만 영구 저장한다. 강의 Phase 0은 지금 검증할 수 있고 코드 Phase 1 이상은 일반 첨부의 인증 blob 패턴 뒤에 진행한다.
