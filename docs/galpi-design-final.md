...
- **H6d 단축어 일정 생성 로컬 구현:** 화면 없는 iPhone 단축어에서 명시적인 새 일정·할 일·알림 생성만 기존 `schedule_prepare` validator 뒤 `shortcut-task:<requestId>`로 즉시·멱등 저장한다. PWA의 카드 확인과 shortcut bearer의 exact route scope는 유지하고 기존 일정 mutation·다른 쓰기는 계속 닫는다. 로컬 전체 386/386을 통과했으며 Pi·실기기 인수는 남아 있다.
