...
- **현재 단계:** V3.5와 V4-A 논문 검색·전문 능동 독서, V4.5 A0·A1 shadow, S0b-2 Pi 승인형 복구, S0c 공용 쓰기 경로와 A1b 전역 청크+노트 soft prior shadow 검색까지 Pi 운영 적용을 마쳤다. A1b는 `adb41a6`, 한국어 경계 보정은 `fc332e2`, 실사용 관찰 도구는 `8655706`에서 구현했고 제품·비서·운영 경로는 `4ce7fdc`에서 갈피/시온과 `/home/pi/galpi`로 이관했다. Pi 전체 테스트 104개, audit 66/66, 배포 파일 hash, 인증 API·UI·온라인 백업과 schema_version 외 application table의 논리 동일성·SQLite 무결성·외래키를 인수했다. S0d Markdown-only Q&A 재색인은 `68604af`에서 로컬 구현해 전체 테스트 109개와 로컬 audit 7/7을 통과했으며 Pi에는 아직 배포하지 않았다. 실제 모델 답변은 기존 노트 회수를 유지한다.
...
- **S0d Markdown-only Q&A 재색인(로컬 구현 완료·Pi 미배포, `68604af`):** 단일 정본 file-only Q&A만 strict parser·계획 hash로 재검증하고, 임베딩 성공 뒤 기존 topic repair의 백업·서버 중지 확인·DB transaction·rollback 경로에서 provenance·본문 hash·`ready` 청크를 복구한다. 출처가 여러 개이거나 현재 노트 배정과 다르면 수동 검토로 남긴다. stale 원문, 임베딩·DB 실패, 재실행 멱등성을 포함한 전체 테스트 109개와 로컬 readonly audit 7/7, 복구 계획 `clean`을 통과했다.
