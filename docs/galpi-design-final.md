...
- **첨부 U0a·U0b 로컬 구현·강의 미구현:** schema v12 blob/upload와 schema v13 message 연결, 인증 단일 파일 업로드, `CONTEXT_N` snapshot, 실행 lease, 모델 호출 전 replay 만료·history tombstone까지 로컬에서 구현했고 전체 회귀 360/360을 통과했다. 첨부 원문은 아직 모델에 넣지 않는다. flag 기본값은 `false`이며 Pi에는 미배포다. 다음은 U0c composer 업로드·취소·카드 UI다. library는 승인 후에만 영구 저장하고 강의 코드 Phase 1 이상은 일반 첨부 기반 뒤에 진행한다.
