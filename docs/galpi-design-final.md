...
- **첨부 U0a~U0c·composer 정리 로컬 구현, 강의 미구현:** schema v12 blob/upload와 schema v13 message 연결, 인증 단일 파일 업로드, `CONTEXT_N` snapshot, 실행 lease, 모델 호출 전 replay 만료, 업로드·취소와 공통 카드/tombstone까지 구현했다. composer는 위 입력·아래 도구의 두 줄 구조이며 첨부는 `+` 메뉴, 오른쪽 한 자리는 빈 입력에서 반이중 음성·입력 뒤 전송으로 전환한다. 데스크톱은 전체 폭 하단 바 없는 600px floating composer와 40px 제어, 모바일은 safe-area 면과 44px 타깃을 쓴다. 전체 회귀 368/368과 1440px·1024px·390px·320px overflow 0을 통과했다. 첨부 원문은 아직 모델에 넣지 않는다. flag 기본값은 `false`이며 Pi에는 미배포다. 다음은 U1 PDF·MD·TXT 읽기다. library는 승인 후에만 영구 저장하고 강의 코드 Phase 1 이상은 일반 첨부 기반 뒤에 진행한다.
