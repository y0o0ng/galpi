...
- **첨부 U0a~U0c·U1a 로컬 구현, 강의 미구현:** schema v12 blob/upload·v13 message replay 연결에 이어 schema v14 `attachment_documents/chunks`와 MD·TXT·PDF on-demand 파싱을 구현했다. hash+parser version 재사용, single-flight, 30초 timeout, 스캔 PDF `needs_ocr`, Markdown 헤딩/줄 범위·PDF 페이지 범위와 replay/orphan 파싱 데이터 삭제를 집행한다. composer·카드 UI는 기존 U0c 계약을 유지하고 원문은 아직 GPT에 넣지 않는다. 전체 378개 중 377개와 organizer 격리 7/7을 통과했다. flag 기본값은 `false`, Pi 미배포이며 다음은 U1b 관련 청크 회수·bounded GPT 주입이다. library는 승인 후에만 영구 저장한다.
