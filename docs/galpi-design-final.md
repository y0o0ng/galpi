...
- **첨부 U0a~U0c·U1a~U1b Pi 기술 인수, U1c 보정 배포 대기, 강의 미구현:** schema v12~14 업로드·message replay·MD/TXT·PDF on-demand 파싱과 현재/replay 첨부 최대 3개의 `attachment_document_search → read`를 Pi에 활성화했다. 첫 모델 요청은 원문 0자이고 도구 뒤 관련 청크만 호출당 5,000자·합계 10,000자로 제한한다. iPhone 실제 MD 84,175 bytes·1,717줄·109청크의 최초 질문과 파일명·줄 출처는 통과했다. replay 후속 답변이 도구를 재호출하지 않아 자동 topic Q&A 1건이 생긴 결함은 현재/replay 후보 존재 자체로 자동 저장을 막도록 보정했고 로컬 386/386을 통과했다. Pi 배포·exact Q&A 제거·같은 실기기 replay 재확인이 남았으며 library는 승인 후에만 영구 저장한다.
