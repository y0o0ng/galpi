...
- **첨부 U0a~U0c·U1a~U1b 로컬 구현, 강의 미구현:** schema v12~14 업로드·message replay·MD/TXT/PDF on-demand 파싱에 이어 현재/replay 첨부 최대 3개의 `attachment_document_search → read`를 공용 GPT tool loop에 연결했다. 첫 모델 요청에는 원문을 넣지 않고 도구 선택 뒤 관련 청크만 호출당 5,000자·합계 10,000자로 제한한다. 파일명·페이지/줄 출처, 비신뢰 자료 경계, temporary 근거 답변의 자동 topic 저장 제외을 집행하며 library는 승인 후에만 영구 저장한다. 로컬 전체 384/384를 통과했고 flag 기본값은 `false`, Pi 미배포이다. 다음은 schema 11→14 배포·iPhone/iPad 문서 질문 인수다.
