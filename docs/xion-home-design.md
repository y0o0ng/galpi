...
## 12. 날씨 — 모양은 정했고 아직 만들지 않았다

2026-08-21 논의에서 정한 것이다. **구현하지 않았다.**

**모양은 머리줄(2절의 0번) 확장이다.** 별도 카드도 아이콘도 아니다.

```
좋은 저녁이야, 8월 21일 금요일 · 22° 🌧️ 오후부터 비
```
...
**이것이 깨는 계약은 10절의 `서버 변경 0줄` 하나다.** 외부에서 가져오므로 캐시와 갱신
주기가 생긴다. 반면 1절의 "정상 상태는 정보가 적은 상태다"는 깨지 않는다 — 그 문장이
금지하는 것은 *채우기 위해 정보를 지어내는 것*이고, 날씨는 지어낸 것이 아니라 사람이
아침에 실제로 알고 싶은 것이다. **다만 날씨는 항상 있으므로 홈이 영영 비지 않는 화면이
된다.** 9절의 빈 상태가 사라지는 것은 감수한 대가다.

**순서는 뉴스 다음이다.** 홈에 `알아둘 것`이 붙은 것이 2026-08-21이라, 그 상태로 며칠
써본 뒤에도 허전하면 그때 만든다. 둘을 동시에 얹으면 어느 쪽이 화면을 살렸는지 가를 수
없다.

**구현 메모.** 기상청 단기예보는 격자 좌표(`nx`·`ny`)를 요구하지만 **Pi가 집에 고정돼
있으므로 상수 하나다** — 위경도 변환 코드가 필요 없다. 실제로 까다로운 곳은 발표 시각
규칙 하나뿐이다(02/05/08/11/14/17/20/23시, 각 발표 +10분 뒤부터 조회 가능). 쓰는 값은
`TMP`·`SKY`·`PTY`·`POP` 넷이면 충분하다. 공공데이터포털 키 발급에 승인 대기가 있을 수
있다.
