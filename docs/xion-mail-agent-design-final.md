...
>
> 상태: **Phase 1~3은 Pi 배포·실기기 인수까지 완료했다**(2026-08-19). **Phase 4는 preference·라우팅 억제 · 메일 검색 · thread 단위 Attention까지 인수했고**(같은 날) 대화 생성 경로만 남았다. thread 묶기는 Attention 축에서만 하며 알림은 메일마다 그대로 울린다(27절 A안). **네이버 웍스가 세 번째 provider로 붙었다**(schema v21) — 같은 IMAP 구현에 호스트만 다르다. 검색은 4절대로 화면이 아니라 대화에 있다 — 채팅 도구 `mail_search` 하나이며 사용자가 물은 턴에만 결과가 컨텍스트에 들어오고, 본문이 없으므로 찾는 것은 제목·발신자와 판단이 남긴 값이다. 만들기는 알림 카드의 발신자 억제 한 동작뿐이며, `always_notify`·`skip_analysis`는 저장 통로가 없어 라우팅이 승격하지 않는다 — 대화 생성 경로를 열 때 함께 연다. 단계별 실측과 남은 통과 기준은 [로드맵](roadmap.md)의 독립 트랙 `MAIL-1~4`에 있다.
