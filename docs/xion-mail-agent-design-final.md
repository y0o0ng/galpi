...
>
> 상태: **Phase 1~4를 Pi 배포·실기기 인수까지 완료했다**(Phase 1~3은 2026-08-19, Phase 4는 preference·라우팅 억제 · 메일 검색 · thread 단위 Attention이 같은 날, 대화 생성 경로가 2026-08-20). thread 묶기는 Attention 축에서만 하며 알림은 메일마다 그대로 울린다(27절 A안). **네이버 웍스가 세 번째 provider로 붙었다**(schema v21) — 같은 IMAP 구현에 호스트만 다르다. 검색은 4절대로 화면이 아니라 대화에 있다 — 채팅 도구 `mail_search` 하나이며 사용자가 물은 턴에만 결과가 컨텍스트에 들어오고, 본문이 없으므로 찾는 것은 제목·발신자와 판단이 남긴 값이다. 만들기는 둘이다 — 알림 카드의 발신자 억제 한 동작과, 3.4·3.5대로 사용자가 말로 표현할 때만 쓰는 채팅 도구 `mail_preference_set`이다. 규칙은 사용자 발화에서만 나오고 메일에 적힌 같은 요청은 데이터지 지시가 아니다(19절). 통로가 생기면서 `always_notify`는 11.1대로 한 칸만 승격하고(`silent → batch`, `immediate`는 모델이 `urgent`/`action_required`로 본 메일만) 억제가 승격을 이기며, `skip_analysis`는 기존 pre-LLM bypass가 집행한다. 확인·되돌리기는 에이전트 탭 Mail 상세의 `메일 알림 규칙`이고 거기서 만들지는 않는다. **알림 카드는 그 메일의 본문을 열고(23절) 그 메일을 일정 후보로 만든다(15절)** — 본문은 여전히 저장하지 않고 열 때마다 Provider에서 읽으며, 일정은 후보 카드를 사용자가 `등록`해야 기존 task API에서 만들어진다. 단계별 실측은 [로드맵](roadmap.md)의 독립 트랙 `MAIL-1~4`에 있다.
...
구현은 알림 카드의 `일정 등록` 버튼이고 채팅의 후보 카드와 **같은 컴포넌트**를 쓴다. 판단이 이미 뽑아둔 기한을 그대로 쓰므로 모델을 다시 부르지 않는다. 멱등키가 `mail-attention:<id>`라 같은 메일로 두 번 눌러도 일정은 하나이고, 알림은 기한이 있는 일정에 서버가 붙이는 기본 알림에 맡긴다. 기한이 없거나 이미 지난 메일에는 버튼을 만들지 않는다 — task API가 과거 기한을 거절하므로 눌러도 실패할 버튼이 된다.
...
Notification card
 └ 클릭/액션으로 펼침
    ...
    - deadline
    - 판단 이유
    - 제목 줄을 누르면 본문(텍스트) — 저장된 것이 아니라 그때 Provider에서 읽는다
    - [완료] [나중에] [일정 등록] [알리지 마]
```

본문은 카드의 확장이지 새 화면이 아니다. HTML을 그리지 않고 텍스트만 넣으며(원격
이미지·추적 픽셀이 들어올 구멍을 만들지 않는다), 첨부는 이름과 크기까지다. 한 계정에서
동시에 하나만 읽어서 연타가 IMAP 연결을 쌓지 않게 한다. 읽어도 메일함의 읽음 상태는
