...
- 공통 Tavily 요청(`lib/web/service.js`)은 응답 본문 읽기까지 요청당 10초로 제한한다. 채팅 검색·페이지 추출에도 같은 제한을 적용한다.
- 시간 초과·본문 읽기/JSON 파싱 실패는 기존 provider 실패 코드로 처리하며 성공 결과로 캐시하거나 사용량을 기록하지 않는다. 응답을 받지 못한 요청의 실제 provider 과금 여부는 알 수 없다.
- 실패한 관심은 기존 1시간 backoff 뒤 수집 tick에서 재시도한다. 다른 관심은 계속 처리하고, 수집이 끝나면 running 상태를 해제한다.
