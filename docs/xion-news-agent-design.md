...
9. **문턱이 정해지기 전의 홈 노출** — 수집·판단·조회는 `NEWS_AGENT_ENABLED`로 돌리고 홈만 `NEWS_SURFACE_ENABLED`로 따로 연다. 표본을 모으려면 수집을 켜야 하는데, 그러면 아직 정하지 못한 `SURFACE_THRESHOLD`가 홈 판정을 시작하기 때문이다. 문턱을 정한 뒤에 켠다.
