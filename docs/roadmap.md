...
6. V5-A Phase 2까지 검증되면 V5-B 주식 분석, 그다음에만 외부 캘린더 읽기·일정 조정 같은 V5-C 역할 검토. 갈피 내부 수동 task/reminder인 V4.5-C와는 별개다. **예외는 V5-C 메일 에이전트 하나로, 외부 게시·자금·플랫폼 정책 위험이 없어 2026-08-17에 순서를 앞당겼다**(아래 절).
...
### V5-C 메일 에이전트 (2026-08-17 추가, 다음 제품 작업)

시온이 Gmail·네이버 메일을 백그라운드로 보고 **사용자가 실제로 신경 써야 하는 것만** 알린다. 상세 단일 기준은 [Xion Mail Agent 설계](xion-mail-agent-design-final.md)이고 여기에 상세를 복제하지 않는다.

- **V5-A 재개를 기다리지 않는다.** V5-A가 검증하려던 위험(외부 플랫폼 계정 정책·상업 게시·수익성)이 메일 에이전트에는 없다. 사용자 자기 메일을 읽고 사용자에게만 알리는 읽기 전용 관찰자라 자금도 외부 게시도 없다. 순서 예외는 여기까지이며 V5-A·V5-B의 남은 단계 순서는 그대로다.
...
- 재사용: 기존 Web Push 구독·transport·delivery loop, 알림 탭, 기존 task API. **새 Dashboard·범용 Notification Framework·전용 메일함 화면을 만들지 않는다.**
- 단계는 설계 문서 24절의 Phase 1~4다 — Provider durable sync → 분석 큐 + Attention → 알림 UX → 피드백/검색. **Phase 1 통과 전에 LLM을 붙이지 않는다.**
- 전체는 `MAIL_AGENT_ENABLED` 기본 `false` 뒤에 둔다.

통과 기준(요약): 읽음 여부와 무관하게 새 메일을 잡고 · 최초 연결이 과거 메일로 울리지 않고 · cursor 손상·재시작·중복 poll에서 중복이 0건이며 · 분석이 실패해도 좌초하지 않고 사람이 복구할 수 있고 · Push가 전부 실패해도 Attention이 남는다.
