...
- [ ] Push payload의 key 집합이 허용 목록(`version`·`type`·`targetKind`·`targetId`·`notifySeq`·`url`)과 정확히 일치하고, 직렬화된 문자열에 발신자·제목·요약·개수가 하나도 나타나지 않는다 — MAIL-3. 개별 필드 부재만 보면 기존 필드에 내용을 끼워 넣는 것을 못 잡는다. **절반만 잠겼다**: `test/mail-push.test.js`가 message target의 key 집합 일치와 batch payload에 `item_count`가 안 나타나는 것은 값으로 잠갔지만, **batch target의 key 집합 일치**와 **발신자·제목·요약 문자열이 직렬화 결과에 없다는 것**은 아직 단언이 없다(그 테스트가 쓰는 메일에 제목·발신자가 비어 있어 우연히 통과한다)
