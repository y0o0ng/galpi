...
- **MAIL-3 — 알림 UX**: Immediate/Batch/Silent · quiet hours · snooze 재알림 · 기기별 delivery · 알림 탭 메일 필터 · **Push payload는 routing metadata만**(설계 13.1). 잠금화면 미리보기 설정은 2026-08-18 없앴다 — 설정이 있으면 민감 내용을 payload에 넣는 코드 경로가 존재하고 그 경로가 있는 한 버그 하나로 샌다.
...
- [ ] Push payload의 key 집합이 허용 목록(`version`·`type`·`targetKind`·`targetId`·`notifySeq`·`url`)과 정확히 일치하고, 직렬화된 문자열에 발신자·제목·요약·개수가 하나도 나타나지 않는다 — MAIL-3. 개별 필드 부재만 보면 기존 필드에 내용을 끼워 넣는 것을 못 잡는다
