...
- U0a 서버 업로드 기반은 2026-08-04 로컬 구현을 마쳤다. schema v12 blob/upload 분리, 인증된 단일 파일 route, `GALPI_DATA_DIR/attachments/tmp`, 형식·MIME·시그니처·UTF-8·크기·SHA 검증과 60분 orphan 정리를 구현했고 전체 회귀 356/356을 통과했다. flag 기본값은 `false`이며 Pi에는 아직 배포하지 않았다. 다음은 U0b 메시지 연결·replay snapshot·모델 호출 전 만료다.
