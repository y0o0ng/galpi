...
- U0a·U0b는 2026-08-04 로컬 구현을 마쳤다. schema v12 blob/upload 분리와 인증 업로드, schema v13 message 연결, `CONTEXT_N` snapshot, 실행 lease, 모델 호출 전 replay 만료·tombstone 복원을 구현했고 전체 회귀 360/360을 통과했다. 첨부 원문은 아직 모델에 보내지 않는다. flag 기본값은 `false`이며 Pi에는 미배포다. 다음은 U0c composer 업로드·취소·카드 UI다.
