...
- U3a는 schema v15와 명시적 `서재 저장` 카드 액션으로 MD·TXT·PDF를 Vault 원본+평면 Attachment 노트에 원자적으로 승격한다. file fsync·SHA/크기 검증, exact orphan 재시도, blob dedup, temporary cleanup을 적용했고, 회수된 Attachment 노트만 기존 문서 도구로 전문을 읽는다. library-only 후속 답변은 일반 저장을 허용하되 temporary가 하나라도 살아 있으면 후보 상한 밖이어도 계속 막는다. Pi·iPhone 인수 전이며 Codex 제목·요약, 인증 원본 열기, 이미지는 U3b·U2 이후다.
