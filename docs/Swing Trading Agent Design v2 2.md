...
#### 인접 파라미터 안정성이 왜 아직 안 되는가

14.4·21.2는 18/20/22일 같은 인접값으로 여러 번 돌려 붕괴가 없는지 보라고 한다. 지금 지표 파라미터(`BREAKOUT_WINDOW`·`RS_LOOKBACK`·`ATR_WINDOW` 등)는 모듈 상수라 실행마다 바꿀 수 없다. 1.4는 "전략 버전과 지표 파라미터"를 사용자 승인 항목으로 두므로, 이 값들은 원래 `PolicyVersion`에 있어야 한다. **그 이관이 인접 파라미터 검증의 선행 조건이고 다음 자연스러운 작업이다.**
...
지표 파라미터를 `PolicyVersion`으로 옮기고 워크포워드 분할(14.4)을 만드는 것, 그리고 실데이터다. 그 셋이 있어야 14.7 판정이 의미를 가진다.
