...
9.2의 나머지 한도는 20.1이 별도 단계로 둔 "리스크 엔진과 PolicyVersion"의 것이다. 아직 주문을 낼 수 있는 경로가 없어 미집행이 위험을 만들지 않는다. 다만 **크기 계산만으로 9.2를 지킨다고 오해하면 안 된다.**

- 동시 보유 5종목, 섹터 최대 비중 25%, 계획된 총 손절 위험 1.25%, 60일 상관 0.75 쌍 합산 25%
- 일일 손실 -1.0%, 주간 손실 -2.5%, 고점 대비 낙폭 -5%/-7%/-10%의 수량 축소·진입 중단·HALT
- 섹터 한도와 7.2의 증권 종류 필터는 둘 다 종목 분류 데이터가 필요하다. 리스크 엔진 단계에서 `securities` 표를 함께 추가한다.

#### 남은 것

리스크 엔진과 PolicyVersion(9.2), 체결·비용 모델(10.1·10.4), 청산 규칙(7.5), 성과 지표(14.7)다. 미체결은 일봉에서 20분 재산정을 모델링할 수 없으므로 재산정 없이 취소하고 `NO_FILL` 건수를 기록한다. 재산정을 흉내내는 쪽이 낙관 편향을 만들고, 기록한 건수가 나중에 실행과의 불일치 크기를 알려준다.
