...
### NDX100 구성원 복구 — 위키는 불가능하고 QQQ는 가능하다 (2026-08-10)

`backtest/qqq.py`와 `real_run.py`의 `qqq` 단계다. **파싱까지 끝났고 멤버십 병합은 남았다.**

#### 위키에는 NDX100의 과거 구성원 표가 없다

SP500에서 통한 방법의 전제가 NDX100에는 없다.

- `List of NASDAQ-100 companies`는 2005-11-17에 만들어진 뒤 **2026-04-01 판까지도 24자짜리 리다이렉트**였다. 지금의 72,995자 문서는 2026년 중반에 생겼다.
- 본체인 `Nasdaq-100` 문서에는 구성원 표가 **있었던 적이 없다.** 2012년 판은 본문 20,618자에 표가 0개(전부 산문), 2017-12 판은 전고점 표 하나, 현재 판도 전고점·연도별 수익률·마일스톤 표뿐이다.

diff할 "그 시점의 전체 목록"이 존재하지 않는다. `SNAPSHOT_PAGES`에 SP500만 남기고 이 근거를 적어뒀다.

#### 그런데 NDX100에도 상쇄가 있다

|근거|값|
|---|---|
|`LMCK` 주식이 생긴 날|2014-07-09|
|우리 구성원 시작|2008-01-02|
|`DISCK` 주식이 생긴 날|2008-09-18|
|우리 구성원 시작|2008-01-02|
|2010-01-04 구성원 수|**정확히 100**(불변식 99~101)|

**존재하지도 않던 주식 둘이 들어 있는데 총 100이면 진짜 구성원 둘이 빠진 것이다.** SP500의 498과 같은 모양이다.

#### QQQ 보유종목이 대안이다

Invesco QQQ Trust, Series 1(CIK 1067839)은 나스닥100 완전복제 UIT라 보유종목이 곧 구성원 명단이고, SEC 제출이라 공개·무료이며 **삭제 의무가 없다.**

|양식|건수|기간|성격|
|---|---|---|---|
|`N-30B-2`|22|2005~2025 제출|기준일은 감사받은 회계연도 말(9/30)|
|`NPORT-P`|27|2019-11~|구조화 XML|

**위키보다 나은 점**은 날짜가 "늦어도 이 판"이 아니라 정확한 기준일이라는 것이다. 편출일 지연 보정이 필요 없다. **못한 점**은 연 1회라 같은 회계연도에 들어왔다 나간 종목이 안 잡힌다는 것이고, 그 부류는 인수·실패가 많아 약한 편향이 남는다. 2019년부터는 분기라 촘촘해진다.

#### 두 읽기를 시도하고 구성원 수 밴드가 심판한다

20년치 서식을 규칙 하나로 덮으려던 시도를 접었다. 실패한 것들이 이렇다.

- **제목 줄 앵커**: `Schedule of Investments`가 목차에도 있고 2016년 판부터는 감사 의견 문장 속에만 나온다.
- **이름 다음에 주식 수**: 2016년 판은 `주식수 / 이름 / (a) / 평가액` 순으로 뒤집혀 있다.
- **후보 간격으로 명세 자르기**: 페이지 넘김이 13~15줄 틈을 만들어 2014년 판이 38개에서 끊겼다. 간격을 넓히면 뒤쪽 재무제표(`Net investment income`)가 딸려 들어온다.

대신 줄 기반과 표 기반을 다 시도하고 구성원 수가 95~115에 드는 쪽을 쓴다. **둘 다 안 들면 그 판을 버린다** — 94개짜리 명단을 그대로 쓰면 다음 판과의 diff에서 없던 편출이 여섯 건 생긴다. 다섯 판을 버렸고 창 안은 2015-09-30 하나다.

**2014년 이전 스냅샷이 전부 정확히 100개로 나온 것이 파싱이 맞다는 독립 증거다.** 다중 클래스 허용 전 나스닥100은 정확히 100이고, 그 불변식은 파서가 모르는 지식이다.

기준일은 본문이 아니라 제출 목록의 `reportDate`를 쓴다. 구조화된 값이라 서식과 무관하다.

#### 이름을 티커로 옮기는 규칙

명세에는 티커가 없고 회사 이름만 있다. 공고 표의 종목명(그 시절 이름)과 벤더 목록 이름(현재 주인 이름)을 겹쳐 471개 중 434개가 풀리고, 나머지는 `NAME_OVERRIDES`가 근거와 함께 받아 **4개만 남는다**(리버티 인터랙티브 계열).

함정 셋을 실측으로 만났다.

- **각주를 먼저 뗀다.** `(a)`를 남기면 구두점을 지우는 단계에서 `A`가 되어 클래스 A와 섞인다. 이 하나 때문에 467개 중 166개가 안 풀렸다.
- **`N.V.`는 마침표를 붙여서 지운다.** 공백으로 바꾸면 두 낱말이 되어 법인 꼬리표 목록에 안 걸린다.
- **클래스 충돌은 클래스끼리만 본다.** "같은 티커를 다투면 거부"로 하면 `Facebook`과 `Meta Platforms`가 서로를 막는다. 막아야 할 것은 한 회사의 **서로 다른 클래스**뿐이다(`GOOGL`/`GOOG`). `NPORT-P`는 `<name>`이 아니라 `<title>`에 클래스가 있다.

못 푼 이름은 추측으로 메우지 않는다. 양쪽 스냅샷에서 함께 빠지므로 유령 사건을 만들지 않고, 목록으로 남아 사람이 판단한다.

#### 남은 일 — 이름 사전이 시간을 모른다

사건 359건을 만들어 병합하니 **불변식 위반 223건**이 났다. 겉모습은 셋인데 근본은 하나다.

|증상|실체|
|---|---|
|`2022-12-31 BHI 추가`|2022년 보유 `Baker Hughes`가 2017년 사건에 기록된 옛 티커 `BHI`로 풀렸다. 정답은 `BKR`|
|`2016-09-30 AVGO 제거` 뒤에도 구성원|2015-09-30 판이 빠져 2년을 건너뛴 diff가 개명(`Avago`→`Broadcom`)과 엉켰다|
|`2009-09-30 구성원 98개`|해석 못 한 종목 때문에 스냅샷이 모자라 구성원 수가 어긋났다|

**이름 사전도 `SYMBOL_RENAMES`·`CIK_OVERRIDES`처럼 날짜를 받아야 한다.** "티커는 회사가 아니다"를 이름 축에서 다시 만난 것이다.

#### 그 뒤 — 날짜를 준 사전과 나머지 셋

`ticker_on`이 지수 사건 한 줄을 관측 하나로 보고 `add`는 "그날부터 이 티커", `remove`는 "그날까지 이 티커"로 읽는다. 같은 날 편출·편입 쌍인 개명이 이 규칙으로 정확히 갈린다. 그다음 걸린 셋도 전부 같은 얘기의 변주였다.

|증상|실체|
|---|---|
|구성원이 하나 모자람|`Avago`와 `Broadcom Corporation`은 2012~2015년에 **다른 회사**인데 아바고가 2016년 인수하며 `AVGO`와 이름을 함께 가져갔다. 정규화 키로는 `Broadcom Corp.`과 `Broadcom Inc.`을 못 갈라 오버라이드를 **원본 이름 그대로** 맞추게 바꿨다|
|`LLC`라는 없는 티커|줄바꿈으로 쪼개진 `Class A` 조각이 사전의 아무 `...LLC`에 붙었다. 클래스만 남고 회사가 없는 키는 종목이 아니다|
|2014-04부터 밴드 초과|**우리 사전 지식이 틀렸다.** 다중 클래스 전환일은 2014-12-22가 아니라 구글이 갈라진 2014-04-03이다. QQQ의 감사받은 2014-09-30 보유 103종목이 그것을 교정했다 — 새 소스가 기존 불변식을 더 정확하게 만든 자리다|

편출일 지연 판정은 심볼 공간을 둘 다 보게 했다. 공고·위키는 지수 티커를 쓰고 QQQ는 이름으로 풀어 벤더 코드가 나오기도 한다(`TFCF`). 한쪽만 보면 그 소스가 닫은 구간이 통째로 안 걸린다.

**결과: 구간 1,237개 위반 0, 미커버 0구간. 세 그물을 모두 통과해 `survivorship_biased=False`로 선언했다.**

-----

### 15년 실데이터 14.7 판정 (2026-08-10)

`real_run.py`의 `run` 단계다. **판정은 `FAIL`이고 blocker는 없다.**

blocker가 없다는 것이 이 실행의 값이다. `SURVIVORSHIP_BIASED`도 `NO_WALK_FORWARD`도 걸리지 않아 **숫자를 그대로 읽을 수 있다.** 구간 2007-01-04~2026-08-07, 4,929세션, 518거래.

|지표|값|판정|
|---|---|---|
|기대값|-0.046R|FAIL|
|승률 / 평균이익 / 평균손실|39.4% / +1.21R / -0.86R||
|Profit Factor|0.865|FAIL|
|Sharpe / Sortino|-0.178 / -0.237|FAIL|
|최대낙폭|7.5%|TARGET|
|fold 안정성|4/18 = 0.222|FAIL|
|평균 익스포저|6.3%||
|최종 자산|93,030 (19.6년에 -7.0%)||

#### 비용 탓이 아니다

```
순손익 -6,970  +  비용 11,851  =  비용 전 +4,881
거래당: 비용 $22.9, 비용 전 +$9.4 (위험 $250 기준 ≈ +0.04R)
```

**비용 전 엣지가 거래당 0.04R이다.** 14.7이 요구하는 0.31R과 자릿수가 다르다. "좋은 전략을 비용이 잡아먹었다"가 아니라 애초에 엣지가 노이즈 수준이라는 뜻이고, 스모크 34거래에서 본 +0.05R이 518거래에서도 그대로 나왔다. 지불구조로도 같다 — 승률 39.4%의 손익분기 페이오프는 1.54인데 실제는 1.40이다.

#### 구조적으로 드러난 둘

**실적 청산이 최대 청산 사유다.** `EARNINGS 188` > `INITIAL_STOP 169`. 진입한 것의 36%가 자기 손절이 아니라 실적 임박으로 쫓겨난다. 논지가 전개되기 전에 나간다.

**익스포저가 6.3%다.** 프로필 상한이 60%인데 진입 게이트가 좁아(`SMA_NOT_ALIGNED` 93만 회·`NO_BREAKOUT` 72만 회 스킵) 연 26거래뿐이다. 최대낙폭 7.5%가 좋아 보이는 것도 위험 관리가 아니라 돈을 안 써서다.

#### 홀드아웃은 한 번 소모됐다

`plan.folds`에 홀드아웃이 마지막 항목으로 들어가 워크포워드가 함께 돌렸다. 2025-08-07~2026-08-07이 기대값 +0.104로 유일하게 괜찮았다. `record_holdout_run`을 부르지 않아 횟수는 남지 않았지만 **이미 봤다.** 파라미터를 바꿔 다시 돌리면 그 구간은 표본 밖이 아니다.

`parameter_neighbourhood`가 `UNDETERMINED`인 것은 중심 기대값이 음수라 붕괴비율을 낼 수 없어서다. 인접값 여덟이 전부 음수(-0.039~-0.290)라 근처에 고원이 없다.

-----

### 청산 해부가 내 해석을 뒤집었다 (2026-08-11)

`backtest/report.py`의 청산 해부와 진입 깔때기다. 보고서는 `trading/runs/`에 남는다.

#### 건수만 보고 판단한 것이 틀렸다

판정 직후 나는 "진입의 36%가 실적 임박으로 쫓겨나 논지가 전개되기 전에 나간다"고 읽었다. **틀렸다.** 평균 R과 평균 MFE를 나란히 놓으니 정반대였다.

|청산 사유|건수|평균 R|손익|평균 MFE|평균 MAE|보유|
|---|---|---|---|---|---|---|
|`EARNINGS`|188|**+0.63**|**+21,222**|1.29|-0.43|11.8세션|
|`MAX_HOLD`|41|**+1.78**|+13,229|2.61|-0.40|40.0세션|
|`TIME_STOP`|21|-0.20|-653|0.55|-0.66|20.0세션|
|`TRAILING_STOP`|99|-0.16|-2,698|**1.74**|-0.71|19.0세션|
|`INITIAL_STOP`|169|-1.15|**-38,070**|0.28|-2.19|5.4세션|

실적 청산은 **최대 수익원**이다. MFE 1.29 중 0.63을 실현했으니 절반을 챙겼다. 건수는 규칙이 얼마나 자주 발동하는지를 말할 뿐 그 규칙이 돈을 버는지는 말하지 않는다. **MFE 열이 없었으면 계속 틀린 채로 갔다.**

#### 실제로 눈에 띄는 셋

**추적손절이 이익을 토해낸다.** 99건이 MFE 1.74R까지 갔다가 평균 -0.16R로 끝난다. 활성화 1 ATR·추적 3 ATR이 그 변동성에서 너무 헐거운 것으로 보인다.

**최대보유가 최고 성적이다.** +1.78R에 MFE 2.61. 40세션을 꽉 채운 거래가 제일 좋다는 것은 보유기간 상한이 이익을 자르고 있다는 뜻이다.

**초기 손절이 -1R을 못 지킨다.** 손절가는 정의상 -1.0R인데 실현이 -1.15R이다. 갭 체결(`GAP_FILL` 28건)과 비용 때문이고 169건 × 0.15R ≈ 25R의 추가 손실이다. `MAE -2.19`는 우리 체결이 아니라 그날 장중 저가라 손실 크기가 아니다 — 손절은 -1R에 체결되고 가격이 그 뒤로 더 빠진 것이다.

#### 익스포저 6.3%의 원인은 리스크 한도가 아니다

|건너뛴 이유|비중|
|---|---|
|`SMA_NOT_ALIGNED`|47.51%|
|`NO_BREAKOUT`|36.78%|
|`LIQUIDITY_BELOW_MIN`|7.58%|
|`MAX_POSITIONS_REACHED`|**0.22%**|
|`DAILY_ENTRY_CAP`|**0.01%**|

84%가 추세·돌파 조건에서 죽고 동시보유·일일 상한에는 거의 안 걸린다. **한도를 풀어도 아무 일이 일어나지 않는다.** 익스포저가 낮은 것은 한도 탓이 아니라 신호가 안 나오기 때문이고, 넓힐 곳이 있다면 진입 조건 자체다.

-----

### 베이스라인부터 다시 세우는 계획 (2026-08-11 결정)

**규칙이 너무 두텁게 쌓여 귀속이 안 된다**는 것이 사용자 판단이다. 지금 `StrategyParameters`에 33개 파라미터가 있고 진입 한 번에 최소 열두 규칙이 겹친다 — 레짐 분류, 유니버스 필터(7.2), SMA 정렬, 20일 돌파, RS·추세 랭킹, ATR 수량, 9.2 리스크 한도 넷, 지정가 추격(+0.25 ATR), 갭 취소, 초기 손절, 추적손절, 시간손절, 최대보유, 실적 청산. 이 상태에서 파라미터를 흔들면 **어느 규칙이 엣지를 만드는지 알 수 없다.**

그래서 다음 단계는 파라미터 튜닝이 아니라 **베이스라인에서 규칙을 하나씩 얹으며 기여를 재는 것**(ablation)이다. 판정이 아니라 학습이 목적이다.

#### 이 방식에 필요한 것 — 홀드아웃을 보지 않는 실행 모드

**지금 구조로 ablation을 돌리면 홀드아웃이 매번 소모된다.** `plan_walk_forward`는 홀드아웃을 `folds`의 마지막 항목으로 넣고 `run` 단계가 그것을 `record_holdout_run`으로 기록하며, run_id에 정책 서명이 들어가므로 **규칙 조합을 바꿀 때마다 새 관측 1회**가 된다. 열 가지 조합을 보면 홀드아웃을 열 번 본 것이고 그 구간은 더 이상 표본 밖이 아니다.

연구 단계에는 **홀드아웃 구간을 아예 제외한 실행**이 필요하다. 판정할 준비가 됐을 때 한 번만 열어야 한다. 그것이 다음 세션의 첫 작업이다.

#### 재료는 이미 있다

- `trading/runs/`에 실행마다 마크다운과 JSON이 남아 조합 간 비교가 된다.
- 정책 서명이 규칙 조합을 식별하므로 같은 조합의 재실행은 새 관측이 아니다.
- 청산 해부·진입 깔때기가 "왜 그런 결과인지"를 준다. 위 실적 청산처럼 **집계만 보면 반대로 읽는다.**

-----


### 자기 잠금 두 개와 좀비 포지션 (2026-08-13~14)

ablation을 시작하려고 실행 모드를 만들다가, **2026-08-10 판정의 숫자가 전략의 성질이 아니었다**는 것을 먼저 발견했다.

#### 첫째 문 — 계좌 낙폭이 진입을 막는다

자산 고점이 2008-05-20이고 최종 낙폭이 7.49%였다. `HardLimits.drawdown_block_entries`가 0.07이므로 **2016-04-21 이후 10년 동안 신규 진입이 0건**이다. 포지션이 없으면 자산이 안 움직이고, 자산이 안 움직이면 낙폭도 안 내려간다 — 한 번 걸리면 못 빠져나오는 고리다. 2012년 이후 자산값이 하나뿐이었고 4,929세션 중 3,322세션이 포지션 0이었다.

그래서 그 판정의 "평균 익스포저 6.33%"와 "최대낙폭 7.49%"는 위험 관리의 결과가 아니라 **잠금이 만든 값**이다. 20.0의 "익스포저 6.3%는 리스크 한도 탓이 아니다"(건너뛴 이유의 84%가 SMA 정렬·돌파)는 실행 **전반부**에 대해서만 맞다. `DRAWDOWN_BLOCK_ENTRIES`가 skip의 0.52%로 보였던 것은 그 카운터가 유니버스×일 단위라 세션 단위 사실을 가렸기 때문이다.

#### 둘째 문 — 레짐에도 같은 낙폭이 있다

`HardLimits`의 낙폭 조치 넷을 해제하고 J/K 실험을 돌렸더니 이번에는 2011-08-04에 얼었다. `classify_regime`이 보는 `green_max_drawdown`(0.05)·`red_min_drawdown`(0.08)은 **`StrategyParameters`에 있다.** 계좌 낙폭 8% → RED → `rank_candidates`가 후보를 아예 안 만듦 → 진입 0 → 자산 정지. 4,677세션 중 3,928(84%)이 RED였고, 계좌 낙폭을 빼고 같은 세션을 다시 판정하면 GREEN 3,638 · YELLOW 109 · RED 930이다. **RED 84%는 시장이 아니라 우리 손익이 만든 값이었다.**

엔진에서 계좌 낙폭이 규칙에 닿는 곳은 여섯이고(`risk.py` 넷, `regime.py` 둘) 처음에 넷만 닫았다.

#### 셋째 — 상장폐지 포지션이 불멸이다

두 문을 다 닫고 나니 세 번째가 드러났다. 보유 세션을 **바가 있는 날만** 세고 있어서, 거래가 멈춘 종목이 영영 늙지 않아 만기가 오지 않았다. `jt-k42`가 다섯 칸을 물려 2021년 이후 자산값이 하나뿐이었다. 기준선에서 `POSITION_STALE`이 0이었던 이유는 그 실행이 2016년에 이미 죽어 들고 있는 것이 없었기 때문이다 — **버그가 다른 버그에 가려져 있었다.**

더 나쁜 경로도 있었다. 같은 티커를 다른 회사가 물려받으면(`MER`은 2008년 메릴린치, 2018-08-28부터 다른 계열) 몇 년 정지해 있던 좀비가 **다른 회사 주가로 부활한다.**

#### 상태 기계

사용자가 정한 전이다. 요점은 **바가 없다는 것 자체는 청산 신호가 아니고, 팔 수 없을 때 가짜 체결을 만들지 않는다**는 것이다.

```
ACTIVE ──청산 조건 + 거래가능──→ 정상 청산
       ──청산 조건 + 거래불가──→ EXIT_PENDING_UNTRADEABLE
       ──폐지·era 종료────────→ DELISTED_EXIT (재개를 기다리지 않는다)
EXIT_PENDING_UNTRADEABLE
       ──같은 security 재개───→ 첫 실제 거래 가능 가격(시초가)
       ──런 끝까지 미해결─────→ UNRESOLVED_EXIT (민감도 시나리오)
```

K=21에 8~15일째만 정지면 16일째 ACTIVE로 돌아와 21일째 정상 청산하고, 18~25일째 정지면 21일째에 상태만 바뀌었다가 26일째 첫 가격에 나간다.

`delistings` 표가 그 판정의 입력이다. 루프가 "오늘 이후로 바가 없다"를 직접 보면 미래를 보는 것이라, 적재 단계에서 날짜로 기록하고 루프는 `last_trade_date <= as_of`만 묻는다 — 멤버십 구간·실적 캘린더와 같은 등급의 사실이다. 실측 **294 폐지 · 23 미해결**이고, 벤더 폐지 목록(EODHD 2회 호출)이 288개를 확인해준다.

#### security identity는 전략 파라미터와 분리한다

`MER`류를 가르는 데 `max_hold`를 쓰려다 사용자가 막았다. **그러면 K=21과 K=42가 같은 티커를 다른 회사로 판정한다** — 보유기간의 효과를 비교하려는 실험에 데이터 정합성 처리가 섞여 들어간다. `IDENTITY_MAX_GAP_SESSIONS = 21`(≈1개월)을 데이터 계층에 두고 매매 결정에는 쓰지 않는다. 공백 길이 분포가 62·69·73·85·182·231…로 이어져 자를 자리가 없어 고른 값이고, 6~20세션까지 내리면 400건 넘게 딸려 온다. 걸리는 심볼은 20개이고 보고서가 이름으로 찍는다.

#### 민감도

정체불명 청산은 전 런에서 **단 1건, `RX`(IMS Health)**다. 마지막 10세션이 21.82~21.99에 못 박혀 있고(현금 인수 합의가에 수렴한 모양) 계열이 18개월 뒤 $5.05로 재개된다 — **그 포지션은 0원이 아니었다.** 따라서 `LAST_CLOSE`를 읽고 `ZERO`는 하한이 아니라 아티팩트로 본다. 명시적 폐지는 5~8건에 +$374~977로 무시할 수준이다.

-----

### J/K 실험 — 랭킹은 일을 하지만 벤치마크를 못 이긴다 (2026-08-14)

J=63·skip=5를 고정하고 K만 바꿨다. 진입은 RS-only(7.4 게이트 전부 해제, 점수도 `z(rs63_5)` 단독), 레짐은 라벨만, 낙폭 게이트는 해제. 구간 2007-01-04 ~ 2025-08-06으로 **홀드아웃을 제외해 소모하지 않았다.** 모든 런이 `UNDETERMINED`다.

|코어|거래|기대값|PF|총수익|MDD|Sharpe|익스포저|
|---|---|---|---|---|---|---|---|
|`jt-k21`|1,002|-0.015|0.97|-3.9%|12.5%|-0.03|17.5%|
|`jt-k42`|514|**+0.229**|1.29|+35.4%|9.4%|0.38|17.9%|
|`jt-core-exit`|1,584|-0.075|0.87|-23.1%|26.8%|-0.32|16.8%|

**1. 오래 들고 있을수록 낫다.** K=21 → K=42에서 부호가 바뀌고 양수 연도가 11/19 → 13/19이다. core-1 청산 한 벌은 만기 하나보다 나쁘다(양수 연도 5/19). 청산 해부도 같은 방향이다 — 손실은 사실상 `INITIAL_STOP` 하나(451건 -1.167R, -112,603)이고 끝까지 들고 간 `MAX_HOLD`는 111건 평균 **+1.795R**, 추적손절은 MFE 1.98까지 갔다가 +0.074R로 끝난다. **자르는 규칙이 잃고 버티는 규칙이 번다.**

**2. 양수 기대값 자체는 아무것도 증명하지 않는다.** 무작위 선택 대조군(`jt-random-k42`, 랭킹만 무작위, 10시드)이 **전부 양수**이고 PF도 전부 1을 넘는다(최소 +0.062 · 중앙 +0.173 · 최대 +0.314). `jt-k42`는 그 분포의 2/11위이고 z=0.77 — 잡음 안이다.

**3. 노출로 통제하면 랭킹이 이긴다.** 무작위는 익스포저가 26%로 더 높다 — **모멘텀 상위 종목은 ATR가 커서 `R/2ATR` 수량이 작아지고**, 같은 위험을 지면서 자본을 덜 쓴다. 각자의 노출 일치 벤치마크와의 격차는 `jt-k42` **-12.7%p**, 무작위 중앙 -44.7%p로 11개 중 1위다(최선의 시드 -20.4%p).

**4. 그래도 벤치마크는 못 이긴다.** 같은 비율로 SPY를 들고 있었으면 +48.1%인데 `jt-k42`는 +35.4%다. 비용을 되돌린 +47.4%가 벤치마크와 사실상 같으므로 **선택이 더한 값은 0에 가깝고 비용 $11,965가 그대로 격차**다. 위험 조정으로는 더 나쁘다(MDD 9.4% vs 7.3%, Sharpe 0.38 vs 0.69). 평균 노출 고정(+46.7%)과 일별 노출 일치(+48.1%)가 비슷하므로 **노출 타이밍의 기여도 거의 없다.**

벤치마크는 거래 비용을 내지 않고 그날의 전략 노출을 미리 아는 것으로 치므로 **실행 불가능하고 일부러 후하게 잡은 상대**다. 여기 지는 것이 결정적이지는 않지만 이기지 못한다는 것은 분명하다.

시장 상태별로는 세 코어 모두 `BULL/LOW_VOL`(59.5% 세션)에서 벌고 `BEAR/HIGH_VOL`(12.5%)·`CORRECTION/LOW_VOL`(11.8%)에서 잃는다. 다만 PF는 세 칸 다 1 근처라 **그 구간에서 산 거래가 나쁜 것이 아니라 그 구간을 들고 지낸 날이 나쁘다.**

#### 구조

규칙 한 벌이 코어이고 코어 하나가 파일 하나다(`trading/core/`). 정책 서명은 파라미터와 한도만 덮고 진입·청산·레짐 모드는 서명 밖이라, 코어를 파일로 남기지 않으면 "이 숫자는 어떤 규칙으로 냈나"가 실행 스크립트에 흩어진다. `CoreDefinition.run_kwargs()`가 규칙 설정의 유일한 통로이고 러너 둘이 그것만 쓴다. 산출물은 `trading/runs/<실험>/`에 모으고 폴더마다 README가 무엇을 물었고 무엇이 답이었는지 적는다.

**2026-08-10 기준선 보고서는 이 엔진과 다르다.** 정책 서명은 그대로지만 **서명은 엔진을 덮지 않는다.** 다시 돌리지 않기로 했고(core-1 회귀 계획 없음) 보고서 맨 위에 경고를 달았다.
