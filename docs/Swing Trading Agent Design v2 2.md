...
`PLACEHOLDER_COMMISSION_BPS = 25.0`(0.25%)는 **자리표시자**다. 사용자 계좌의 실제 요율로 바꿔야 하고, 그때까지 `CostModel.confirmed`가 거짓이다. 생존편향 플래그와 같은 이유로 코드가 기억한다. 확인되지 않은 비용으로 낸 결과는 12.4의 손익분기 게이트와 14.7의 성과 통과 기준에 쓸 수 없다.

이 숫자는 사소하지 않다. 왕복 0.5%는 거래당 계획 위험 0.25%의 두 배이므로, 요율이 실제로 이 수준이면 스윙 한 번의 기대값 구조가 달라진다. 12.4의 손익분기 게이트가 먼저 걸릴 가능성이 있다.
...
청산 규칙(7.5), 실행 loop, 성과 지표(14.7)다. 체결·거래 결과의 DB 기록(17장 `broker_orders`·`positions`·`trade_outcomes`)은 loop 단계에서 함께 만든다.
