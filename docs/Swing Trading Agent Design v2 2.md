...
수량과 최소수량 예외(9.1.1), 하드 한도(9.2), 체결·비용 모델(10.1·10.4), 청산 규칙(7.5), 성과 지표(14.7)다. 미체결은 일봉에서 20분 재산정을 모델링할 수 없으므로 재산정 없이 취소하고 `NO_FILL` 건수를 기록한다. 재산정을 흉내내는 쪽이 낙관 편향을 만들고, 기록한 건수가 나중에 실행과의 불일치 크기를 알려준다.
