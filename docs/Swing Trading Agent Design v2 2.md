...
#### 열린 질문: "계획된 총 손절 위험"의 뜻

합성 실행에서 **동시 보유가 5종목에 도달하지 못하고 4에서 멈췄다.** 원인은 9.2의 총 손절 위험 1.25% 한도이고, 그 한도를 "현재가 − 손절가"로 재고 있기 때문이다. 주가가 오르고 추적손절이 아직 따라오지 않은 포지션은 진입 시 계획한 0.25%보다 **많은** 위험을 쓰는 것으로 계산된다.

그런데 5종목 × 0.25% = 1.25%로 두 한도가 정확히 맞물린다. 이는 설계가 이 한도를 **진입 시점 계획 위험**의 합으로 의도했다는 강한 근거다. 지금 구현은 상승장에서 신규 진입을 스스로 막아 max_positions 5를 사실상 4로 줄인다.

대안은 `수량 × max(0, min(현재가, 진입가) − 손절가)`다. 주가가 올라도 계획 위험을 넘지 않고, 추적손절이 진입가 위로 가면 0이 된다. 다만 손절가가 진입가 아래인 승자의 실제 위험 금액은 과소평가한다. **사용자 승인 전에는 바꾸지 않는다.**

#### 남은 것

성과 지표(14.7)다. 체결 단위 DB 기록은 지표 단계에서 필요해지면 표를 추가한다.
