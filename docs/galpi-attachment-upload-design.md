...
- ~~인증된 원본 열기·다운로드 UI~~ → 2026-08-05 별도 단계로 구현·배포했다(아래 U-원본).
...
- Codex 제목·요약 보강(U3b)은 여전히 열려 있다. 이미지 library 승격은 U2와, 인증된 원본 열기는 별도 단계로 인수했다.

### U-원본 — 인증된 원본 열기 ✅ Pi 배포 완료 (2026-08-05)

U1부터 계속 미뤄온 항목이다. 문서와 이미지가 같은 경로를 쓴다.

- `lib/attachment-originals.js`가 scope별로 판정한다. 임시 첨부는 그 대화 안에서만(`session_id`가 아직 없으면 통과), 서재 첨부는 영구 자료라 대화와 무관하게 연다. 경로도 scope에 따라 `tmpDir` 안인지 Vault `_attachments` 안인지 나눠 확인한다.
- `GET /api/attachments/:attachmentId/original`은 기존 전역 토큰 인증을 그대로 타고 파일을 스트리밍한다. `X-Content-Type-Options: nosniff`와 `Cache-Control: no-store`를 붙인다. PDF·이미지만 `inline`이고 MD·TXT는 브라우저가 같은 오리진에서 렌더하지 않도록 `attachment`로 내려받게 한다. 한글 파일명은 ASCII 대체본과 RFC 5987을 함께 보낸다.
- **전체 sha256은 다시 계산하지 않는다.** 승격과 모델 입력 경로가 이미 확인하며 열 때마다 20MiB PDF를 통째로 읽는 비용이 크다. 경로와 크기 일치까지만 본다.
- 인증이 헤더 토큰이라 `<a href>`·`<img src>`로 못 연다. blob을 받아 object URL로 열고, iOS가 `await` 뒤의 `window.open`을 막으므로 **클릭 시점에 빈 창을 먼저 잡아두고** 준비되면 그 창을 보낸다. 막히면 `<a download>` 폴백으로 떨어진다.
