...
> 현재 상태 (2026-07-14): 1차 검색과 2차 논문 저장·중복 차단·논문 서재 UI, 일반 노트 열람과 공개 PDF 외부 링크를 Pi에 배포했다 (`c9bf470`). 실제 TradingAgents 논문 저장, 임베딩 생성, 하이브리드 검색 회수까지 인수했으며 실제 브라우저의 PDF 열기와 질문 컨텍스트·Codex 태그/링크 처리가 남았다.
