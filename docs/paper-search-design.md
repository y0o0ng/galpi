...
> 현재 상태 (2026-07-15): 1차 검색과 2차 논문 저장·중복 차단·논문 서재 UI, 일반 노트 열람과 공개 PDF 외부 링크를 Pi에 배포했다 (`c9bf470`). 실제 TradingAgents 논문 저장, 임베딩 생성, 하이브리드 검색 회수까지 인수했다. 2.5A Phase A 파서 spike와 Phase B 로컬 색인·검색은 구현 완료했다. 실제 TradingAgents PDF를 Mac 임시 DB·볼트에서 97개 청크로 색인해 방법론·실험·한계 질문 3/3의 목표 근거가 top 4에 들어왔다. 서버에는 전용 테이블 초기화만 연결했으며 Pi 앱 배포, URL 다운로드·SSRF 방어, 모델 도구와 UI는 아직 구현·인수 전이다.
...
- [ ] Pi 앱 배포 후 전용 테이블 생성·원본 캐시·검색 재검증
