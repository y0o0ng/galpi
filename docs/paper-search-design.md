...
> 현재 상태 (2026-07-14): 1차 검색과 2차 논문 저장·중복 차단·논문 서재 UI, 일반 노트 열람과 공개 PDF 외부 링크를 Pi에 배포했다 (`c9bf470`). 실제 TradingAgents 논문 저장, 임베딩 생성, 하이브리드 검색 회수까지 인수했다. 2.5A 전문 능동 독서는 설계 완료·구현 전이며, 실제 브라우저의 PDF 열기와 질문 컨텍스트·Codex 태그/링크 처리가 먼저 남았다.
...
**PDF 파서 결정은 구현 전 spike에서 확정한다.** 후보는 Pi의 Poppler `pdftotext`와 Node 기반 파서다. TradingAgents, 2단 편집 논문, 초록 없음, 스캔 PDF 함정 케이스로 페이지·섹션 보존율과 Pi 메모리/시간을 비교한다. Claude PDF 전체 입력은 표·그림 품질은 좋지만 TradingAgents에서 87,811토큰이므로 기본 파서로 쓰지 않는다. 자동 OCR도 첫 구현 범위에서 제외한다.
...
- [ ] TradingAgents와 함정 PDF로 페이지·섹션·2단 편집 추출 비교
- [ ] Pi 처리 시간·최대 RSS·실패 형태 측정 후 파서 선택
- [ ] 사용자에게 선택 이유·의존성·운영 영향을 설명하고 구현 컨펌

**B. 로컬 색인·검색 — 모델 호출 없이**

- [ ] 별도 `lib/paper-fulltext.js` + 전용 DB 테이블 + 원본 캐시
- [ ] 같은 PDF 동시 색인 직렬화, SHA-256 중복 차단, 실패 복구
- [ ] 실제 질문 세트에서 원하는 방법론·실험·한계 청크가 top 4에 회수
- [ ] 일반 노트 검색·Codex 큐·paper 노트 본문에 전문 청크가 섞이지 않음
