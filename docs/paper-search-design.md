...
> 현재 상태 (2026-07-15): 1차 검색과 2차 논문 저장·중복 차단·논문 서재 UI, 일반 노트 열람과 공개 PDF 외부 링크를 Pi에 배포했다 (`c9bf470`). 실제 TradingAgents 논문 저장, 임베딩 생성, 하이브리드 검색 회수까지 인수했다. 2.5A Phase A 파서, Phase B 로컬 색인·검색, Phase C 안전 다운로드·전문 도구도 Pi에 배포했다 (`4c3d07f`, `1078df5`, `6bd1f57`). Pi 실사용 DB에서 TradingAgents PDF 38페이지·104,235자를 97개 청크·97개 임베딩으로 색인했다. 초록 질문은 전문 도구 0회, 세부 질문은 1회·4,999자로 관련 섹션과 PDF 페이지를 반환했다. 10+10 품질 평가는 별도 컨펌 전이다.
...
- [x] **전문 = 외부 콘텐츠**: 도구 결과를 명령이 아닌 자료로 감싸고 PDF 안 URL·지시를 실행하지 않음
- [x] 전문 다운로드는 저장된 논문의 검증된 공개 HTTP(S) URL만 허용하고 사설 IP·리다이렉트·용량·시간을 제한
- [x] 전문 도구는 임의 경로/URL을 받지 않고 서버가 허용한 `paperId`와 반환된 `chunkId`만 받음
...
**2.5A — 저장 논문 전문 능동 독서** (Pi 배포·1+1 스모크 인수 완료)
...
- 리서치 에이전트가 `searchPapers`를 재사용: 주 1회 관심 키워드 검색 → 신규 상위 N개 자동 저장(이때만 자동 저장 허용, 별도 폴더/태그로 구분) → 시온 알림
...
- [x] 초록 기반 질문에서 전문 도구 호출 0회 (Pi TradingAgents 스모크: 0회·0자)
- [x] 전문 질문에서 관련 섹션·페이지를 최대 2회 검색하고 근거 위치와 함께 답변 (Pi 스모크: 1회·4,999자)
- [x] 질문당 전문 결과 누적 약 3,000토큰 상한과 전체 PDF 기본 입력 금지가 실제로 집행됨 (서버 하드 상한 10,000자, 실호출 4,999자)
...
2026-07-15 Mac 임시 환경에서 TradingAgents 38페이지·104,235자를 97개 청크로 색인했다. 임베딩 없이 BM25 계열 검색만 사용한 기준 질문에서도 Analyst Team, Maximum Drawdown/Simulation Setup, Conclusion이 각각 방법론·실험·한계 질문의 top 4 안에 들어왔다(3/3). 임베딩 하이브리드 경로는 주입형 벡터 단위 테스트를 통과했고 Phase C 코드가 기존 질문 임베딩을 전문 검색에 재사용한다. Pi 실사용 DB에서는 최초 전문 질문 때 OpenAI 청크 임베딩 97개를 일괄 생성했고, 질의 임베딩과 결합해 §5.1·§5.2, PDF p.10 근거를 회수했다.
...
- 단일 GPT: 기존 노트 검색이 후보 paper 노트를 회수한 경우에만 provider-neutral 전문 도구를 노출
...
- [x] Pi 실제 초록 질문 1개에서 전문 도구 0회·0자
- [x] Pi 실제 전문 질문 1개에서 search 1회·4,999자와 섹션·페이지 근거 반환
- [ ] 초록 답변 가능 질문 10개에서 도구 오호출 0회
- [ ] 전문 필요 질문 10개에서 섹션/페이지 근거 회수 성공률 8/10 이상
- [x] 어떤 모델 출력에도 호출 2회·누적 10,000자 상한이 깨지지 않음 (서버 단위 테스트와 Pi 4,999자 실호출)
- [ ] 같은 논문 질문 10회에서 PDF 재다운로드·재색인 0회
- [x] 전체 PDF 87,811토큰 경로가 명시적 후속 기능 외에는 호출되지 않음 (서버가 청크 단위만 반환)
