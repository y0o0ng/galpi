...
>
> 상태: A0·A1 shadow, S0b-2 Pi 실제 복구, S0c 공용 topic 쓰기 경로와 A1b 전역 청크 shadow 검색 Pi 인수 완료. 한국어 경계 보정은 로컬 후보이며 A2 전환 전 실사용 trace 관찰 중
>
...
이 후보는 아직 Pi에 배포하지 않았다. 남은 실패는 시 작품의 정확 청크 1건, 포트폴리오 원문 청크 1건, 환율 무관 질의의 false positive 1건이다. 최신성·supersession과 함께 실제 shadow trace를 더 관찰하기 전에는 A2로 전환하지 않는다.
