...
- `ambiguous`는 fail-close다. 지시 대상을 확정하지 못하면 **원문을 corpus에 던지는 약한 fallback을 만들지 않는다.** 검색이 없었다는 사실만 답변 모델에 알려서 되묻게 하고, 근거를 회수한 척하지 않는다. 해석기 오류·타임아웃·깨진 구조화 출력도 같은 경로다.
