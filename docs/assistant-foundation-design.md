...
>
> 위치: V4-A 논문 검색 완료 후, V4-B 음성과 V5-B 주식 분석 전에 진행
...
### 4.8 대화 의존 질의 해석 (Stage B)

답변 생성은 이미 같은 세션의 최근 대화를 본다. 그런데 회수는 이번 발화 하나로 다시 시작한다. 그래서 다음 흐름에서 근거가 사라진다.

```text
사용자: 수원에서 군대 사람들 만난 건 언제지?
시온:   8월 20일이야.
사용자: 그때 뭐 했었지?
```

세 번째 턴의 회수에 들어가는 것은 `그때 뭐 했었지?` 뿐이다. `searchPastMessages`는 현재 세션을 제외하므로 같은 대화 안의 지시 대상을 되살려주지 않고, `activeNotes`는 사용자가 직접 고른 자료일 뿐 직전 회수 결과가 아니다.

그래서 기존 회수 기계 **앞에** 한정된 질의 해석 단계를 하나 둔다. 특수 명령(`일정!` 같은 접두어)이나 일정 전용 검색기를 만들지 않는다.

```text
resolveRetrievalQuery({ userText, recentConversation }) -> pass | resolved | no_retrieval | ambiguous
```

|결과|검색 질의|corpus 회수|
|---|---|---|
|`pass`|사용자 발화 원문|기존 그대로|
|`resolved`|해석된 자립 질의|해석된 질의로|
|`no_retrieval`|없음|하지 않음|
|`ambiguous`|없음|하지 않음|

- `pass`는 자체 완결 발화다. 지시어·짧은 이어가기 표현만 보는 저렴한 어휘 게이트가 먼저 거르므로 **자립 질문에는 해석 모델을 부르지 않는다.**
- `resolved`는 최근 대화에서 지시 대상을 확정할 수 있을 때다. 해석기는 어시스턴트 발화도 근거로 쓴다 — 위 예시처럼 날짜를 먼저 말한 쪽이 어시스턴트일 수 있다.
- `no_retrieval`은 정상 성공이다. `응 그래서?`처럼 최근 대화만으로 답할 수 있는 턴에 억지 검색어를 만들지 않는다. 답변 생성은 평소대로 대화 히스토리를 받는다.
- `ambiguous`는 fail-close다. 지시 대상을 확정하지 못하면 **원문을 corpus에 던지는 약한 fallback을 만들지 않는다.** 검색이 없었다는 사실만 답변 모델에 알려서 되묻게 하고, 근거를 회수한 척하지 않는다. 해석기 오류·타임아웃·깨진 구조화 출력도 같은 경로다. **대화 의존 발화인데 근거로 삼을 같은 세션 대화가 아예 없을 때도 같다** — 새 세션의 첫 발화가 `그때 뭐 했었지?`면 지시 대상이 존재하지 않으므로 모델을 부르지 않고 바로 `ambiguous`다.
- **이번 턴 첨부가 있으면 해석 단계를 열지 않는다.** `이거`가 가리키는 것은 `<current_attachments>`와 첨부 도구 경로가 이미 정하는데, 해석기는 `userText`와 최근 대화만 보므로 그 대상을 알 수 없다. 대화로 다시 해석하면 없는 지시 대상을 만들거나 `ambiguous`로 첨부 경로를 막는다. 첨부 내용은 해석기에 넣지 않고, 첨부 라우팅·replay·저장 계약도 그대로다.

**원문과 검색 질의는 끝까지 다른 것이다.** `userText`는 DB·대화 히스토리·모델이 보는 사용자 발화 자리에 그대로 남고, `retrievalQuery`는 이 턴에만 사는 파생값이다. 사용자가 말하지 않은 문장을 말한 것처럼 저장하거나 보여주지 않는다. `resolved`일 때 해석된 질의는 **임베딩 · 노트 후보 랭킹 · 교차 세션 과거 대화 검색 · A2 전역 회수에 한꺼번에** 들어간다. 한쪽만 바꾸면 같은 턴 안에서 두 질의가 서로 다른 것을 찾는다.

해석기는 새 모델 선택 env 값을 만들지 않고 채팅과 같은 모델 스냅샷을 쓴다. 입력은 이미 답변 경로가 들고 있는 같은 세션 최근 대화(최대 6개 메시지 · 메시지당 400자)뿐이고, 이것을 위해 더 긴 대화 저장소를 새로 만들지 않는다. A2 랭킹 문턱·명시 노트 계약·컨텍스트 예산은 그대로다.

trace(schema v26)에는 `resolution_outcome`과 `retrieval_query_sha256`만 추가한다.

- `query_sha256`은 예나 지금이나 **사용자 발화 원문**의 hash다. 과거 trace의 뜻을 조용히 바꾸지 않는다.
- `retrieval_query_sha256`은 실제로 검색에 쓰인 질의의 hash이고, `no_retrieval`·`ambiguous`에서는 NULL이다.
- 해석된 질의 본문도 추가 대화 평문도 저장하지 않는다. NULL `resolution_outcome`은 v26 이전 실행이며 `pass`로 소급 해석하지 않는다.
- 중단(`abstentions`)은 "검색이 돌았는데 고른 근거가 없음"을 계속 뜻한다. corpus를 아예 돌리지 않은 턴은 별도로 센다.

**Stage C는 열지 않는다.** 직전 회수 결과 이월, N턴 회수 TTL, 세션 회수 캐시, 오래된 근거 축출 정책, 직전 회수 soft prior 같은 상태를 이 단계에서 만들지 않는다. Stage B가 자립 질의를 제대로 만들어내는데도 관련 근거가 반복해서 밀려나거나 사라진다는 실측이 나온 뒤에만 연다. 위 trace가 그 판단 근거다.
