...
# 54. Architecture Review — 계층 사이의 보완 계약 후보

> 상태: **REVIEW NOTES RECORDED — PROPOSALS NOT YET ADOPTED**
>
> 검토일: 2026-09-05
>
> 사용자는 아래 구조 검토 의견을 문서에 기록하도록 요청했다.
> 기록 승인은 새 semantic contract의 채택이나 구현 착수로 해석하지 않는다.
> 기존 R2 ACCEPTED/CLOSED 계약과 R3 사전등록은 그대로 유지한다.

## 54.1 Overall assessment

현재의 큰 골격은 유지하는 방향을 제안한다.

```text
원본 경험 보존
+ 근거·가정·시간·상황 범위를 가진 수정 가능한 해석
+ 동일 상태를 여러 질문에 맞게 읽는 Projection
+ 직접 검색과 제한된 연관 확장을 사용하는 Context Assembly
```

이 구조는 일반 선호, 상황별 예외, 일시적인 요청을 함께 표현할 수 있다.
User Model을 고정 프로필 복사본으로 만들지 않는 §45.5,
Projection마다 별도 truth/update engine을 두지 않는 §45.2,
원본 단서에서 주변 맥락으로 확장하는 §46.6~§46.11이 특히 일관된다.

아래 지적은 새 기억 유형이나 독립 subsystem 추가 제안이 아니다.
기존 구조에서 **갱신·근거·시간의 의미가 읽기까지 이어지는 계약**을 더 명확히 할 후보들이다.
현재 구현의 재현된 버그나 성능 검증 결과로 읽지 않는다.

## 54.2 Proposal A — 갱신 실패 시 보존 상태와 읽기 상태의 구분

근거: §42.2 SI-10은 검증 실패 시 다음을 유지한다.
...
저장 무결성을 위해 옛 상태를 보존하는 것과,
그 상태를 계속 현재 사실로 단정해 사용하는 것은 다른 결정이다.

예: 기존 기억은 서울 거주이고 사용자가 “부산으로 이사했어”라고 명시했는데,
새 derived state의 검증·갱신이 실패한 경우다.
옛 기록은 보존하더라도 다음 답변에서 서울 거주를 현재 사실로 단정하면 안 된다.

보완 제안:

- 명시적인 변경·정정 근거를 받았을 때, 영향을 받는 기존 claim의 재검토 필요 상태를 읽기 경로에서도 인식하게 한다.
- 갱신 완료 전 Context Assembly가 옛 claim만 단독으로 현재 사실처럼 제공하지 않도록, 새 근거·미해결 상태를 함께 전달하거나 단정을 제한하는 계약을 둔다.
- 원본 보존과 갱신의 원자성은 유지한다. 임의의 상충 문장 하나만으로 모든 관련 기억을 무효화하는 규칙은 제안하지 않는다.

후속 결정 대상은 이 상태를 여는 근거·적용 범위, 갱신 실패 동안의 읽기 동작,
재검토 완료 시 해제 조건이다. 정확한 필드·상태 이름·동기/비동기 처리는 미정이다.
§46.10~§46.13에 반증과 미해결 상태를 담을 자리는 있지만,
이 연결이 필수로 집행되는 조건은 더 명확해질 필요가 있다.

## 54.3 Proposal B — 동일 원천의 중복 지지와 자기 재진술 순환 방지

근거: §42.2 SI-3/SI-8과 §51.5는 source support와 retrieval/access를 구분한다.
추가로 다음 경로의 여러 표현을 독립된 지지 근거로 중복 계산하지 않는 계약을 제안한다.
...
이 경로는 다섯 개의 독립 관측이 아니다.
시온의 답변이 대화 원본으로 저장됐다는 사실은 “시온이 그렇게 말했다”의 근거이며,
그 안의 사용자 관련 주장이 새로 확인됐다는 뜻은 아니다.

보완 제안:

- 동일 원천의 요약·Projection·재진술은 존재하는 support를 전달하며, 그 자체로 독립적인 확인을 추가하지 않는다.
- 시온의 자기 재진술과 사용자의 새로운 명시적 확인·독립적인 관측을 구분한다.
- derived claim을 재입력된 assistant 문장으로 다시 지지하는 순환이 stable memory나 Core의 확신을 높이지 않도록 원천 계보를 확인한다.

이는 모든 반복 발언을 버리자는 제안이 아니다.
사용자가 실제로 다시 확인한 사실과 자동으로 복제된 표현을 구분하자는 것이다.
기존 Derivation / Dependency Provenance를 활용하되,
독립성·재진술·새 확인의 판정 방식과 support 표현은 후속 결정으로 남긴다.

## 54.4 Proposal C — 사실의 유효 시점과 시스템이 알게 된 시점의 구분

근거: §43.7은 temporal validity를 Timeline view와 분리하고,
§46.10은 질문의 시간 의도에 따라 현재·과거 상태를 회수한다.
여기에 다음 두 시간의 의미를 명시적으로 구분하는 보완을 제안한다.

```text
사실/상태가 실제로 적용되는 시점·기간
!=
그 근거를 시스템이 받아 해당 이해를 가질 수 있게 된 시점
```

예: 사용자가 9월에 “7월부터 부산에 살았어”라고 말했다.

- “8월에 어디 살았지?”는 나중에 확보한 근거까지 반영한 8월의 거주 상태를 묻는다.
- “8월 당시 너는 내가 어디 산다고 알고 있었지?”는 당시 시스템에 알려진 근거와 해석을 묻는다.

두 질문은 서로 다른 답을 가질 수 있다.
늦게 도착한 정보와 과거 오해를 설명하려면 유효 시점만으로는 부족하다.

보완 제안은 두 시간의 의미와 조회 의도를 분리하는 것이다.
정확한 schema, 원본 수신과 derived 해석 형성 시점의 기록 범위,
과거 상태의 재구성 방식은 후속 설계에서 결정한다.
실제 적용 시점이 불명확한 경우 수신 시각을 그 대용으로 확정하지 않는다.

## 54.5 Existing-contract reminder — 원본 직접 회수 경로 유지

이 항목은 새 구조 제안이 아니라 §46.6과 §46.11의 구현 해석을 재확인한다.

```text
source evidence / QA chunk
→ direct anchor retrieval
→ bounded reader-context reconstruction
```

원본 경험은 가설 생성·일반화·모든 Projection의 materialization을
필수로 완료해야만 회수할 수 있는 것이 아니다.
“지난번 우리가 무슨 이야기를 했지?”는 아직 정리되지 않은 원본에서도 답할 수 있어야 한다.
§51의 spine을 모든 기억이 반드시 순서대로 통과하는 실행 pipeline으로 번역하지 않는다.
원본 직접 회수에도 기존 접근·범위·삭제·authority 계약은 동일하게 적용한다.

## 54.6 Disposition

세 보완 제안은 **OPEN / 미채택**으로 기록한다.
후속 memory 구조·구현 검토에서 기존 계약과의 관계 및 채택 여부를 판단한다.
이 기록만으로 R3 측정창·실험 순서·threshold를 바꾸거나,
새 schema·runtime behavior·local-memory 연구 단계를 열지 않는다.
