...
네 개입을 모두 거쳤는데도 **§4 전체 경제성 허들을 통과하지 못하면**
**Momentum v2 standalone strategy는 종료한다.**

> **2026-08-17 정정 — Phase 6과 같은 contract repair를 §8에도 적용한다. 이번에는 결과를
> 본 뒤다.** 이 문단은 원래 종료 조건을 **`gap ≤ 0`** 하나로 적었는데, 그것은 §3의
> "네 alpha family를 다 써도 **경제성 허들**을 못 넘으면 종료"와 어긋난다.
>
> **실제로 그 틈이 벌어졌다.** 최종 후보 `jt-j126-k42-sma200`은 `G = +4.99% > 0`이라
> 옛 §8 문구로는 종료 조건이 **발동하지 않는데**, §4 허들은 **Sharpe 0.46 < 0.60**으로
> 미달이고 alpha 예산은 4/4 소진됐다. 그대로 두면 "gap이 양수니까 §8은 안 걸렸다,
> 그러니 계속해도 된다"는 **재개 루프홀**이 남는다.
>
> **이 수정은 결과를 구제하지 않고 종료를 넓힌다.** 옛 문구로 살아남았을 경우까지
> 종료시키는 방향이고, 이미 결과가 나온 뒤라는 사실을 여기에 명시해 둔다. **허들 값은
> 하나도 바꾸지 않았다.**
>
> **2026-08-17 이 조건은 실제로 발동했다** — `MOMENTUM_V2 = CLOSED / FROZEN`.
...
## 부록 — 닫힌 연구 기록 (2026-08-18 CLAUDE.md에서 이동)

`momentum-v2` 계열은 §8 종료 조건이 발동해 닫혔다. 아래는 그 과정에서 나온 관측과 해석으로, 앞으로의 작업을 구속하지 않으므로 인계 파일(`CLAUDE.md`)에서 이곳으로 옮겼다. **문장은 그대로이고 하나도 지우지 않았다.** 실험별 산출물은 `trading/runs/*/README.md`, 실측·완료 기록은 설계 문서 20.0절에 있다.

**여전히 구속하는 규칙은 옮기지 않고 `CLAUDE.md`에 남겼다** — 실전 경로 금지 · `bars_daily` 삭제 금지 · 홀드아웃 코드 불변식 · 코어 파일 규칙 · `paper-core-v1` 동결 · J·K·슬롯 재탐색 금지 · 미해결 종목 · 기록용 코어 보존 등이다. 판단이 애매한 항목은 남기는 쪽으로 갔다.
...
- **`RS = ABS(자기) − ABS(시장)` 항등식 때문에 두 신호가 강하게 중복될 구조적 이유가 있다.** 다만 항등식만으로 RS 상위의 `ABS > 0`이 **일반적으로 보장되지는 않는다** — 정확한 조건은 `RS > −ABS(시장)`이고 시장 모멘텀이 음수인 날에는 성립하지 않을 수 있다. **`NON_BINDING`의 직접 근거는 3,385일 전체의 composition change 0이지 산술이 아니다.** 원인 설명용 **exploratory 후속 진단**(97일 간격 34일 표본, **판정에 쓰지 않았고 문턱 변경 근거도 아니다**)에서는 시장 ABS가 양수인 UP 날짜가 91.9%였고 **그 34일 표본에서 TOP5의 ABS 최솟값이 +0.1876**이었다. 전체 3,385일의 TOP5 ABS 분포는 재지 않았다. **"ABS가 쓸모없다"로 일반화하지 않는다** — 잰 것은 `J126 + SMA200` + TOP5 구조뿐이다.
- **checksum이 두 유니버스에서 정확히 재현됐다.** control 관측 가중이 ALL +1.188% · NDX100 +2.043%로 PR #15와 일치한다. **관측 가중(checksum)과 날짜 가중(paired 기준선)을 갈라 두고 결과를 본 뒤 유리한 쪽을 고르지 않는다.** 다만 **이 러너의 control 표본 구조에서는 둘이 정의상 같게 닫힌다** — forward 없는 종목을 랭킹 전에 빼고 `min_score_population` 통과 날짜만 세므로 control TOP5가 항상 정확히 5관측이고(실측 `3,385 × 5 = 16,925`), 같은 크기 집단의 평균의 평균은 전체 평균이다. "필터가 안 바꿔서 우연히 같다"가 아니다.
- **FIP는 신호 층에서 떨어졌다 — `FIP_SIGNAL_REJECTED`**(2026-08-17 PR #20, `runs/fip-quality/`). `ID(126,5) = sign(PRET) × (n_neg − n_pos) / (n_pos + n_neg)`이고 `PRET`은 RS의 자기 항 `absolute_momentum(126,5)`을 그대로 쓴다. Stage A HARD 다섯 중 **넷 PASS · D 하나 FAIL**이다 — A `BINDING`(21.7%) · B `mean(D) +0.055%` · C `median 0.000%` · **D 위상 양수 59.5%(25/42) < 60%** · E NDX100 `+0.002%`. control checksum은 PR #17과 부동소수까지 일치했다(ALL +1.188% · NDX100 +2.043%).
- **ABS와 달리 필터는 실제로 걸렸다.** 3,385일 중 21.7%에서 TOP5 구성이 바뀌고 후보 교체율이 5.0%다(PR #17은 구성 변경 0일). 그래서 이번 실패는 **"안 걸려서 모른다"가 아니라 "걸렸는데 기여가 문턱에 못 미쳤다"**이다. 다만 효과 크기가 작다 — `mean(D) +0.055%`는 control `+1.188%` 대비 미미하고 NDX100은 사실상 0이다. **양수 `D` 날짜 11.7%를 "대부분 나빠졌다"로 읽지 않는다** — 78%의 날짜는 TOP5가 아예 안 바뀌어 `D_t = 0`이다.
- **처치가 작동하지 않은 것이 아니다.** 표시 180건이 **전부** 다음 세션 시초에 정상 체결됐고(untradeable 지연 0 · 폐지·미해결 선점 0 · 항등식 잔차 0) 평균 31세션을 잘라냈다. 거래가 445 → 529로 늘고 평균 보유가 41.8 → 31.1로 줄면서 비용이 $10,993 → **$12,556**으로 올랐다. **"작동 안 했다"와 "작동했는데 손해였다"를 섞지 않는다.**
- **`ΔS`의 절반 이상이 벤치마크 쪽이다.** `ΔB = −9.15%p`이고 `ΔG = −6.42%p`다. 시장이 SMA200 아래일 때 나가면 노출이 16.4% → 15.1%로 줄어 노출 일치 SPY도 함께 낮아진다. 그래서 **"전략이 −15.57%p 나빠졌다"와 "고른 종목이 나빠졌다"는 다르다** — 후자는 `ΔG`다. 보조 분해는 `ΔF = −3.92%p` · `ΔT = −5.23%p`로, PR #16과 달리 **타이밍 몫도 음수다.**
...
- **hard stop이 있어도 실현 손실은 planned stop risk를 넘는다 — 472건 전부(100%)다.** `STOP_FILL`조차 중앙 1.079배인데 이것은 갭이 아니라 **산술**이다 — 손절가에서 정확히 체결돼도 주당 손실이 `2ATR`이고 비용이 그 **위에** 얹히는데 `planned_risk = 수량 × 2ATR`는 비용을 포함하지 않는다. `GAP_FILL`은 중앙 1.223 · 최대 4.778이다. **그래서 A가 살아남았더라도 0.25%를 "maximum loss"라고 부를 수는 없었다.**
- **Phase 4는 alpha 개입이 아니라 의미 정정이다.** 그래서 PR #19 시점에 알파 개입은 셋이었고 남은 것이 FIP 하나였다. **그 마지막 카드는 PR #20에서 소진됐다.**
