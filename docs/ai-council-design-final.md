...
- **현재 단계:** V3.5와 V4-A 논문 검색·전문 능동 독서, V4.5 A0 기준선·A1 청크 회수 shadow mode와 S0b-2 Pi 승인형 복구까지 완료했다. `699d1e9`, `7e4fdc5`의 schema version 2·청크 hash/상태·승인형 apply를 운영 적용해 M60 원본은 유지하고 향수 토픽의 동일 복사본만 제거했으며, 제목 캐시 8건과 DB-only 청크 4건을 정리했다. 백업 `20260716-1733`, 활성 Markdown Q&A/ready 청크 65/65 일치, 감사 작업 0건, DB 무결성·외래키·전체 테스트 76개·서비스 재기동을 확인했다.
- **다음 단계 설계:** append/split/merge/archive를 공용 QA-LOG parser와 topic mutation queue로 직렬화하고 원자적 파일 교체·DB transaction·실패 복원을 기존 쓰기 경로에 적용한다. 이후 A1b에서 전역 청크 검색+노트 soft prior와 무관 evidence 중단 임계값을 같은 Pi 비공개 20개 평가로 shadow 보정한다. 상세 설계와 실측 근거·통과 기준은 [assistant-foundation-design.md](assistant-foundation-design.md)를 따른다.
- **완료:**
