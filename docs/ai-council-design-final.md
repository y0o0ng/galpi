...
- **현재 단계:** V3.5와 V4-A 논문 검색·전문 능동 독서, V4.5 A0 기준선·A1 청크 회수 shadow mode, S0b-2 Pi 승인형 복구와 S0c 공용 쓰기 경로까지 Pi 운영 적용을 마쳤다. append·split·merge·archive/restore가 strict QA-LOG parser와 전역 queue를 공유하고, 원문 precondition·임시 파일+rename·다중 파일 snapshot·SQLite transaction·실패 복원을 적용한다. Pi 전체 테스트 85개, readonly audit 65/65와 DB hash 불변, 서비스·인증 API를 인수했다.
- **다음 단계 설계:** A1b에서 전역 청크 검색+노트 soft prior와 무관 evidence 중단 임계값을 같은 Pi 비공개 20개 평가로 shadow 보정한다. 상세 설계와 실측 근거·통과 기준은 [assistant-foundation-design.md](assistant-foundation-design.md)를 따른다.
- **완료:**
