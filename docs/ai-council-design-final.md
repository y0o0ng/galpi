...
- **현재 단계:** V3.5와 V4-A 논문 검색·전문 능동 독서, V4.5 A0 기준선·A1 청크 회수 shadow mode와 S0b-2 Pi 승인형 복구까지 완료했다. S0c 공용 쓰기 경로는 `d41defe`에서 로컬 구현했다. append·split·merge·archive/restore가 strict QA-LOG parser와 전역 queue를 공유하고, 원문 precondition·임시 파일+rename·다중 파일 snapshot·SQLite transaction·실패 복원을 적용한다. 전체 테스트 85개, 임시 서버 HTTP mutation 시나리오와 로컬 audit Q&A 7/7을 통과했으며 Pi에는 아직 배포하지 않았다.
- **다음 단계 설계:** `d41defe`를 Pi에 배포해 전체 테스트 85개, readonly topic audit 65/65, 서비스·인증 API를 인수한다. 이후 A1b에서 전역 청크 검색+노트 soft prior와 무관 evidence 중단 임계값을 같은 Pi 비공개 20개 평가로 shadow 보정한다. 상세 설계와 실측 근거·통과 기준은 [assistant-foundation-design.md](assistant-foundation-design.md)를 따른다.
- **완료:**
...
- **S0c 공용 토픽 쓰기 경로(로컬 완료, `d41defe`):** append·split·merge·archive/restore를 `lib/topic-mutation.js`의 queue와 strict QA-LOG parser로 통합했다. 파일 원문 precondition, 원자 교체, 다중 snapshot, DB transaction, 실패 복원과 숨은 청크의 source 삭제 차단을 적용했다. merge 보관 실패는 부분 성공으로 남기지 않는다. 전체 테스트 85개와 임시 HTTP 통합 시나리오·로컬 readonly audit를 통과했으며 Pi 인수 전이다.
