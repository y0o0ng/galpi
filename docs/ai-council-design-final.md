...
- **2.5A 로컬 색인·검색(구현 완료, Pi 앱 배포 전):** `paper_documents`/`paper_chunks`, 해시 기반 숨김 원본 캐시, 동시 색인 직렬화, SHA-256·파서 버전 중복 차단, 중단 복구, 섹션·페이지 청킹과 BM25/선택적 임베딩 검색을 구현했다. Mac 임시 환경에서 TradingAgents를 97개 청크로 색인하고 방법론·실험·한계 질문 3/3의 목표 근거가 top 4에 들어오는 것을 확인했다. 서버에는 스키마 초기화만 추가했고 URL 다운로드·SSRF 방어, 모델 도구와 UI는 아직 연결하지 않았다.
