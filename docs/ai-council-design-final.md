...
- **현재 단계:** V3.5와 저장 상태 복원 수정은 Pi 인수 완료. V4 논문 검색 1차 코드는 `a70d9d0`으로 구현했고 Pi 배포·성공 검색 검증을 기다린다. Pi가 기본 런타임이며 Mac은 개발/편집 보조로 둔다.
- **완료:**
...
- **V4 논문 검색 1차(코드 완료, Pi 검증 대기):** Semantic Scholar 관련도 검색을 `lib/paper-search.js`에 분리하고 `/api/papers/search`, `/paper` 결과 카드, 선택적 `S2_API_KEY`, 10분 캐시, 429/timeout 처리와 단위 테스트를 추가했다. 저장·PDF·LLM 요약은 2차 이후로 유지한다.
