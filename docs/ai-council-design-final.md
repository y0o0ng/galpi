...
- **현재 단계:** V3.5와 저장 상태 복원 수정, V4 논문 검색 1차까지 Pi 인수 완료. 다음은 논문 저장·`paper_id` 중복 차단 2차다. Pi가 기본 런타임이며 Mac은 개발/편집 보조로 둔다.
- **완료:**
...
- **V4 논문 검색 1차(Pi 인수 완료):** Semantic Scholar 관련도 검색을 `lib/paper-search.js`에 분리하고 `/api/papers/search`, `/paper` 결과 카드, 선택적 `S2_API_KEY`, 10분 캐시, 429/5xx 지수 백오프·timeout 처리와 단위 테스트 10개를 추가했다. Pi 키 실검색 HTTP 200·결과 10개·성공 응답 캐시와 Playwright 카드 렌더링을 통과했으며, 저장·PDF·LLM 요약은 2차 이후로 유지한다.
