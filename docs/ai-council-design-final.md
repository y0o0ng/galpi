...
- **V4 논문 검색 1차(Pi 인수 완료):** Semantic Scholar 관련도 검색을 `lib/paper-search.js`에 분리하고 `/api/papers/search`, `/paper` 결과 카드, 선택적 `S2_API_KEY`, 10분 캐시, 429/timeout 처리와 단위 테스트 8개를 추가했다. Pi 키 실검색 HTTP 200·결과 10개·캐시와 Playwright 카드 렌더링을 통과했으며, 저장·PDF·LLM 요약은 2차 이후로 유지한다.
